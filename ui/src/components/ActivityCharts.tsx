...
function runDayTooltip(entry: DashboardRunActivityDay): string {
  const lines = [
    entry.total === 1
      ? t("activityCharts.tooltip.runsSingular", { defaultValue: "{{date}}: {{total}} run", date: entry.date, total: entry.total })
      : t("activityCharts.tooltip.runsPlural", { defaultValue: "{{date}}: {{total}} runs", date: entry.date, total: entry.total }),
  ];
  if (entry.succeeded > 0) lines.push(t("activityCharts.tooltip.succeeded", { defaultValue: "  succeeded: {{n}}", n: entry.succeeded }));
  if (entry.recovered > 0) lines.push(t("activityCharts.tooltip.recovered", { defaultValue: "  recovered: {{n}} (retry succeeded)", n: entry.recovered }));
  if (entry.failed > 0) {
    lines.push(t("activityCharts.tooltip.failed", { defaultValue: "  failed: {{n}}", n: entry.failed }));
    const codes = Object.entries(entry.failedByErrorCode ?? {}).sort((a, b) => b[1] - a[1]);
    for (const [code, count] of codes) lines.push(`    ${code}: ${count}`);
  }
  if (entry.other > 0) lines.push(t("activityCharts.tooltip.other", { defaultValue: "  other: {{n}}", n: entry.other }));
  return lines.join("\n");
}
...
export function RunActivityChart(props: RunChartProps) {
  ...
  if (!hasData) return <p className="text-xs text-muted-foreground">{t("activityCharts.empty.noRuns", { defaultValue: "No runs yet" })}</p>;

  const legendItems = [
    { color: runSegmentColors.succeeded, label: t("activityCharts.runLegend.succeeded", { defaultValue: "Succeeded" }) },
    ...(hasRecovered ? [{ color: runSegmentColors.recovered, label: t("activityCharts.runLegend.recovered", { defaultValue: "Recovered" }) }] : []),
    { color: runSegmentColors.failed, label: t("activityCharts.runLegend.failed", { defaultValue: "Failed" }) },
    { color: runSegmentColors.other, label: t("activityCharts.runLegend.other", { defaultValue: "Other" }) },
  ];
...
export function PriorityChart({ issues }: { issues: { priority: string; createdAt: Date }[] }) {
  ...
  if (!hasData) return <p className="text-xs text-muted-foreground">{t("activityCharts.empty.noTasks", { defaultValue: "No tasks" })}</p>;
    ...
    <div>
      <div className="flex items-end gap-(--sz-3px) h-20">
        {days.map(day => {
          ...
          const heightPct = (total / maxValue) * 100;
          return (
            <div key={day} className="flex-1 h-full flex flex-col justify-end" title={t("activityCharts.tooltip.issues", { defaultValue: "{{date}}: {{total}} issues", date: day, total })}>
              {total > 0 ? (
                <div className="flex flex-col-reverse gap-px overflow-hidden" style={{ height: `${heightPct}%`, minHeight: 2 }}>
      ...
      <DateLabels days={days} />
      <ChartLegend items={priorityOrder.map(p => ({ color: priorityColors[p], label: t(`activityCharts.priority.${p}`, { defaultValue: p.charAt(0).toUpperCase() + p.slice(1) }) }))} />
    </div>
  );
}
...
export function IssueStatusChart({ issues }: { issues: { status: string; createdAt: Date }[] }) {
  ...
  return (
    <div>
      ...
      <DateLabels days={days} />
      <ChartLegend items={statusOrder.map(s => ({ color: statusColors[s] ?? "var(--hex-6b7280)", label: t(`activityCharts.status.${s}`, { defaultValue: statusLabels[s] ?? s }) }))} />
    </div>
  );
}
