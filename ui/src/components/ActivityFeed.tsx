...
const FILTER_OPTIONS: Array<{ value: FilterValue; label: string; tKey: string }> = [
  { value: "all", label: "All", tKey: "activityFeed.filters.all" },
  { value: "in-progress", label: "In Progress", tKey: "activityFeed.filters.inProgress" },
  { value: "for-review", label: "In Review", tKey: "activityFeed.filters.inReview" },
  { value: "completed", label: "Done", tKey: "activityFeed.filters.done" },
];
...
function CollapsedFeedGroup({
  ...
  const actorName = actor?.name
    ?? (group.latestEvent.actorType === "system" ? t("activityFeed.actor.system", { defaultValue: "System" })
      : group.latestEvent.actorType === "user" ? t("activityFeed.actor.board", { defaultValue: "Board" })
      : t("activityFeed.actor.unknown", { defaultValue: "Unknown" }));
    ...
    <div>
      ...
      >
        ...
        <span className="flex-1 min-w-0 truncate">
          ...
          <span data-fc="verb" className="ml-1 text-(--hex-959596)">{t("activityFeed.group.updatesVerb", { defaultValue: "made {{n}} updates to", n: group.events.length })}</span>
...
export function ActivityFeed({ className }: ActivityFeedProps) {
        ...
        separator = (
          <div className="flex items-center gap-2 px-4 py-1.5" key={`sep-${index}`}>
            ...
            <span className="text-(length:--text-nano) font-medium text-muted-foreground uppercase tracking-wider">
              {t("activityFeed.separator.earlier", { defaultValue: "Earlier" })}
            </span>
            <div className="h-px flex-1 bg-border" />
          </div>
        );
  ...
  const renderGrouped = () => {
    ...
    return Array.from(groups.entries()).map(([groupKey, items]) => {
      ...
      const label = isOther
        ? t("activityFeed.groups.other", { defaultValue: "Other activity" })
  ...
  const emptyMessage = useMemo(() => {
    ...
    if (agents.length === 0) {
      return {
        text: t("activityFeed.empty.noAgents", { defaultValue: "No agents set up yet. Add an agent to get started." }),
        showPulse: false,
      };
    }
    ...
    if (allPaused) {
      return {
        text: t("activityFeed.empty.allPaused", { defaultValue: "All agents are paused. Resume agents from the sidebar to see activity." }),
        showPulse: false,
      };
    }
    return {
      text: t("activityFeed.empty.running", { defaultValue: "Your agents are running — activity will appear here shortly." }),
      showPulse: true,
    };
  }, [agents, t]);
    ...
    <aside className={cn("flex min-h-0 min-w-0 flex-1 flex-col bg-background", className)}>
      ...
      <div className="relative flex shrink-0 items-start justify-between gap-2 px-4 py-3">
        ...
        <div className="min-w-0 flex-1">
          <h3 className="text-sm font-semibold">{t("activityFeed.header.title", { defaultValue: "Agent Feed" })}</h3>
          <p className="text-xs text-muted-foreground">
            {t("activityFeed.header.subtitle", { defaultValue: "Live activity from your agents" })}
          </p>
        </div>
        <div className="flex items-center gap-1">
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                ...
                aria-label={t("activityFeed.actions.groupByTaskAria", { defaultValue: "group by task" })}
            ...
            <TooltipContent side="bottom">
              {groupMode === "flat"
                ? t("activityFeed.tooltip.groupByTask", { defaultValue: "Group by task" })
                : t("activityFeed.tooltip.showFlat", { defaultValue: "Show flat" })}
            </TooltipContent>
          </Tooltip>
          ...
          <DropdownMenu>
              ...
              <TooltipTrigger asChild>
                <DropdownMenuTrigger asChild>
                  <Button
                    ...
                    size="icon-sm"
                    className="shrink-0 text-muted-foreground"
                    aria-label={t("activityFeed.actions.filterByAria", { defaultValue: "filter by" })}
                  >
                    <ListFilter className="h-3.5 w-3.5" />
                  </Button>
                </DropdownMenuTrigger>
              </TooltipTrigger>
              <TooltipContent side="bottom">{t("activityFeed.tooltip.filterBy", { defaultValue: "Filter by" })}</TooltipContent>
            </Tooltip>
            <DropdownMenuContent align="end" className="w-48">
              ...
              >
                {FILTER_OPTIONS.map(({ value, label, tKey }) => (
                  <DropdownMenuRadioItem key={value} value={value}>
                    {t(tKey, { defaultValue: label })}
                  </DropdownMenuRadioItem>
                ))}
              </DropdownMenuRadioGroup>
              <DropdownMenuSeparator />
              ...
              >
                {t("activityFeed.actions.showAllActivity", { defaultValue: "Show all activity" })}
              </DropdownMenuCheckboxItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
          ...
          <div className="flex min-h-0 flex-1 items-center justify-center p-6 h-full">
            <div className="flex flex-col items-center gap-2 max-w-(--sz-16rem)">
              ...
              <p className="text-center text-sm text-muted-foreground">
                {emptyMessage?.text ?? t("activityFeed.empty.fallback", { defaultValue: "Activity from your agents will appear here." })}
              </p>
            </div>
          </div>
        ) : groupMode === "flat" ? (
