...
export function ActivityRow({ event, agentMap, userProfileMap, entityNameMap, entityTitleMap, className }: ActivityRowProps) {
  ...
  const actorName = actor?.name ?? (event.actorType === "system" ? t("activityRow.actor.system", { defaultValue: "System" }) : userProfile?.label ?? (event.actorType === "user" ? t("activityRow.actor.board", { defaultValue: "Board" }) : event.actorId || t("activityRow.actor.unknown", { defaultValue: "Unknown" })));
