  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent className="w-72 p-3" align="start">
        <Input
          placeholder={t("agentIconPicker.search.placeholder", { defaultValue: "Search icons..." })}
          value={search}
        ...
        <div className="grid grid-cols-7 gap-1 max-h-48 overflow-y-auto">
          ...
          {filtered.length === 0 && (
            <p className="col-span-7 text-xs text-muted-foreground text-center py-2">{t("agentIconPicker.emptyText", { defaultValue: "No icons match" })}</p>
          )}
        </div>
      </PopoverContent>
    </Popover>
