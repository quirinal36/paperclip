      ...
      <PopoverTrigger asChild>
        ...
        >
          <span className={cn("min-w-0 truncate", !selectedAgent && "text-muted-foreground")}>
            {selectedAgent?.name ?? placeholder ?? t("agentMultiSelect.select.placeholder", { defaultValue: "Select agent…" })}
          </span>
      ...
      <PopoverContent className="w-(--radix-popover-trigger-width) p-0" align="start">
        <div className="border-b border-border p-3">
          <Input
            ...
            placeholder={t("agentMultiSelect.filter.placeholder", { defaultValue: "Filter agents" })}
            className="h-8"
            autoFocus
          />
        </div>
        {agents.length === 0 ? (
          <div className="px-3 py-4 text-sm text-muted-foreground">{emptyMessage ?? t("agentMultiSelect.empty.noAgents", { defaultValue: "No agents yet." })}</div>
        ) : (
          <div className="max-h-60 overflow-y-auto py-1">
            {filteredAgents.map((agent) => (
              <button
                ...
                aria-label={t("agentMultiSelect.select.agentAriaLabel", { defaultValue: "Select {{name}}", name: agent.name })}
                onClick={() => {
                  onChange(agent.id);
                  setOpen(false);
                }}
            ...
            {filteredAgents.length === 0 ? (
              <div className="px-3 py-4 text-sm text-muted-foreground">{t("agentMultiSelect.empty.noMatches", { defaultValue: "No matches." })}</div>
            ) : null}
          </div>
        )}
      </PopoverContent>
            ...
            <span className="flex min-w-0 items-center">
              ...
              <span className="truncate">
                {triggerLabel ?? (selectedCount === 0
                  ? t("agentMultiSelect.trigger.selectAgents", { defaultValue: "Select agents" })
                  : selectedCount === 1
                    ? t("agentMultiSelect.trigger.selectedOne", { defaultValue: "{{selected}} agent selected", selected: selectedCount })
                    : t("agentMultiSelect.trigger.selectedMany", { defaultValue: "{{selected}} agents selected", selected: selectedCount }))}
              </span>
            </span>
            <ChevronRight className="h-4 w-4 shrink-0 text-muted-foreground" />
          </Button>
              ...
              return (
                ...
                >
                  <Checkbox
                    ...
                    aria-label={t("agentMultiSelect.option.allowAriaLabel", { defaultValue: "Allow {{name}}", name: agent.name })}
          ...
          <div className="flex items-center justify-between border-t border-border px-3 py-2">
            <span className="text-xs text-muted-foreground">
              {workingAgentIds.size === 0
                ? t("agentMultiSelect.footer.noneSelected", { defaultValue: "No agents selected" })
                : t("agentMultiSelect.footer.countSelected", { defaultValue: "{{selected}} selected", selected: workingAgentIds.size })}
            </span>
            <div className="flex items-center gap-2">
              {staged ? (
                <Button type="button" variant="ghost" size="sm" onClick={() => setOpen(false)} disabled={pending}>
                  {t("agentMultiSelect.actions.cancel", { defaultValue: "Cancel" })}
                </Button>
              ) : null}
              <Button
                type="button"
              ...
              >
                {staged
                  ? pending
                    ? t("agentMultiSelect.actions.saving", { defaultValue: "Saving…" })
                    : t("agentMultiSelect.actions.save", { defaultValue: "Save" })
                  : t("agentMultiSelect.actions.done", { defaultValue: "Done" })}
              </Button>
            </div>
          </div>
        </PopoverContent>
      ...
      {showSelectionPreview && selectedAgents.length > 0 ? (
        <div className="space-y-0.5">
          ...
          {selectedAgents.length > 3 ? (
            <p className="px-1.5 pt-0.5 text-xs text-muted-foreground">{t("agentMultiSelect.preview.andMore", { defaultValue: "and {{remaining}} more", remaining: selectedAgents.length - 3 })}</p>
          ) : null}
        </div>
      ) : null}
    </div>
