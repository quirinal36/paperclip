...
const FILTERS: Array<{ label: string; value: FilterValue; labelKey: string }> = [
  { label: "All", value: "all", labelKey: "artifactsPanel.filters.all" },
  { label: "In Progress", value: "in_progress", labelKey: "artifactsPanel.filters.inProgress" },
  { label: "For Review", value: "for_review", labelKey: "artifactsPanel.filters.forReview" },
  { label: "Completed", value: "completed", labelKey: "artifactsPanel.filters.completed" },
];
...
function statusBadge(status: string): { labelKey: string | null; label: string; className: string } {
  switch (status) {
    case "active":
    case "draft":
      return { labelKey: "artifactsPanel.status.inProgress", label: "In Progress", className: "bg-blue-500/10 text-blue-600 dark:text-blue-400" };
    case "ready_for_review":
      return { labelKey: "artifactsPanel.status.forReview", label: "For Review", className: "bg-amber-500/10 text-amber-600 dark:text-amber-400" };
    case "approved":
    case "merged":
      return { labelKey: "artifactsPanel.status.completed", label: "Completed", className: "bg-green-500/10 text-green-600 dark:text-green-400" };
    case "changes_requested":
      return { labelKey: "artifactsPanel.status.changesRequested", label: "Changes Requested", className: "bg-orange-500/10 text-orange-600 dark:text-orange-400" };
    case "failed":
      return { labelKey: "artifactsPanel.status.failed", label: "Failed", className: "bg-red-500/10 text-red-600 dark:text-red-400" };
    default:
...
export function ArtifactsPanel({ taskId, isAgentWorking, openDocKey, openDocTitle, onClearOpenDoc, onApprove, onReject }: ArtifactsPanelProps) {
  ...
  const effectiveViewingDoc = openDocKey
    ? { key: openDocKey, title: openDocTitle ?? t("artifactsPanel.documentViewer.defaultTitle", { defaultValue: "Document" }) }
    : viewingDoc;
  ...
  return (
    <div className="flex flex-col h-full" data-artifacts-panel>
      <div className="px-4 py-3 border-b border-border flex items-center gap-2">
        ...
        <h3 className="text-sm font-semibold">{t("artifactsPanel.title", { defaultValue: "Artifacts" })}</h3>
      </div>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-auto-hide">
        {isLoading ? (
          <div className="flex items-center justify-center py-8 text-muted-foreground text-sm">
            ...
            {t("artifactsPanel.list.loading", { defaultValue: "Loading..." })}
          </div>
        ) : filtered.length === 0 ? (
          <div className="px-4 py-8 text-center">
            ...
            <p className="text-sm text-muted-foreground">
              {workProducts?.length === 0
                ? t("artifactsPanel.list.emptyDefault", { defaultValue: "Your team's deliverables and plans will appear here as they're produced." })
                : t("artifactsPanel.list.emptyFiltered", { defaultValue: "No artifacts match this filter." })}
            </p>
          </div>
        ) : (
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        {showGenerating ? (
                          <Badge variant="ghost" className="[&>svg]:size-2.5 text-(length:--text-nano) px-1.5 bg-blue-500/10 text-blue-600 dark:text-blue-400">
                            ...
                            {t("artifactsPanel.item.generating", { defaultValue: "Generating..." })}
                          </Badge>
                        ) : (
                          <Badge variant="ghost" className={cn("text-(length:--text-nano) px-1.5", badge.className)}>
                            {badge.labelKey ? t(badge.labelKey, { defaultValue: badge.label }) : badge.label}
                          </Badge>
                        )}
                      </div>
                      {wp.summary && (
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto scrollbar-auto-hide p-4">
        {isLoading ? (
          <div className="flex items-center justify-center py-8 text-muted-foreground text-sm">
            <Loader2 className="h-4 w-4 mr-2 animate-spin" />
            {t("artifactsPanel.documentViewer.loading", { defaultValue: "Loading document..." })}
          </div>
        ) : error ? (
          <p className="text-sm text-muted-foreground">{t("artifactsPanel.documentViewer.notAvailable", { defaultValue: "Document not available yet." })}</p>
        ) : doc?.body ? (
          <div className="prose prose-sm dark:prose-invert max-w-none">
            <MarkdownBody>{doc.body}</MarkdownBody>
          </div>
        ) : (
          <p className="text-sm text-muted-foreground">{t("artifactsPanel.documentViewer.empty", { defaultValue: "Document is empty." })}</p>
        )}
      </div>
      ...
      {needsAction && (
        <div className="border-t border-border px-4 py-3 bg-background shrink-0">
          <p className="text-(length:--text-micro) text-muted-foreground mb-2">{t("artifactsPanel.documentViewer.reviewPrompt", { defaultValue: "This document needs your review." })}</p>
          <div className="flex items-center gap-3">
            <Button size="lg" className="h-11 px-8 text-base font-semibold flex-1 rounded-lg bg-green-700 hover:bg-green-800 text-white border-0" onClick={onApprove}>
              {t("artifactsPanel.actions.approve", { defaultValue: "Approve" })}
            </Button>
            <Button size="lg" className="h-11 px-8 text-base font-semibold flex-1 rounded-lg bg-red-900 hover:bg-red-950 text-white border-0" onClick={() => {
              ...
              {t("artifactsPanel.actions.reject", { defaultValue: "Reject" })}
            </Button>
          </div>
        </div>
      )}
        ...
        <div className="border-t border-green-500/30 bg-green-500/5 px-4 py-3 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <p className="text-(length:--text-compact) font-medium text-green-700 dark:text-green-400">
              {t("artifactsPanel.documentViewer.approved", { defaultValue: "Approved — hire tasks created" })}
            </p>
          </div>
        </div>
      )}
        ...
        <div className="border-t border-orange-500/30 bg-orange-500/5 px-4 py-3 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <p className="text-(length:--text-compact) font-medium text-orange-700 dark:text-orange-400">
              {t("artifactsPanel.documentViewer.changesRequested", { defaultValue: "Changes requested — CEO is revising" })}
            </p>
          </div>
        </div>
      )}
