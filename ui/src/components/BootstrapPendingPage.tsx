  ...
  return (
    <div className="mt-6 border-t border-border pt-5">
      <div className="flex items-center gap-2 text-sm font-medium">
        ...
        <span>
          {t("bootstrapPendingPage.cliFallback.preferHostPrompt", {
      ...
      <p className="mt-2 text-sm text-muted-foreground">
        {hasActiveInvite
          ? t("bootstrapPendingPage.cliFallback.activeInvite", {
              defaultValue:
          ...
          : t("bootstrapPendingPage.cliFallback.runCommand", {
              defaultValue:
...
function claimErrorCopy(error: BootstrapPendingPageProps["claimError"]) {
  if (error?.status === 409) {
    return {
      title: t("bootstrapPendingPage.errors.alreadyClaimed.title", {
        defaultValue: "Someone else has already claimed this instance.",
      }),
      body: t("bootstrapPendingPage.errors.alreadyClaimed.body", {
        defaultValue:
  ...
  if (error?.status === 401) {
    return {
      title: t("bootstrapPendingPage.errors.sessionExpired.title", {
  ...
  return {
    title: t("bootstrapPendingPage.errors.serverUnreachable.title", {
...
export function BootstrapPendingPage({
    ...
    return (
      <StateChrome>
        <h1 className="text-xl font-semibold">
          {t("bootstrapPendingPage.waiting.title", {
            defaultValue: "This Paperclip is waiting on its first admin",
          })}
        </h1>
        <p className="mt-2 text-sm text-muted-foreground">
          {t("bootstrapPendingPage.waiting.description", {
            defaultValue:
        ...
        <p className="mt-4 text-xs text-muted-foreground">
          {t("bootstrapPendingPage.waiting.publicModeNote", {
            defaultValue:
    ...
    return (
      <StateChrome>
        <div className="flex items-start gap-3">
          ...
          <div>
            <h1 className="text-xl font-semibold">
              {t("bootstrapPendingPage.success.title", {
                defaultValue: "You're the instance admin",
              })}
            </h1>
            <p className="mt-2 text-sm text-muted-foreground">
              {t("bootstrapPendingPage.success.description", {
                defaultValue:
        ...
        <div className="mt-5 flex items-center gap-3">
          ...
          <span className="text-sm text-muted-foreground">
            {t("bootstrapPendingPage.success.redirecting", { defaultValue: "Redirecting..." })}
          </span>
        </div>
        <div className="mt-5">
          <Button asChild variant="outline">
            <a href="/">
              {t("bootstrapPendingPage.success.continueToDashboard", {
  ...
  if (!session) {
    return (
      <StateChrome>
        <h1 className="text-xl font-semibold">
          {t("bootstrapPendingPage.setup.title", {
            defaultValue: "Finish setting up this Paperclip",
          })}
        </h1>
        <p className="mt-2 text-sm text-muted-foreground">
          {t("bootstrapPendingPage.setup.signInDescription", {
            defaultValue:
        ...
        <div className="mt-5">
          <Button asChild>
            <Link to="/auth?next=/">
              {t("bootstrapPendingPage.setup.signInAction", {
  ...
  return (
    <StateChrome>
      ...
      <p className="mt-2 text-sm text-muted-foreground">
        {t("bootstrapPendingPage.setup.claimDescription", {
          defaultValue:
      ...
      <div className="mt-5 flex flex-wrap items-center gap-3">
        <Button onClick={onClaim} disabled={isClaiming}>
          ...
          {isClaiming
            ? t("bootstrapPendingPage.setup.claiming", { defaultValue: "Claiming..." })
            : t("bootstrapPendingPage.setup.claimAction", { defaultValue: "Claim this instance" })}
        </Button>
        <span className="text-sm text-muted-foreground">
          {t("bootstrapPendingPage.setup.signedInAs", { defaultValue: "Signed in as " })}
          <span className="font-medium text-foreground">{displayIdentity(session)}</span>
        </span>
      </div>
      <p className="mt-3 text-xs text-muted-foreground">
        {t("bootstrapPendingPage.setup.wrongAccount", { defaultValue: "Wrong account?" })}{" "}
        <Link to="/auth?next=/" className="underline underline-offset-2">
          {t("bootstrapPendingPage.setup.switchAccount", { defaultValue: "Switch account" })}
        </Link>
        .
      </p>
      {claimError && (
