    ...
    <span className="relative inline-flex min-w-0 max-w-full">
      ...
      {copied ? (
        ...
        >
          ...
          {t("caseFieldsPanel.copied", { defaultValue: "Copied" })}
        </span>
      ) : null}
    </span>
  );
  ...
  if (typeof value === "boolean") {
    return value ? (
      <Check
        ...
        aria-label={t("caseFieldsPanel.value.booleanTrue", { defaultValue: "true" })}
      />
    ) : (
      <EmptyValue />
    );
  ...
  return (
    <section className="space-y-2">
      <div className="flex items-baseline gap-2">
        <h2 className="text-sm font-semibold">{t("caseFieldsPanel.title", { defaultValue: "Fields" })}</h2>
        <span className="text-xs text-muted-foreground">
          {t("caseFieldsPanel.subtitle", {
      ...
      <Card className="gap-0 py-0">
        {entries.length === 0 ? (
          <div className="px-4 py-3 text-sm text-muted-foreground">
            {t("caseFieldsPanel.empty", { defaultValue: "No fields set" })}
          </div>
        ) : (
