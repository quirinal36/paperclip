  ...
  return (
    <span className={cn("relative inline-flex min-w-0 max-w-full", containerClassName)}>
      <button
        ...
        title={value}
        aria-label={t("caseIdentifierKey.copyAriaLabel", { defaultValue: "Copy {{label}} {{value}}", label, value })}
        onClick={handleCopy}
      >
        {value}
      </button>
      {copied ? (
        ...
        >
          ...
          {t("caseIdentifierKey.copied", { defaultValue: "Copied" })}
        </span>
      ) : null}
    </span>
  );
...
export function CaseIdentifierKey({
  ...
  return (
    ...
    >
      <CaseCopyableToken
        value={identifier}
        label={t("caseIdentifierKey.label.caseId", { defaultValue: "case ID" })}
