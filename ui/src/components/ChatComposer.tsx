...
export const ChatComposer = forwardRef<ChatComposerHandle, ChatComposerProps>(function ChatComposer(
  ...
  const resolvedPlaceholder =
    placeholder ?? t("chatComposer.placeholder.default", { defaultValue: "Message…" });
  const resolvedSendLabel =
    sendLabel ?? t("chatComposer.actions.send.label", { defaultValue: "Send message" });
  const attachLabel = t("chatComposer.actions.attach.label", { defaultValue: "Attach files" });
      ...
      {isDragOver && canAttach ? (
        ...
        >
          <span className="inline-flex items-center gap-2">
            ...
            {t("chatComposer.dropOverlay.label", { defaultValue: "Drop to attach" })}
          </span>
        </div>
      ) : null}
          ...
          {attachments.map((attachment) => {
            ...
            const statusLabel =
              attachment.status === "uploading"
                ? t("chatComposer.attachment.status.uploading", { defaultValue: "Uploading…" })
                : attachment.status === "error"
                  ? attachment.error ??
                    t("chatComposer.attachment.status.uploadFailed", { defaultValue: "Upload failed" })
                  : attachment.inline
                    ? t("chatComposer.attachment.status.insertedInline", { defaultValue: "Inserted inline" })
                    : t("chatComposer.attachment.status.attached", { defaultValue: "Attached" });
            return (
              <div
                key={attachment.id}
                className={cn(
