...
export function DocumentAnnotationPanel(props: AnnotationPanelProps) {
      ...
      <Sheet open={props.open} onOpenChange={props.onOpenChange}>
        ...
        >
          <SheetTitle className="sr-only">
            {t("documentAnnotationPanel.sheetTitle", {
              defaultValue: "Comments on {{documentKey}} revision {{revisionNumber}}",
              documentKey: props.documentKey,
              revisionNumber: props.documentRevisionNumber,
            })}
          </SheetTitle>
  ...
  return (
    <aside
      role="complementary"
      aria-label={t("documentAnnotationPanel.regionLabel", {
        defaultValue: "Annotations for {{documentKey}}, revision {{revisionNumber}}",
...
function AnnotationPanelBody(props: AnnotationPanelProps) {
  ...
  const currentUser = useMemo(() => {
    ...
    return {
      id: user?.id ?? null,
      name: user?.name?.trim() || user?.email?.trim() || t("documentAnnotationPanel.currentUser.you", { defaultValue: "You" }),
  ...
  const createThread = useMutation({
    ...
    onError: (error, _body, context) => {
      ...
      setMutationError(error instanceof Error && error.message
        ? error.message
        : t("documentAnnotationPanel.errors.createComment", { defaultValue: "Failed to create comment." }));
    },
  ...
  const addReply = useMutation({
    ...
    onError: (error, _variables, context) => {
      ...
      setMutationError(error instanceof Error && error.message
        ? error.message
        : t("documentAnnotationPanel.errors.addReply", { defaultValue: "Failed to add reply." }));
    },
  ...
  const updateStatus = useMutation({
    ...
    onError: (error, _variables, context) => {
      ...
      setMutationError(error instanceof Error && error.message
        ? error.message
        : t("documentAnnotationPanel.errors.updateStatus", { defaultValue: "Failed to update comment status." }));
    },
    ...
    <>
      ...
      >
        <span className="text-(length:--text-micro) tabular-nums text-muted-foreground">
          {t("documentAnnotationPanel.revisionShort", {
        ...
        <Button
          ...
          aria-label={t("documentAnnotationPanel.actions.closePanel", { defaultValue: "Close annotation panel" })}
        >
          <X className="h-4 w-4" />
        </Button>
      </div>
      ...
      {props.pendingAnchor ? (
        <div className="border-t border-border bg-popover px-3 py-2">
          ...
          <Textarea
            ...
            placeholder={t("documentAnnotationPanel.composer.placeholder", { defaultValue: "Write a comment…" })}
          ...
          <div className="mt-2 flex items-center justify-end gap-2">
            ...
            >
              {t("documentAnnotationPanel.actions.cancel", { defaultValue: "Cancel" })}
            </Button>
            <Button
              type="button"
              size="sm"
            ...
            >
              {createThread.isPending
                ? t("documentAnnotationPanel.actions.posting", { defaultValue: "Posting…" })
                : t("documentAnnotationPanel.actions.comment", { defaultValue: "Comment" })}
            </Button>
          </div>
        </div>
      ) : null}
        ...
        {props.expanded ? (
          <div className="space-y-2 px-3 py-2">
            ...
            <Textarea
              ...
              placeholder={t("documentAnnotationPanel.reply.placeholder", { defaultValue: "Reply…" })}
              className="resize-y rounded-none text-sm"
            ...
            <div className="flex items-center justify-end gap-2">
              ...
              >
                {thread.status === "resolved" ? (
                  <>
                    <RotateCcw className="h-3 w-3" /> {t("documentAnnotationPanel.actions.reopen", { defaultValue: "Reopen" })}
                  </>
                ) : (
                  <>
                    <Check className="h-3 w-3" /> {t("documentAnnotationPanel.actions.resolve", { defaultValue: "Resolve" })}
                  </>
                )}
              </Button>
              <Button
              ...
              >
                {props.pendingReply
                  ? t("documentAnnotationPanel.actions.sending", { defaultValue: "Sending…" })
                  : t("documentAnnotationPanel.actions.reply", { defaultValue: "Reply" })}
              </Button>
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button
                    ...
                    size="icon-xs"
                    className="text-muted-foreground"
                    title={t("documentAnnotationPanel.actions.moreActionsTitle", { defaultValue: "More actions" })}
                    aria-label={t("documentAnnotationPanel.actions.moreActionsLabel", { defaultValue: "More thread actions" })}
                  >
                ...
                <DropdownMenuContent align="end">
                  ...
                  >
                    ...
                    {t("documentAnnotationPanel.actions.copyLink", { defaultValue: "Copy link" })}
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
            </div>
          ...
          <p className="px-3 py-2 text-xs text-muted-foreground">
            <span className="font-medium text-foreground">
              {thread.comments.length === 1
                ? t("documentAnnotationPanel.thread.commentCountOne", {
                    defaultValue: "{{count}} comment",
                    count: thread.comments.length,
                  })
                : t("documentAnnotationPanel.thread.commentCountOther", {
    ...
    >
      <div className="mb-0.5 flex items-center justify-between gap-2 text-(length:--text-micro)">
        <span className="flex min-w-0 items-center gap-1.5">
          ...
          {author.role === "agent" ? (
            <span className="text-muted-foreground">· {t("documentAnnotationPanel.comment.agentRole", { defaultValue: "agent" })}</span>
          ) : null}
        </span>
...
function resolveAuthor(
  ...
  return {
    name: comment.authorType === "agent"
      ? t("documentAnnotationPanel.author.agent", { defaultValue: "Agent" })
      : t("documentAnnotationPanel.author.board", { defaultValue: "Board" }),
