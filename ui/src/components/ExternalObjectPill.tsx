...
function githubObjectLabel(url: string | null | undefined): string | null {
  ...
  try {
    ...
    if (kind === "pull") return t("externalObjectPill.github.pullRequest", { defaultValue: "PR {{number}}", number });
    if (kind === "issues") return t("externalObjectPill.github.issue", { defaultValue: "Issue {{number}}", number });
    return null;
  } catch {
    return null;
  }
