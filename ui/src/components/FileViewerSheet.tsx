...
function normalizeError(error: unknown): FileViewerErrorShape {
  ...
  return { status: 0, code: "", message: t("fileViewerSheet.errors.generic", { defaultValue: "Something went wrong." }) };
}
...
export function describeDenial(code: string, fallback: string): { title: string; body: string; icon: ReactNode } {
  ...
  if (lower.includes("policy") || lower.includes("denied") || lower.includes("sensitive")) {
    return {
      ...
      title: t("fileViewerSheet.denial.policy.title", { defaultValue: "Viewer blocked for this file" }),
      body: t("fileViewerSheet.denial.policy.body", { defaultValue: "This file is not available through the viewer because it may contain sensitive data." }),
    };
  }
  if (lower.includes("outside") || lower.includes("traversal")) {
    return {
      icon: <Ban aria-hidden="true" className="h-6 w-6 text-red-500" />,
      title: t("fileViewerSheet.denial.outside.title", { defaultValue: "Path is outside the workspace" }),
      body: t("fileViewerSheet.denial.outside.body", { defaultValue: "The viewer can only open files that live under the issue's workspace." }),
    };
  }
  if (lower.includes("archive") || lower.includes("cleaned")) {
    return {
      icon: <FolderOpen aria-hidden="true" className="h-6 w-6 text-muted-foreground" />,
      title: t("fileViewerSheet.denial.archived.title", { defaultValue: "Workspace is no longer available" }),
      body: t("fileViewerSheet.denial.archived.body", { defaultValue: "The isolated worktree for this issue has been cleaned up, so files cannot be previewed." }),
    };
  }
  if (lower.includes("remote")) {
    return {
      icon: <AlertTriangle aria-hidden="true" className="h-6 w-6 text-amber-500" />,
      title: t("fileViewerSheet.denial.remote.title", { defaultValue: "Remote workspace preview not supported" }),
      body: t("fileViewerSheet.denial.remote.body", { defaultValue: "This workspace is hosted remotely and is not available for inline preview yet." }),
    };
  }
  if (lower.includes("too_large") || lower.includes("size")) {
    return {
      icon: <AlertTriangle aria-hidden="true" className="h-6 w-6 text-amber-500" />,
      title: t("fileViewerSheet.denial.tooLarge.title", { defaultValue: "File is too large to preview" }),
      body: t("fileViewerSheet.denial.tooLarge.body", { defaultValue: "This file exceeds the supported preview size." }),
    };
  }
  if (lower.includes("binary") || lower.includes("unsupported")) {
    return {
      icon: <AlertTriangle aria-hidden="true" className="h-6 w-6 text-amber-500" />,
      title: t("fileViewerSheet.denial.unsupported.title", { defaultValue: "Preview not supported for this file type" }),
      body: t("fileViewerSheet.denial.unsupported.body", { defaultValue: "This file does not have a text, image, or video preview available." }),
    };
  }
  return {
    icon: <Ban aria-hidden="true" className="h-6 w-6 text-red-500" />,
    title: t("fileViewerSheet.denial.generic.title", { defaultValue: "Can't preview this file" }),
    body: fallback || t("fileViewerSheet.denial.generic.body", { defaultValue: "The viewer was unable to load this file." }),
  };
}
        ...
        <>
          ...
          {state?.line ? (
            <>
              <span aria-hidden="true" className="opacity-50">·</span>
              <span>
                {t("fileViewerSheet.metadata.line", { defaultValue: "Line {{line}}", line: state.line })}
                {state.column ? t("fileViewerSheet.metadata.column", { defaultValue: ", Col {{column}}", column: state.column }) : ""}
              </span>
            </>
          ) : null}
        </>
      ) : state ? (
        <span className="h-3 w-28 rounded bg-muted animate-pulse" aria-label={t("fileViewerSheet.metadata.loadingDetails", { defaultValue: "Loading file details" })} />
      ) : null}
    </div>
  );
}
...
export function FileContentViewer({ content, highlightedLine, onLoaded }: FileContentViewerProps) {
  ...
  useEffect(() => {
    ...
    onLoaded?.(
      lines.length === 1
        ? t("fileViewerSheet.announce.fileLoadedOne", { defaultValue: "File loaded, {{count}} line.", count: lines.length })
        : t("fileViewerSheet.announce.fileLoadedOther", { defaultValue: "File loaded, {{count}} lines.", count: lines.length }),
    );
  }, [lines, onLoaded]);
  ...
  if (resource.previewKind === "image") {
    ...
    if (!dataUrl) {
      return (
        <FileViewerStateView
          icon={<AlertTriangle aria-hidden="true" className="h-6 w-6 text-amber-500" />}
          title={t("fileViewerSheet.image.unavailable", { defaultValue: "Image preview unavailable" })}
        />
      );
    }
    return (
  ...
  if (resource.previewKind === "video") {
    ...
    if (!dataUrl) {
      return (
        <FileViewerStateView
          icon={<AlertTriangle aria-hidden="true" className="h-6 w-6 text-amber-500" />}
          title={t("fileViewerSheet.video.unavailable", { defaultValue: "Video preview unavailable" })}
        />
      );
    }
    return (
      <div className="flex flex-1 items-center justify-center overflow-auto bg-black p-4">
        <video
          ...
          preload="metadata"
          playsInline
          aria-label={t("fileViewerSheet.video.ariaLabel", { defaultValue: "Video preview: {{title}}", title: resource.title })}
  ...
  if (resource.previewKind === "unsupported" || !lines) {
    return (
      <FileViewerStateView
        icon={<AlertTriangle aria-hidden="true" className="h-6 w-6 text-amber-500" />}
        title={t("fileViewerSheet.unsupported.title", { defaultValue: "Preview not supported for this file type" })}
        body={resource.contentType ? t("fileViewerSheet.unsupported.contentType", { defaultValue: "Content type: {{contentType}}", contentType: resource.contentType }) : undefined}
      />
    );
  }
  ...
  const rawSourceView = (
    <div
      ref={codeScrollRef}
      role="region"
      aria-label={t("fileViewerSheet.code.sourceLabel", { defaultValue: "{{title}} source", title: resource.title })}
      tabIndex={0}
  ...
  return (
    <div className="relative flex min-h-0 flex-1 flex-col">
      <div className="absolute right-3 top-3 z-20">
        <div
          role="group"
          aria-label={t("fileViewerSheet.markdown.modeGroupLabel", { defaultValue: "Markdown preview mode" })}
        ...
        >
          <Button
            ...
            size="icon-sm"
            aria-label={t("fileViewerSheet.markdown.showRendered", { defaultValue: "Show rendered Markdown" })}
            title={t("fileViewerSheet.markdown.renderedTitle", { defaultValue: "Rendered Markdown" })}
          ...
          <Button
            ...
            size="icon-sm"
            aria-label={t("fileViewerSheet.markdown.showRaw", { defaultValue: "Show raw Markdown" })}
            title={t("fileViewerSheet.markdown.rawTitle", { defaultValue: "Raw Markdown" })}
      ...
      {markdownMode === "raw" ? (
        ...
        <div
          role="region"
          aria-label={t("fileViewerSheet.markdown.renderedRegionLabel", { defaultValue: "{{title}} rendered Markdown", title: resource.title })}
          tabIndex={0}
  ...
  if (elapsedMs < 400) {
    return (
      <div className="flex-1 space-y-2 p-6" aria-busy="true" aria-live="polite">
        <span className="sr-only">{t("fileViewerSheet.loading.previewSr", { defaultValue: "Loading file preview" })}</span>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 text-muted-foreground">
        ...
        {t("fileViewerSheet.loading.preview", { defaultValue: "Loading file preview..." })}
      </div>
    </div>
  );
}
...
export function FileViewerSheet({
  ...
  useEffect(() => {
    if (resolveQuery.isError) {
      ...
      setAnnouncement(normalized.message || t("fileViewerSheet.errors.unableToLoad", { defaultValue: "Unable to load file." }));
    }
  ...
  useEffect(() => {
    if (contentQuery.isError) {
      ...
      setAnnouncement(normalized.message || t("fileViewerSheet.errors.unableToLoadContent", { defaultValue: "Unable to load file content." }));
    }
  ...
  const copyToClipboard = useCallback(async (value: string, field: "content" | "link", message: string) => {
    try {
      ...
      showCopyFeedback(null, t("fileViewerSheet.copy.failed", { defaultValue: "Copy failed" }));
    } finally {
  ...
  const handleCopyContent = useCallback(() => {
    ...
    void (async () => {
      ...
      if (!content) {
        showCopyFeedback(null, t("fileViewerSheet.copy.contentsUnavailable", { defaultValue: "File contents unavailable" }));
        return;
      }
      const message = content.content.encoding === "base64"
        ? t("fileViewerSheet.copy.copiedFileData", { defaultValue: "Copied file data" })
        : t("fileViewerSheet.copy.copiedContents", { defaultValue: "Copied contents" });
  ...
  const handleCopyLink = useCallback(() => {
    ...
    void copyToClipboard(window.location.href, "link", t("fileViewerSheet.copy.copiedLink", { defaultValue: "Copied link" }));
  }, [copyToClipboard]);
  ...
  const title = state ? basename(state.path) : t("fileViewerSheet.header.browseTitle", { defaultValue: "Browse workspace" });
  const description = state
    ? middleTruncatePath(state.path)
    : t("fileViewerSheet.header.browseDescription", { defaultValue: "Search and preview files from this issue's workspace." });
            ...
            <div className="flex shrink-0 items-center gap-1 self-start">
              ...
              {cameFromBrowse ? (
                <Button
                  ...
                  aria-label={t("fileViewerSheet.header.backToFiles", { defaultValue: "Back to files" })}
                >
                  <ArrowLeft className="h-3.5 w-3.5" />
                  {t("fileViewerSheet.header.backToFiles", { defaultValue: "Back to files" })}
                </Button>
              ) : null}
              {state ? (
                downloadUrl ? (
                  ...
                  >
                    <a
                      ...
                      aria-label={t("fileViewerSheet.header.download", { defaultValue: "Download file" })}
                      title={t("fileViewerSheet.header.download", { defaultValue: "Download file" })}
                    >
              ...
              {state ? (
                <Button
                  ...
                  aria-label={copiedField === "content" ? t("fileViewerSheet.header.copiedContentsAria", { defaultValue: "Copied file contents" }) : t("fileViewerSheet.header.copyContentsAria", { defaultValue: "Copy file contents" })}
                  title={copiedField === "content" ? t("fileViewerSheet.header.copiedContentsTitle", { defaultValue: "Copied contents" }) : t("fileViewerSheet.header.copyContentsTitle", { defaultValue: "Copy file contents" })}
                  className="h-7 w-7"
                >
              ...
              {state ? (
                <Button
                  ...
                  aria-label={copiedField === "link" ? t("fileViewerSheet.header.copiedLinkAria", { defaultValue: "Copied file view link" }) : t("fileViewerSheet.header.copyLinkAria", { defaultValue: "Copy link to this file view" })}
                  title={copiedField === "link" ? t("fileViewerSheet.header.copiedLinkTitle", { defaultValue: "Copied link" }) : t("fileViewerSheet.header.copyLinkTitle", { defaultValue: "Copy link" })}
                  className="h-7 w-7"
                >
                  {copyingField === "link" ? (
                    <Loader2 className="h-4 w-4 animate-spin" />
              ...
              <Button
                ...
                className="h-7 w-7"
                aria-label={t("fileViewerSheet.header.close", { defaultValue: "Close file viewer" })}
                title={t("fileViewerSheet.header.closeTitle", { defaultValue: "Close" })}
              >
                <X className="h-4 w-4" />
              </Button>
            </div>
          ...
          {state ? (
            <div className="flex min-h-0 flex-1 gap-3 bg-muted/30 p-3">
              ...
              <div
                ...
                aria-label={t("fileViewerSheet.resizeFileTree", { defaultValue: "Resize file tree" })}
  ...
  if (resolveQuery.isError) {
    ...
    if (normalized.status === 404) {
      return (
        <FileViewerStateView
          ...
          title={t("fileViewerSheet.notFound.title", { defaultValue: "File not found" })}
          body={t("fileViewerSheet.notFound.body", { defaultValue: "That file was not found in the active workspace." })}
          actions={
            <>
              {onFallbackToProject ? (
                <Button type="button" variant="secondary" size="sm" onClick={onFallbackToProject}>
                  {t("fileViewerSheet.notFound.tryProject", { defaultValue: "Try project workspace" })}
                </Button>
              ) : null}
              <Button type="button" variant="ghost" size="sm" onClick={onRetry}>
                <RefreshCcw aria-hidden="true" className="mr-1 h-3 w-3" /> {t("fileViewerSheet.actions.retry", { defaultValue: "Retry" })}
              </Button>
            </>
          }
        />
    ...
    if (normalized.status === 422) {
      return (
        <FileViewerStateView
          icon={<FolderOpen aria-hidden="true" className="h-6 w-6 text-muted-foreground" />}
          title={t("fileViewerSheet.noWorkspace.title", { defaultValue: "No workspace available" })}
          body={t("fileViewerSheet.noWorkspace.body", { defaultValue: "This issue does not have a workspace that supports preview yet." })}
        />
      );
    }
  ...
  if (resource.kind === "remote_resource") {
    return (
      <FileViewerStateView
        ...
        title={t("fileViewerSheet.remote.title", { defaultValue: "Remote workspace preview coming soon" })}
        body={t("fileViewerSheet.remote.body", { defaultValue: "This workspace is hosted remotely; inline previews are not supported yet." })}
      />
    );
  }
