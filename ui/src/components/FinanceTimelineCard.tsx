...
export function FinanceTimelineCard({
  ...
  const resolvedEmptyMessage =
    emptyMessage ??
    t("financeTimelineCard.emptyMessage", { defaultValue: "No financial events in this period." });
  return (
    <Card>
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0 space-y-2">
                  ...
                  {(row.description || row.externalInvoiceId || row.region || row.pricingTier) && (
                    <div className="space-y-1 text-xs text-muted-foreground">
                      ...
                      {row.externalInvoiceId ? <div>{t("financeTimelineCard.details.invoice", { defaultValue: "invoice {{id}}", id: row.externalInvoiceId })}</div> : null}
                      {row.region ? <div>{t("financeTimelineCard.details.region", { defaultValue: "region {{region}}", region: row.region })}</div> : null}
                      {row.pricingTier ? <div>{t("financeTimelineCard.details.tier", { defaultValue: "tier {{tier}}", tier: row.pricingTier })}</div> : null}
                    </div>
                  )}
                </div>
                <div className="text-right tabular-nums">
                  ...
                  {row.estimated ? <div className="text-(length:--text-micro) uppercase tracking-(--tracking-eyebrow) text-amber-600">{t("financeTimelineCard.estimated", { defaultValue: "estimated" })}</div> : null}
                </div>
              </div>
            </div>
          ))
