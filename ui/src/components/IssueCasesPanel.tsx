...
export function IssueCasesPanel({ issueId }: { issueId: string }) {
  ...
  return (
    <section className="space-y-2">
      <h3 className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">{t("issueCasesPanel.title", { defaultValue: "Cases" })}</h3>
      <div className="space-y-1">
        {links.map((link) => (
          ...
          >
            ...
            <Badge variant="secondary" className="shrink-0">{t(`issueCasesPanel.roles.${link.role}`, { defaultValue: ROLE_LABEL[link.role] })}</Badge>
