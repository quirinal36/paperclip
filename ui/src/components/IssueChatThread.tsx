...
function IssueChatLiveRunStatusLine({
  ...
  const primary =
    currentToolName
      ? t("issueChatThread.liveStatus.usingTool", { defaultValue: "Using {{tool}}", tool: currentToolName })
      : lastAssistantSnippet
        ? lastAssistantSnippet
        : currentStatusMessage;
  const activityText = lastActivityElapsed
    ? lastActivityAgeMs !== null && lastActivityAgeMs >= 15_000
      ? t("issueChatThread.liveStatus.noOutput", { defaultValue: "no output for {{elapsed}} - still running", elapsed: lastActivityElapsed })
      : t("issueChatThread.liveStatus.ago", { defaultValue: "{{elapsed}} ago", elapsed: lastActivityElapsed })
    : "";
...
function IssueAssigneePausedNotice({ agent }: { agent: Agent | null }) {
  ...
  const pauseDetail =
    agent.pauseReason === "budget"
      ? t("issueChatThread.agentPaused.budget", { defaultValue: "It was paused by a budget hard stop." })
      : agent.pauseReason === "system"
        ? t("issueChatThread.agentPaused.system", { defaultValue: "It was paused by the system." })
        : t("issueChatThread.agentPaused.manual", { defaultValue: "It was paused manually." });
    ...
    <div className="mb-3 rounded-md border border-orange-300/70 bg-orange-50/90 px-3 py-2.5 text-sm text-orange-950 shadow-sm dark:border-orange-500/40 dark:bg-orange-500/10 dark:text-orange-100">
      <div className="flex items-start gap-2">
        ...
        <p className="min-w-0 leading-5">
          <span className="font-medium">{agent.name}</span>{t("issueChatThread.agentPaused.body", { defaultValue: " is paused. New runs will not start until the agent is resumed. " })}{pauseDetail}
        </p>
      </div>
    </div>
  );
...
function fallbackAuthorLabel(message: ThreadMessage) {
  ...
  if (message.role === "assistant") return t("issueChatThread.author.agent", { defaultValue: "Agent" });
  if (message.role === "user") return t("issueChatThread.author.you", { defaultValue: "You" });
  return t("issueChatThread.author.system", { defaultValue: "System" });
}
...
function fallbackTextParts(message: ThreadMessage) {
  ...
  for (const part of message.content) {
    ...
    if (part.type === "tool-call") {
      const lines = [t("issueChatThread.fallback.tool", { defaultValue: "Tool: {{name}}", name: part.toolName })];
      if (part.argsText?.trim()) lines.push(t("issueChatThread.fallback.args", { defaultValue: "Args:\n{{args}}", args: part.argsText }));
      if (typeof part.result === "string" && part.result.trim()) lines.push(t("issueChatThread.fallback.result", { defaultValue: "Result:\n{{result}}", result: part.result }));
        ...
        <div className="flex items-start gap-2">
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs opacity-80">
              {t("issueChatThread.fallback.errorBody", { defaultValue: "Showing a safe fallback transcript instead of crashing the tasks page." })}
            </p>
          </div>
        </div>
      </div>
...
function humanizeValue(value: string | null) {
  if (!value) return t("issueChatThread.common.none", { defaultValue: "None" });
...
function formatInteractionActorLabel(args: {
  ...
  if (userId) {
    return userLabelMap?.get(userId)
      ...
      ?? t("issueChatThread.author.board", { defaultValue: "Board" });
  }
  return t("issueChatThread.author.system", { defaultValue: "System" });
}
...
export function resolveIssueChatHumanAuthor(args: {
  ...
  const resolvedAuthorName = profile?.label?.trim()
    ...
    || (authorUserId === "local-board"
      ? t("issueChatThread.author.board", { defaultValue: "Board" })
      : (isCurrentUser
        ? t("issueChatThread.author.you", { defaultValue: "You" })
        : t("issueChatThread.author.user", { defaultValue: "User" })));
...
function toolCountSummary(toolParts: ToolCallMessagePart[]): string | null {
  ...
  if (commands > 0) {
    parts.push(
      commands === 1
        ? t("issueChatThread.toolCount.command", { defaultValue: "ran {{count}} command", count: commands })
        : t("issueChatThread.toolCount.commands", { defaultValue: "ran {{count}} commands", count: commands }),
    );
  }
  if (other > 0) {
    parts.push(
      other === 1
        ? t("issueChatThread.toolCount.tool", { defaultValue: "called {{count}} tool", count: other })
        : t("issueChatThread.toolCount.tools", { defaultValue: "called {{count}} tools", count: other }),
    );
  }
  return parts.join(", ");
}
  ...
  return (
    <div className="group/pre relative">
      ...
      <button
        ...
        title={t("issueChatThread.actions.copy", { defaultValue: "Copy" })}
        aria-label={t("issueChatThread.actions.copy", { defaultValue: "Copy" })}
        onClick={() => {
          void copyTextToClipboard(children).then(() => {
            ...
            toastActions?.pushToast({
              title: t("issueChatThread.toast.copyFailed", { defaultValue: "Copy failed" }),
              body: error instanceof Error ? error.message : t("issueChatThread.toast.unableToCopyText", { defaultValue: "Unable to copy text" }),
              tone: "error",
            });
          });
        }}
        ...
        {open ? (
          <div className="mt-1 space-y-2 pb-1">
            {nonIntentDetails.length > 0 ? (
              <div>
                <div className="mb-1 text-(length:--text-nano) font-semibold uppercase tracking-(--tracking-eyebrow) text-muted-foreground/60">
                  {t("issueChatThread.tool.input", { defaultValue: "Input" })}
                </div>
            ...
            {result !== undefined ? (
              <div>
                <div className="mb-1 text-(length:--text-nano) font-semibold uppercase tracking-(--tracking-eyebrow) text-muted-foreground/60">
                  {t("issueChatThread.tool.result", { defaultValue: "Result" })}
                </div>
...
function IssueChatUserMessage({
  ...
  const queueBadgeLabel = queueReason === "hold"
    ? t("issueChatThread.queue.deferredWake", { defaultValue: "\u23f8 Deferred wake" })
    : t("issueChatThread.queue.queued", { defaultValue: "Queued" });
    ...
    <div className={cn("flex min-w-0 max-w-(--pct-85) flex-col", isCurrentUser && "items-end")}>
      <div className={cn("mb-1 flex items-center gap-2 px-1", isCurrentUser ? "justify-end" : "justify-start")}>
        ...
        {followUpRequested ? (
          <Badge variant="outline" className="text-(length:--text-nano) uppercase tracking-(--tracking-eyebrow)">
            {t("issueChatThread.badge.followUp", { defaultValue: "Follow-up" })}
          </Badge>
        ) : null}
      </div>
      <div
        ...
        {queued ? (
          <div className="mb-1.5 flex items-center gap-2">
            ...
            {queueTargetRunId && onInterruptQueued ? (
              ...
              >
                {isInterruptingQueuedRun
                  ? t("issueChatThread.queue.interrupting", { defaultValue: "Interrupting..." })
                  : t("issueChatThread.queue.interrupt", { defaultValue: "Interrupt" })}
              </Button>
            ) : null}
            {onCancelQueued ? (
              <Button
              ...
              >
                {t("issueChatThread.actions.cancel", { defaultValue: "Cancel" })}
              </Button>
            ) : null}
          </div>
        ) : null}
        {deleted ? (
          <div className="text-sm italic text-muted-foreground">{t("issueChatThread.comment.deleted", { defaultValue: "Comment deleted" })}</div>
        ) : (
      ...
      {pending ? (
        <div className={cn("mt-1 flex px-1 text-(length:--text-micro) text-muted-foreground", isCurrentUser ? "justify-end" : "justify-start")}>
          {t("issueChatThread.comment.sending", { defaultValue: "Sending..." })}
        </div>
      ) : (
        <div
          className={cn(
        ...
        >
          ...
          {!deleted ? (
            <button
              ...
              title={t("issueChatThread.actions.copyMessage", { defaultValue: "Copy message" })}
              aria-label={t("issueChatThread.actions.copyMessage", { defaultValue: "Copy message" })}
              onClick={() => {
                ...
                void copyTextToClipboard(text).then(() => {
                  ...
                  toastActions?.pushToast({
                    title: t("issueChatThread.toast.copyFailed", { defaultValue: "Copy failed" }),
                    body: error instanceof Error ? error.message : t("issueChatThread.toast.unableToCopyMessage", { defaultValue: "Unable to copy message" }),
                    tone: "error",
                  });
                });
              }}
          ...
          {canDeleteComment ? (
            <button
              ...
              title={t("issueChatThread.actions.deleteComment", { defaultValue: "Delete comment" })}
              aria-label={t("issueChatThread.actions.deleteComment", { defaultValue: "Delete comment" })}
  ...
  return (
    <>
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("issueChatThread.deleteDialog.title", { defaultValue: "Delete comment?" })}</DialogTitle>
            <DialogDescription>
              {t("issueChatThread.deleteDialog.description", { defaultValue: "This will replace the comment with a deleted-comment marker." })}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={confirmDeleteComment}>
              {t("issueChatThread.actions.deleteComment", { defaultValue: "Delete comment" })}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
...
function IssueChatAssistantMessage({
  ...
  const {
    ...
    agentMap,
    onStopRun,
    stopRunLabel = t("issueChatThread.run.stop", { defaultValue: "Stop run" }),
    stoppingRunLabel = t("issueChatThread.run.stopping", { defaultValue: "Stopping..." }),
    stopRunVariant = "stop",
  ...
  const authorName = typeof custom.authorName === "string"
    ...
    : typeof custom.runAgentName === "string"
      ? custom.runAgentName
      : t("issueChatThread.author.agent", { defaultValue: "Agent" });
  const authorAgentId = typeof custom.authorAgentId === "string" ? custom.authorAgentId : null;
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            ...
            title={t("issueChatThread.actions.moreActions", { defaultValue: "More actions" })}
            aria-label={t("issueChatThread.actions.moreActions", { defaultValue: "More actions" })}
          >
        ...
        <DropdownMenuContent align="end">
          ...
          >
            ...
            {t("issueChatThread.actions.copyMessage", { defaultValue: "Copy message" })}
          </DropdownMenuItem>
          ...
          {runHref ? (
            <DropdownMenuItem asChild>
              <Link to={runHref} target="_blank" rel="noreferrer noopener">
                ...
                {t("issueChatThread.actions.viewRun", { defaultValue: "View run" })}
              </Link>
            </DropdownMenuItem>
          ) : null}
        </DropdownMenuContent>
        ...
        <div className="min-w-0 flex-1">
            ...
            <div className="mb-1.5 flex items-center gap-2">
              ...
              {isRunning ? (
                ...
                >
                  ...
                  {t("issueChatThread.badge.running", { defaultValue: "Running" })}
                </Badge>
              ) : null}
            </div>
          )}
          ...
          {deleted ? (
            <div className="rounded-sm bg-muted/40 px-3 py-2 text-sm italic text-muted-foreground">
              {t("issueChatThread.comment.deleted", { defaultValue: "Comment deleted" })}
            </div>
          ) : !folded ? (
            <>
    ...
    <>
      ...
      <Popover open={reasonOpen} onOpenChange={setReasonOpen}>
        ...
        <PopoverContent side="top" align="start" className="w-80 p-3">
          <div className="mb-2 text-sm font-medium">{t("issueChatThread.feedback.reasonPrompt", { defaultValue: "What could have been better?" })}</div>
          <Textarea
            value={downvoteReason}
            onChange={(event) => setDownvoteReason(event.target.value)}
            placeholder={t("issueChatThread.feedback.reasonPlaceholder", { defaultValue: "Add a short note" })}
          ...
          <div className="mt-2 flex items-center justify-end gap-2">
            ...
            >
              {isSaving
                ? t("issueChatThread.actions.saving", { defaultValue: "Saving..." })
                : t("issueChatThread.feedback.saveNote", { defaultValue: "Save note" })}
            </Button>
          </div>
        </PopoverContent>
      </Popover>
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {t("issueChatThread.sharingDialog.description", { defaultValue: "Choose whether voted AI outputs can be shared with Paperclip Labs. This answer becomes the default for future thumbs up and thumbs down votes." })}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 text-sm text-muted-foreground">
            <p>{t("issueChatThread.sharingDialog.savedLocally", { defaultValue: "This vote is always saved locally." })}</p>
            <p>
              {t("issueChatThread.sharingDialog.chooseLead", { defaultValue: "Choose " })}
              <span className="font-medium text-foreground">{t("issueChatThread.sharingDialog.alwaysAllow", { defaultValue: "Always allow" })}</span>
              {t("issueChatThread.sharingDialog.alwaysAllowMid", { defaultValue: " to share this vote and future voted AI outputs. Choose " })}
              <span className="font-medium text-foreground">{t("issueChatThread.sharingDialog.dontAllow", { defaultValue: "Don't allow" })}</span>
              {t("issueChatThread.sharingDialog.dontAllowTail", { defaultValue: " to keep this vote and future votes local." })}
            </p>
          ...
          <DialogFooter>
            ...
            >
              {isSaving
                ? t("issueChatThread.actions.saving", { defaultValue: "Saving..." })
                : t("issueChatThread.sharingDialog.dontAllow", { defaultValue: "Don't allow" })}
            </Button>
            <Button
              type="button"
              disabled={!pendingSharingDialog || isSaving}
            ...
            >
              {isSaving
                ? t("issueChatThread.actions.saving", { defaultValue: "Saving..." })
                : t("issueChatThread.sharingDialog.alwaysAllow", { defaultValue: "Always allow" })}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
  ...
  const rowContent = (
    <div className="min-w-0 flex-1">
      <div className={cn("flex flex-wrap items-center gap-x-1.5 gap-y-1 text-xs", isCurrentUser && "justify-end")}>
        ...
        <span className="text-muted-foreground">{t("issueChatThread.activity.updatedThisTask", { defaultValue: "updated this task" })}</span>
        <a
          href={anchorId ? `#${anchorId}` : undefined}
        ...
        >
          ...
          {expanded
            ? t("issueChatThread.expiredConfirmation.hide", { defaultValue: "Hide confirmation" })
            : t("issueChatThread.expiredConfirmation.label", { defaultValue: "Expired confirmation" })}
        </button>
      </div>
      {expanded ? (
...
function SystemNoticeCommentRow({
  ...
  const handleCopy = () => {
    void copyTextToClipboard(bodyText).then(() => {
      ...
      toastActions?.pushToast({
        title: t("issueChatThread.toast.copyFailed", { defaultValue: "Copy failed" }),
        body: error instanceof Error ? error.message : t("issueChatThread.toast.unableToCopyNotice", { defaultValue: "Unable to copy system notice" }),
        tone: "error",
      });
    });
  };
  ...
  const handleCopyLink = () => {
    ...
    void copyTextToClipboard(url).then(() => {
      ...
      toastActions?.pushToast({
        title: t("issueChatThread.toast.copyFailed", { defaultValue: "Copy failed" }),
        body: error instanceof Error ? error.message : t("issueChatThread.toast.unableToCopyNoticeLink", { defaultValue: "Unable to copy system notice link" }),
        tone: "error",
      });
    });
  };
        ...
        <div className="mt-1 flex items-center justify-end gap-1.5 px-1 opacity-0 transition-opacity group-hover:opacity-100">
          ...
          {anchorId ? (
            <button
              type="button"
              className="inline-flex h-6 w-6 items-center justify-center text-muted-foreground transition-colors hover:text-foreground"
              title={t("issueChatThread.actions.copyLink", { defaultValue: "Copy link" })}
              aria-label={t("issueChatThread.actions.copyLinkToNotice", { defaultValue: "Copy link to system notice" })}
          ...
          <button
            type="button"
            className="inline-flex h-6 w-6 items-center justify-center text-muted-foreground transition-colors hover:text-foreground"
            title={t("issueChatThread.actions.copyNoticeText", { defaultValue: "Copy notice text" })}
            aria-label={t("issueChatThread.actions.copyNotice", { defaultValue: "Copy system notice" })}
            onClick={handleCopy}
          >
            {copied ? <Check className="h-3.5 w-3.5" /> : <Copy className="h-3.5 w-3.5" />}
          </button>
    ...
    return (
      <IssueChatMetadataRow anchorId={anchorId} icon={rowIcon}>
        <div className="flex flex-wrap items-baseline gap-x-1.5 gap-y-0.5 text-xs">
          ...
          <span className="text-muted-foreground">
            {custom.followUpRequested === true
              ? t("issueChatThread.activity.requestedFollowUp", { defaultValue: "requested follow-up" })
              : t("issueChatThread.activity.updatedThisTask", { defaultValue: "updated this task" })}
          </span>
          <a
            href={anchorId ? `#${anchorId}` : undefined}
        ...
        {statusChange ? (
          <div className="flex flex-wrap items-center gap-1.5 text-xs">
            <span className="text-(length:--text-nano) font-medium uppercase tracking-wider text-muted-foreground/70">
              {t("issueChatThread.field.status", { defaultValue: "Status" })}
            </span>
        ...
        {assigneeChange ? (
          <div className="space-y-1">
            <div className={cn("flex flex-wrap items-center gap-1.5 text-xs", isCurrentUser && "justify-end")}>
              <span className="text-(length:--text-nano) font-medium uppercase tracking-wider text-muted-foreground/70">
                {t("issueChatThread.field.assignee", { defaultValue: "Assignee" })}
              </span>
        ...
        {workspaceChange ? (
          <div className="flex flex-wrap items-center gap-1.5 text-xs">
            <span className="text-(length:--text-nano) font-medium uppercase tracking-wider text-muted-foreground/70">
              {t("issueChatThread.field.workspace", { defaultValue: "Workspace" })}
            </span>
            <span className="text-muted-foreground">
    ...
    return (
      <IssueChatMetadataRow anchorId={anchorId} icon={rowIcon}>
        <div className="flex flex-wrap items-center gap-x-1.5 gap-y-0.5 text-xs">
          ...
          <span className="text-muted-foreground">{t("issueChatThread.activity.run", { defaultValue: "run" })}</span>
          <Link
...
function IssueChatDeletedComment({
  ...
  const custom = issueChatMessageCustom(message);
  const anchorId = typeof custom.anchorId === "string" ? custom.anchorId : undefined;
  const authorName = typeof custom.authorName === "string" ? custom.authorName : t("issueChatThread.author.comment", { defaultValue: "Comment" });
  ...
  return (
    <div id={anchorId} className="flex items-start gap-2.5 py-1.5">
      ...
      <div className="min-w-0 rounded-md border border-dashed border-border bg-muted/20 px-3 py-2 text-sm text-muted-foreground">
        ...
        <span>{t("issueChatThread.comment.deletedThisComment", { defaultValue: " deleted this comment" })}</span>
  ...
  async function attachFile(file: File) {
    ...
    try {
      if (onImageUpload && file.type.startsWith("image/")) {
        ...
        setComposerAttachments((prev) => prev.map((item) =>
          item.id === attachmentId
            ? { ...item, status: "error", error: t("issueChatThread.composer.fileTypeUnsupported", { defaultValue: "This file type cannot be attached here" }) }
            : item,
        ));
      }
    } catch (err) {
  ...
  return (
    ...
    >
          ...
          <div className="flex max-w-md items-center gap-3 rounded-md bg-background/80 px-3 py-2 text-left shadow-sm ring-1 ring-border/60">
            ...
            <div className="min-w-0">
              ...
              <div className="mt-0.5 text-xs leading-5 text-muted-foreground">
                {t("issueChatThread.composer.dropHint", { defaultValue: "Images insert into the reply. Other files are added to this task." })}
              </div>
            </div>
          </div>
        </div>
          ...
          {composerAttachments.map((attachment) => {
            ...
            const statusLabel =
              attachment.status === "uploading"
                ? t("issueChatThread.composer.uploadingToTask", { defaultValue: "Uploading to task" })
                : attachment.status === "error"
                  ? attachment.error ?? t("issueChatThread.composer.uploadFailed", { defaultValue: "Upload failed" })
                  : attachment.inline
                    ? t("issueChatThread.composer.insertedInline", { defaultValue: "Inserted inline" })
                    : t("issueChatThread.composer.attachedToTask", { defaultValue: "Attached to task" });
            return (
              <div
                key={attachment.id}
                className={cn(
      ...
      <div className="flex flex-wrap items-center justify-end gap-3">
            ...
            <Popover open={workModeMenuOpen} onOpenChange={setWorkModeMenuOpen}>
              ...
              >
                ...
                <div className="mt-1 border-t px-2 py-1.5 text-(length:--text-nano) text-muted-foreground">
                  {t("issueChatThread.composer.cyclesModes", { defaultValue: "Cmd/Ctrl+. cycles modes" })}
                </div>
              </PopoverContent>
            </Popover>
          ) : null}
        ...
        {enableReassign && reassignOptions.length > 0 ? (
          <InlineEntitySelector
            ...
            placeholder={t("issueChatThread.composer.responsible", { defaultValue: "Responsible" })}
            noneLabel={t("issueChatThread.composer.noResponsible", { defaultValue: "No responsible" })}
            searchPlaceholder={t("issueChatThread.composer.searchResponsible", { defaultValue: "Search responsible..." })}
            emptyMessage={t("issueChatThread.composer.noResponsibleFound", { defaultValue: "No responsible found." })}
            onChange={setReassignTarget}
            className="h-8 text-xs"
            renderTriggerValue={(option) => {
              if (!option) return <span className="text-muted-foreground">{t("issueChatThread.composer.responsible", { defaultValue: "Responsible" })}</span>;
        ...
        <Button size="sm" disabled={!canSubmit} onClick={() => void handleSubmit()}>
          {submitting
            ? t("issueChatThread.composer.posting", { defaultValue: "Posting..." })
            : t("issueChatThread.composer.send", { defaultValue: "Send" })}
        </Button>
      </div>
      ...
      <AlertDialog open={noAssigneeDialogOpen} onOpenChange={setNoAssigneeDialogOpen}>
        ...
        >
          <AlertDialogHeader>
            <AlertDialogTitle>{t("issueChatThread.noAssignee.title", { defaultValue: "No responsible selected" })}</AlertDialogTitle>
            <AlertDialogDescription>
              {t("issueChatThread.noAssignee.description", { defaultValue: "This comment will be posted without an assignee, so no agent will be woken to act on it. Go back to pick a responsible, or send anyway." })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel
            ...
            >
              {t("issueChatThread.noAssignee.goBack", { defaultValue: "Go back" })}
            </AlertDialogCancel>
            <AlertDialogAction
            ...
            >
              {t("issueChatThread.noAssignee.sendAnyway", { defaultValue: "Send anyway" })}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
...
export function IssueChatThread({
  ...
  const resolvedEmptyMessage = emptyMessage
    ?? (variant === "embedded"
      ? t("issueChatThread.empty.embedded", { defaultValue: "No run output yet." })
      : t("issueChatThread.empty.full", { defaultValue: "This task conversation is empty. Start with a message below." }));
              ...
              {showComposer ? (
                <div data-testid="issue-chat-thread-notices" className="space-y-2">
                  ...
                  {legacyRecoverySourceIssue ? (
                    <SystemNotice
                      tone="info"
                      label={t("issueChatThread.legacyRecovery.label", { defaultValue: "Legacy recovery task" })}
                      body={
                        <span>
                          {t("issueChatThread.legacyRecovery.body", { defaultValue: "Legacy recovery task. Newer recovery actions live on the source task" })}
