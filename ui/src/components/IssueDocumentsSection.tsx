...
function getRevisionActor(
  ...
  if (revision.createdByUserId) {
    ...
    return {
      kind: "user",
      name: profile?.label ?? (revision.createdByUserId === "local-board" ? t("issueDocumentsSection.actor.board", { defaultValue: "Board" }) : revision.createdByUserId.slice(0, 8)),
      imageUrl: profile?.image ?? null,
    };
  }
  return { kind: "system", name: t("issueDocumentsSection.actor.system", { defaultValue: "System" }) };
}
...
export function IssueDocumentsSection({
  ...
  const deleteDocument = useMutation({
    mutationFn: (key: string) => documentSubject.deleteDocument
      ...
      : Promise.reject(new Error(t("issueDocumentsSection.errors.deleteUnavailable", { defaultValue: "Document deletion is not available" }))),
    onSuccess: () => {
      setError(null);
  ...
  const restoreDocumentRevision = useMutation({
    mutationFn: ({ key, revisionId }: { key: string; revisionId: string }) =>
      documentSubject.restoreDocumentRevision
        ...
        : Promise.reject(new Error(t("issueDocumentsSection.errors.restoreUnavailable", { defaultValue: "Document revision restore is not available" }))),
  ...
  const setDocumentLock = useMutation({
    mutationFn: ({ key, locked }: { key: string; locked: boolean }) =>
      documentSubject.setDocumentLock
        ...
        : Promise.reject(new Error(t("issueDocumentsSection.errors.lockUnavailable", { defaultValue: "Document locking is not available" }))),
    onSuccess: (document) => {
      syncDocumentCaches(document);
    ...
    onError: (err) => {
      setError(err instanceof Error ? err.message : t("issueDocumentsSection.errors.lockUpdateFailed", { defaultValue: "Failed to update document lock" }));
    },
  });
  ...
  const newDocumentKeyError =
    draft?.isNew && draft.key.trim().length > 0 && !DOCUMENT_KEY_PATTERN.test(draft.key.trim())
      ? t("issueDocumentsSection.errors.keyPattern", { defaultValue: "Use lowercase letters, numbers, -, or _, and start with a letter or number." })
      : null;
  ...
  const commitDraft = useCallback(async (
    ...
    if (!normalizedKey || !normalizedBody) {
      if (currentDraft.isNew) {
        setError(t("issueDocumentsSection.errors.keyAndBodyRequired", { defaultValue: "Document key and body are required" }));
      } else if (!normalizedBody) {
        setError(t("issueDocumentsSection.errors.bodyEmpty", { defaultValue: "Document body cannot be empty" }));
      }
      if (options?.trackAutosave) {
        resetAutosaveState();
      }
    ...
    if (!DOCUMENT_KEY_PATTERN.test(normalizedKey)) {
      setError(t("issueDocumentsSection.errors.keyPatternFull", { defaultValue: "Document key must start with a letter or number and use only lowercase letters, numbers, -, or _." }));
      if (options?.trackAutosave) {
        resetAutosaveState();
      }
      return false;
    ...
    try {
      ...
      if (isLockedDocumentError(err)) {
        setError(t("issueDocumentsSection.errors.documentLocked", { defaultValue: "Document is locked. Unlock it before editing." }));
        resetAutosaveState();
        invalidateIssueDocuments();
        return false;
      }
      if (isDocumentConflictError(err)) {
        try {
          ...
          setError(t("issueDocumentsSection.errors.remoteLoadFailed", { defaultValue: "Document changed remotely and the latest version could not be loaded" }));
          return false;
        }
      }
  ...
  const previewRevision = useCallback((doc: IssueDocument, revisionId: string) => {
    ...
    if (documentConflict?.key === doc.key || documentHasUnsavedChanges(doc, draft)) {
      setError(t("issueDocumentsSection.errors.saveOrCancelRevision", { defaultValue: "Save or cancel your local changes before viewing an older revision." }));
      return;
    }
    resetAutosaveState();
  ...
  const toggleDocumentLock = useCallback((doc: IssueDocument, locked: boolean) => {
    ...
    if (locked && (documentConflict?.key === doc.key || documentHasUnsavedChanges(doc, draft))) {
      setError(t("issueDocumentsSection.errors.saveOrCancelLock", { defaultValue: "Save or cancel local changes before changing the document lock." }));
      return;
    }
      ...
      {isEmpty && !draft?.isNew ? (
        <div className="flex flex-wrap items-center justify-end gap-2 min-w-0">
          ...
          <Button variant="outline" size="sm" onClick={beginNewDocument} className="shrink-0">
            ...
            <span className="sm:hidden">{t("issueDocumentsSection.actions.newShort", { defaultValue: "New" })}</span>
          </Button>
        </div>
      ) : (
      ...
      {draft?.isNew && (
        ...
        >
          <Input
            ...
            placeholder={t("issueDocumentsSection.fields.documentKey", { defaultValue: "Document key" })}
          />
          {newDocumentKeyError && (
          ...
          {!isPlanKey(draft.key) && (
            <Input
              ...
              placeholder={t("issueDocumentsSection.fields.optionalTitle", { defaultValue: "Optional title" })}
            />
          )}
          <MarkdownEditor
            value={draft.body}
            ...
            placeholder={t("issueDocumentsSection.fields.markdownBody", { defaultValue: "Markdown body" })}
            bordered={false}
          ...
          <div className="flex items-center justify-end gap-2">
            ...
            >
              {upsertDocument.isPending ? t("issueDocumentsSection.actions.saving", { defaultValue: "Saving..." }) : t("issueDocumentsSection.actions.createDocument", { defaultValue: "Create document" })}
            </Button>
          </div>
        </div>
      )}
                ...
                actionsSlot={
                  <>
                    ...
                    <Button
                      ...
                      title={isLocked ? t("issueDocumentsSection.tooltips.unlockDocument", { defaultValue: "Unlock document" }) : t("issueDocumentsSection.tooltips.lockDocument", { defaultValue: "Lock document" })}
                      aria-label={isLocked ? t("issueDocumentsSection.tooltips.unlockDocumentAria", { defaultValue: "Unlock {{key}} document", key: doc.key }) : t("issueDocumentsSection.tooltips.lockDocumentAria", { defaultValue: "Lock {{key}} document", key: doc.key })}
                    ...
                    >
                      ...
                      <span title={t("issueDocumentsSection.tooltips.lockedDocument", { defaultValue: "Locked document" })} aria-label={t("issueDocumentsSection.tooltips.lockedDocument", { defaultValue: "Locked document" })} className="inline-flex h-6 w-6 items-center justify-center text-amber-700 dark:text-amber-300">
                    ...
                    <Button
                      ...
                      title={copiedDocumentKey === doc.key ? t("issueDocumentsSection.tooltips.copied", { defaultValue: "Copied" }) : t("issueDocumentsSection.tooltips.copyDocument", { defaultValue: "Copy document" })}
                    ...
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button
                          ...
                          title={t("issueDocumentsSection.tooltips.documentActions", { defaultValue: "Document actions" })}
                        >
                ...
                >
                      ...
                      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        <div className="space-y-1">
                          <p className="text-sm font-medium text-amber-800 dark:text-amber-200">
                            {t("issueDocumentsSection.revision.viewing", { defaultValue: "Viewing revision {{revisionNumber}}", revisionNumber: selectedHistoricalRevision.revisionNumber })}
                          </p>
                          <p className="text-xs text-muted-foreground">
                            {t("issueDocumentsSection.revision.historicalNotice", { defaultValue: "This is a historical preview. Restoring it creates a new latest revision and keeps history append-only." })}
                          </p>
                        </div>
                          ...
                          {!isLocked ? (
                            ...
                            >
                              {restoreDocumentRevision.isPending && restoreDocumentRevision.variables?.key === doc.key
                                ? t("issueDocumentsSection.actions.restoring", { defaultValue: "Restoring..." })
                  ...
                  {activeConflict && !isHistoricalPreview && (
                    <div className="rounded-md border border-amber-500/30 bg-amber-500/5 px-3 py-3">
                      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        <div className="space-y-1">
                          <p className="text-sm font-medium text-amber-800 dark:text-amber-200">{t("issueDocumentsSection.conflict.outOfDate", { defaultValue: "Out of date" })}</p>
                          <p className="text-xs text-muted-foreground">
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          >
                            {activeConflict.showRemote ? t("issueDocumentsSection.actions.hideRemote", { defaultValue: "Hide remote" }) : t("issueDocumentsSection.actions.reviewRemote", { defaultValue: "Review remote" })}
                          </Button>
                          <Button
                            variant="outline"
                            size="sm"
                          ...
                          <Button
                            ...
                            disabled={upsertDocument.isPending}
                          >
                            {upsertDocument.isPending ? t("issueDocumentsSection.actions.saving", { defaultValue: "Saving..." }) : t("issueDocumentsSection.actions.overwriteRemote", { defaultValue: "Overwrite remote" })}
                          </Button>
                        </div>
                      </div>
                      {activeConflict.showRemote && (
                        <div className="mt-3 rounded-md border border-border/70 bg-background/60 p-3">
                          <div className="mb-2 flex items-center gap-2 text-(length:--text-micro) text-muted-foreground">
                            <span>{t("issueDocumentsSection.conflict.remoteRevision", { defaultValue: "Remote revision {{revisionNumber}}", revisionNumber: activeConflict.serverDocument.latestRevisionNumber })}</span>
                            <span>•</span>
                            <span>{t("issueDocumentsSection.conflict.updatedAt", { defaultValue: "updated {{time}}", time: relativeTime(activeConflict.serverDocument.updatedAt) })}</span>
                          </div>
              ...
              {confirmDeleteKey === doc.key && (
                <div className="mt-3 flex items-center justify-between gap-3 rounded-md border border-destructive/20 bg-destructive/5 px-4 py-3">
                  <p className="text-sm text-destructive font-medium">
                    {t("issueDocumentsSection.deleteConfirm.message", { defaultValue: "Delete this document? This cannot be undone." })}
                  </p>
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <Button
                      ...
                      disabled={deleteDocument.isPending}
                    >
                      {deleteDocument.isPending ? t("issueDocumentsSection.actions.deleting", { defaultValue: "Deleting..." }) : t("issueDocumentsSection.actions.delete", { defaultValue: "Delete" })}
                    </Button>
                  </div>
                </div>
              )}
