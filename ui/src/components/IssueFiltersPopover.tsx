  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <Button variant={buttonVariant} size={iconOnly ? "icon" : "sm"} className={`text-xs ${iconOnly ? "relative h-8 w-8 shrink-0" : ""} ${activeFilterCount > 0 ? "text-blue-600 dark:text-blue-400" : ""}`} title={iconOnly ? (activeFilterCount > 0 ? t("issueFiltersPopover.button.filtersCount", { defaultValue: "Filters: {{filterCount}}", filterCount: activeFilterCount }) : t("issueFiltersPopover.button.filter", { defaultValue: "Filter" })) : undefined}>
          <Filter className={iconOnly ? "h-3.5 w-3.5" : "h-3.5 w-3.5 sm:h-3 sm:w-3 sm:mr-1"} />
          {!iconOnly && <span className="hidden sm:inline">{activeFilterCount > 0 ? t("issueFiltersPopover.button.filtersCount", { defaultValue: "Filters: {{filterCount}}", filterCount: activeFilterCount }) : t("issueFiltersPopover.button.filter", { defaultValue: "Filter" })}</span>}
      ...
      >
        <div className="space-y-3 p-3">
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            ...
            <div className="min-w-0 space-y-3">
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground">{t("issueFiltersPopover.sections.responsible", { defaultValue: "Responsible" })}</span>
                <div className="max-h-32 space-y-0.5 overflow-y-auto">
                  <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                    ...
                    />
                    <span className="text-sm">{t("issueFiltersPopover.responsible.none", { defaultValue: "No responsible" })}</span>
                  </label>
                  {currentUserId ? (
                    <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                      <Checkbox
                      ...
                      <span className="text-sm">{t("issueFiltersPopover.responsible.me", { defaultValue: "Me" })}</span>
                    </label>
                  ) : null}
                  {(agents ?? []).map((agent) => (
                ...
                <div className="space-y-1">
                      ...
                      {selectedCreatorOptions.map((creator) => (
                        <Badge key={creator.id} variant="secondary" className="gap-1 pr-1">
                          ...
                          <button
                            ...
                            aria-label={t("issueFiltersPopover.creator.removeAria", { defaultValue: "Remove creator {{label}}", label: creator.label })}
                          >
                            <X className="h-3 w-3" />
                          </button>
                        </Badge>
                  ...
                  <div className="max-h-32 space-y-0.5 overflow-y-auto">
                    {visibleCreatorOptions.length > 0 ? visibleCreatorOptions.map((creator) => {
                      ...
                      <div className="px-2 py-1 text-xs text-muted-foreground">{t("issueFiltersPopover.creator.noMatch", { defaultValue: "No creators match." })}</div>
                    )}
                  </div>
                </div>
              ) : null}
            ...
            <div className="min-w-0 space-y-3">
              ...
              {enableExternalObjectFilters ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("issueFiltersPopover.sections.externalObjectStatus", { defaultValue: "External object status" })}</span>
                  <div className="space-y-0.5">
              ...
              <div className="space-y-1">
                ...
                <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                  ...
                  />
                  <span className="text-sm">{t("issueFiltersPopover.visibility.liveOnly", { defaultValue: "Live runs only" })}</span>
                </label>
