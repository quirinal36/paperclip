  ...
  if (status === "completed") {
    return (
      <span className="inline-flex items-center gap-1 rounded-sm border border-emerald-500/50 bg-emerald-500/10 px-2 py-0.5 text-(length:--text-micro) font-medium text-emerald-900 dark:text-emerald-100">
        ...
        {t("issuePlanDecompositionsSection.status.completed", { defaultValue: "Completed" })}
      </span>
    );
  }
  return (
    <span className="inline-flex items-center gap-1 rounded-sm border border-amber-500/50 bg-amber-500/10 px-2 py-0.5 text-(length:--text-micro) font-medium text-amber-900 dark:text-amber-100">
      ...
      {t("issuePlanDecompositionsSection.status.inFlight", { defaultValue: "In flight" })}
    </span>
  );
}
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-2">
        <h3 className="text-sm font-medium text-muted-foreground">
          {t("issuePlanDecompositionsSection.title", { defaultValue: "Plan decomposition" })}
        </h3>
        <span className="text-(length:--text-micro) text-muted-foreground/80">
          {items.length === 1
            ? t("issuePlanDecompositionsSection.acceptedRevisions.one", {
                defaultValue: "1 accepted plan revision",
              })
            : t("issuePlanDecompositionsSection.acceptedRevisions.other", {
        ...
        {items.map((record) => {
          ...
          const ownerName = record.ownerAgentId
            ? agentMap?.get(record.ownerAgentId)?.name ??
              t("issuePlanDecompositionsSection.ownerFallback", { defaultValue: "agent" })
            : null;
          const revisionLabel =
            record.acceptedPlanRevisionNumber != null
              ? t("issuePlanDecompositionsSection.revisionLabel", {
            ...
            >
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="text-xs text-muted-foreground">
                  {t("issuePlanDecompositionsSection.planLabel", {
                ...
                <span className="inline-flex items-center gap-1 text-xs text-foreground">
                  ...
                  {t("issuePlanDecompositionsSection.childTasksCreated", {
                    defaultValue: "{{created}} of {{requested}} child {{taskWord}} created",
                    created,
                    requested,
                    taskWord:
                      requested === 1
                        ? t("issuePlanDecompositionsSection.taskWord.one", { defaultValue: "task" })
                        : t("issuePlanDecompositionsSection.taskWord.other", {
                            defaultValue: "tasks",
                          }),
                  })}
                </span>
                {record.status === "completed" && requested > 0 ? (
                  <span
                    ...
                    title={t("issuePlanDecompositionsSection.idempotentClaim.tooltip", {
                      defaultValue:
                        "Repeat attempts with this fingerprint reuse this record instead of creating new children",
                    })}
                  >
                    <Repeat className="h-3 w-3" />
                    {t("issuePlanDecompositionsSection.idempotentClaim.label", {
              ...
              <div className="mt-1 flex flex-wrap gap-x-3 gap-y-0.5 text-(length:--text-micro) text-muted-foreground">
                {ownerName ? (
                  <span>
                    {t("issuePlanDecompositionsSection.owner", {
                ...
                {startedAt ? (
                  <span title={formatDateTime(startedAt)}>
                    {t("issuePlanDecompositionsSection.started", {
                ...
                {completedAt ? (
                  <span title={formatDateTime(completedAt)}>
                    {t("issuePlanDecompositionsSection.completedRelative", {
                  ...
                  <span title={formatDateTime(updatedAt)}>
                    {t("issuePlanDecompositionsSection.updatedRelative", {
                ...
                {issueIdentifier ? (
                  ...
                  >
                    {t("issuePlanDecompositionsSection.planDocument", { defaultValue: "Plan document" })}
                  </Link>
                ) : null}
              </div>
