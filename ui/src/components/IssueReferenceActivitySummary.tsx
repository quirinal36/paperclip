...
export function IssueReferenceActivitySummary({ event }: { event: Pick<ActivityEvent, "details"> }) {
  ...
  return (
    <div className="mt-2 space-y-1">
      <Section
        label={t("issueReferenceActivitySummary.sections.addedReferences", { defaultValue: "Added references" })}
        icon={<Plus className="h-3 w-3 text-green-600 dark:text-green-400" aria-hidden="true" />}
        items={added}
      />
      <Section
        label={t("issueReferenceActivitySummary.sections.removedReferences", { defaultValue: "Removed references" })}
