  ...
  return (
    <div className="relative w-48 sm:w-64 md:w-80">
      ...
      <Input
        ...
        placeholder={t("issuesList.search.placeholder", { defaultValue: "Search tasks..." })}
        className="pl-7 text-xs sm:text-sm"
        aria-label={t("issuesList.search.ariaLabel", { defaultValue: "Search tasks" })}
...
function SubIssueProgressSummaryStrip({
  ...
  const costSummaryTooltip = costSummary
    ? t("issuesList.costSummary.tooltip", {
        defaultValue: "{{runCount}} {{runLabel}} across {{issueCount}} {{subtaskLabel}}",
        runCount: costSummary.runCount.toLocaleString(),
        runLabel: costSummary.runCount === 1
          ? t("issuesList.run.singular", { defaultValue: "run" })
          : t("issuesList.run.plural", { defaultValue: "runs" }),
        issueCount: costSummary.issueCount,
        subtaskLabel: costSummary.issueCount === 1
          ? t("issuesList.subtask.singular", { defaultValue: "sub-task" })
          : t("issuesList.subtask.plural", { defaultValue: "sub-tasks" }),
      })
    : "";
        ...
        <div className="min-w-0 flex-1 space-y-2">
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm">
            <span className="font-medium text-foreground">
              {t("issuesList.progress.done", { defaultValue: "{{done}}/{{total}} done", done: summary.doneCount, total: summary.totalCount })}
            </span>
            <span className="text-muted-foreground">
              {t("issuesList.progress.inProgress", { defaultValue: "{{count}} in progress", count: summary.inProgressCount })}
            </span>
            <span className="text-muted-foreground">
              {t("issuesList.progress.blocked", { defaultValue: "{{count}} blocked", count: summary.blockedCount })}
            </span>
            {showCostSummary && (
              <>
                <span
                  className="text-muted-foreground tabular-nums"
                  title={costSummaryTooltip}
                >
                  {t("issuesList.costSummary.tokens", { defaultValue: "{{tokens}} tokens", tokens: formatTokens(totalTokens) })}
                </span>
                <span className="text-muted-foreground tabular-nums">
                  {t("issuesList.costSummary.runtime", { defaultValue: "{{runtime}} runtime", runtime: formatDurationMs(costSummary.runtimeMs) })}
                </span>
              </>
            )}
          </div>
          <div
            role="progressbar"
            aria-label={t("issuesList.progress.ariaLabel", { defaultValue: "Sub-tasks completion progress" })}
            aria-valuemin={0}
          ...
          >
            {statusEntries.map(({ status, count }) => (
              <span
                ...
                title={`${t(`issuesList.status.${status}`, { defaultValue: issueStatusLabels[status] })}: ${count}`}
                aria-hidden="true"
              />
            ))}
          </div>
          ...
          {target && targetIssue ? (
            <>
              <div className="text-xs font-medium text-muted-foreground">
                {target.kind === "next"
                  ? t("issuesList.progress.nextUp", { defaultValue: "Next up" })
                  : t("issuesList.progress.waitingOnBlockers", { defaultValue: "Waiting on blockers" })}
              </div>
              <Link
            ...
            <div className="text-sm font-medium text-foreground">{t("issuesList.progress.noActive", { defaultValue: "No active sub-tasks" })}</div>
          ) : summary.doneCount === summary.totalCount ? (
            <div className="text-sm font-medium text-foreground">{t("issuesList.progress.allDone", { defaultValue: "All sub-tasks done" })}</div>
          ) : (
            <div className="text-sm font-medium text-foreground">{t("issuesList.progress.noActionable", { defaultValue: "No actionable sub-tasks" })}</div>
          )}
        </div>
      </div>
    </div>
...
export function IssuesList({
    ...
    if (currentUserId) {
      options.set(`user:${currentUserId}`, {
        ...
        label: currentUserId === "local-board"
          ? t("issuesList.creator.board", { defaultValue: "Board" })
          : t("issuesList.creator.me", { defaultValue: "Me" }),
        kind: "user",
  ...
  const groupedContent = useMemo(() => {
    ...
    if (viewState.groupBy === "workspace") {
      ...
      return Object.keys(groups)
        ...
        .map((key) => ({
          key,
          label: key === "__no_workspace" ? t("issuesList.groups.noWorkspace", { defaultValue: "No Workspace" }) : (workspaceNameMap.get(key) ?? key.slice(0, 8)),
          items: groups[key]!,
        }));
    }
    if (viewState.groupBy === "project") {
      ...
      return Object.keys(groups)
        ...
        .map((key) => ({
          key,
          label: key === "__no_project" ? t("issuesList.groups.noProject", { defaultValue: "No Project" }) : (projectById.get(key)?.name ?? key.slice(0, 8)),
          items: groups[key]!,
        }));
    }
    if (viewState.groupBy === "parent") {
      ...
      return Object.keys(groups)
        ...
        .map((key) => ({
          key,
          label: key === "__no_parent" ? t("issuesList.groups.noParent", { defaultValue: "No Parent" }) : (issueTitleMap.get(key) ?? key.slice(0, 8)),
          items: groups[key]!,
        }));
    }
    // assignee
    ...
    return Object.keys(groups).map((key) => ({
      ...
      label:
        key === "__unassigned"
          ? t("issuesList.groups.unassigned", { defaultValue: "Unassigned" })
          : key.startsWith("__user:")
            ? (formatAssigneeUserLabel(key.slice("__user:".length), currentUserId, companyUserLabelMap) ?? t("issuesList.groups.user", { defaultValue: "User" }))
  ...
  const createActionLabel = createIssueLabel
    ? t("issuesList.actions.createNamed", { defaultValue: "Create {{label}}", label: createIssueLabel })
    : t("issuesList.actions.createTask", { defaultValue: "Create Task" });
  const createButtonLabel = createIssueLabel
    ? t("issuesList.actions.newNamed", { defaultValue: "New {{label}}", label: createIssueLabel })
    : t("issuesList.actions.newTask", { defaultValue: "New Task" });
    ...
    <div ref={rootRef} className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-2 sm:gap-3">
        ...
        <div className="flex items-center gap-0.5 sm:gap-1 shrink-0">
          {/* View mode toggle */}
          <div className="flex items-center border border-border rounded-md overflow-hidden mr-1" role="group" aria-label={t("issuesList.viewMode.groupAriaLabel", { defaultValue: "View mode" })}>
            <button
              ...
              title={t("issuesList.viewMode.list", { defaultValue: "List view" })}
              aria-label={t("issuesList.viewMode.list", { defaultValue: "List view" })}
            ...
            <button
              ...
              title={t("issuesList.viewMode.board", { defaultValue: "Board view" })}
              aria-label={t("issuesList.viewMode.board", { defaultValue: "Board view" })}
          ...
          {viewState.viewMode === "list" && (
            <Button
              ...
              title={viewState.nestingEnabled ? t("issuesList.nesting.disable", { defaultValue: "Disable parent-child nesting" }) : t("issuesList.nesting.enable", { defaultValue: "Enable parent-child nesting" })}
            >
          ...
          {viewState.viewMode === "board" && (
            <>
              <Button
                ...
                title={boardCompactCards ? t("issuesList.board.useComfortable", { defaultValue: "Use comfortable cards" }) : t("issuesList.board.useCompact", { defaultValue: "Use compact cards" })}
              >
              ...
              <Button
                ...
                title={boardCollapsedStatuses.length > 0 ? t("issuesList.board.expandColdLanes", { defaultValue: "Expand cold lanes" }) : t("issuesList.board.collapseColdLanes", { defaultValue: "Collapse cold lanes" })}
              >
          ...
          <IssueColumnPicker
            ...
            title={t("issuesList.columns.pickerTitle", { defaultValue: "Choose which task columns stay visible" })}
            iconOnly
          />
          ...
          {viewState.viewMode === "list" && (
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="outline" size="icon" className="h-8 w-8 shrink-0" title={t("issuesList.toolbar.sort", { defaultValue: "Sort" })}>
              ...
              <PopoverContent align="end" className="w-48 p-0">
                <div className="p-2 space-y-0.5">
                  {([
                    ["workflow", t("issuesList.sort.workflow", { defaultValue: "Workflow" })],
                    ["status", t("issuesList.sort.status", { defaultValue: "Status" })],
                    ["priority", t("issuesList.sort.priority", { defaultValue: "Priority" })],
                    ["title", t("issuesList.sort.title", { defaultValue: "Title" })],
                    ["created", t("issuesList.sort.created", { defaultValue: "Created" })],
                    ["updated", t("issuesList.sort.updated", { defaultValue: "Updated" })],
          ...
          {viewState.viewMode === "list" && (
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="outline" size="icon" className="h-8 w-8 shrink-0" title={t("issuesList.toolbar.group", { defaultValue: "Group" })}>
              ...
              <PopoverContent align="end" className="w-44 p-0">
                <div className="p-2 space-y-0.5">
                  {([
                    ...
                    ["priority", t("issuesList.groupBy.priority", { defaultValue: "Priority" })],
                    ["assignee", t("issuesList.groupBy.assignee", { defaultValue: "Responsible" })],
                    ["project", t("issuesList.groupBy.project", { defaultValue: "Project" })],
                    ["workspace", t("issuesList.groupBy.workspace", { defaultValue: "Workspace" })],
                    ["parent", t("issuesList.groupBy.parent", { defaultValue: "Parent Task" })],
      ...
      {!searchWithinLoadedIssues && normalizedIssueSearch.length > 0 && searchedIssues.length === ISSUE_SEARCH_RESULT_LIMIT && (
        <p className="text-xs text-muted-foreground">
          {t("issuesList.search.showingUpTo", { defaultValue: "Showing up to {{limit}} matches. Refine the search to narrow further.", limit: ISSUE_SEARCH_RESULT_LIMIT })}
        </p>
      )}
      {boardColumnLimitReached && (
        <p className="text-xs text-muted-foreground">
          {t("issuesList.board.columnLimit", { defaultValue: "Some board columns are showing up to {{limit}} tasks. Refine filters or search to reveal the rest.", limit: ISSUE_BOARD_COLUMN_RESULT_LIMIT })}
        </p>
      )}
      {!isLoading && !externalObjectFilterLoading && filtered.length === 0 && viewState.viewMode === "list" && (
        <EmptyState
          icon={CircleDot}
          message={t("issuesList.empty.message", { defaultValue: "No tasks match the current filters or search." })}
        ...
        <>
              ...
              <IssueGroupHeader
                ...
                trailing={(
                  <Button
                    ...
                    title={t("issuesList.actions.newTaskInGroup", { defaultValue: "New task in {{group}}", group: group.label })}
                    aria-label={t("issuesList.actions.newTaskInGroup", { defaultValue: "New task in {{group}}", group: group.label })}
                ...
                const renderIssueRow = (issue: Issue, depth: number) => {
                  ...
                  const visibleBlockerChips = unresolvedVisibleBlockers
                    .map((blockerId) => {
                      ...
                      const blockerStepSuffix = blockerStep ? t("issuesList.checklist.stepSuffix", { defaultValue: " \u00b7 step {{step}}", step: blockerStep }) : "";
                      return { blockerId, chipLabel: t("issuesList.checklist.blockedBy", { defaultValue: "blocked by {{label}}{{suffix}}", label, suffix: blockerStepSuffix }) };
                    })
                  ...
                  const additionalVisibleBlockerLabel = additionalVisibleBlockerCount > 0
                    ? t("issuesList.checklist.andMore", { defaultValue: " ... and {{count}} more", count: additionalVisibleBlockerCount })
                    : "";
                          ...
                          <>
                            {hasChildren && !isExpanded ? (
                              <span className="ml-1.5 text-xs text-muted-foreground">
                                {t("issuesList.subtask.countParen", {
                                  defaultValue: "({{count}} {{label}})",
                                  count: totalDescendants,
                                  label: totalDescendants !== 1
                                    ? t("issuesList.subtask.plural", { defaultValue: "sub-tasks" })
                                    : t("issuesList.subtask.singular", { defaultValue: "sub-task" }),
                                })}
                              </span>
                            ...
                            {issueBadge ? (
                              issueBadge === "Paused" ? (
                                <Badge variant="ghost"
                                  ...
                                  aria-label={t("issuesList.badge.paused", { defaultValue: "Paused" })}
                                  title={t("issuesList.badge.paused", { defaultValue: "Paused" })}
                                >
                                  <CircleSlash2 className="h-3 w-3" />
                                  {t("issuesList.badge.paused", { defaultValue: "Paused" })}
                                </Badge>
                              ) : (
                            ...
                            {isSuccessfulRunHandoffRequired(issue) ? (
                              <Badge variant="outline"
                                ...
                                aria-label={t("issuesList.badge.needsNextStep", { defaultValue: "Needs next step" })}
                                title={t("issuesList.badge.needsNextStepTitle", { defaultValue: "This task needs a next step" })}
                              >
                                <CircleDot className="h-3 w-3" />
                                {t("issuesList.badge.needsNextStep", { defaultValue: "Needs next step" })}
                              </Badge>
                            ) : null}
                          </>
                        )}
                                ...
                                >
                                    ...
                                    >
                                      {issue.assigneeAgentId && agentName(issue.assigneeAgentId) ? (
                                        ...
                                        <span className="inline-flex items-center gap-1.5 text-xs text-muted-foreground">
                                          ...
                                          {t("issuesList.assignee.label", { defaultValue: "Assignee" })}
                                        </span>
                                      )}
                                    </button>
                                  </PopoverTrigger>
                                  ...
                                  >
                                    <input
                                      ...
                                      placeholder={t("issuesList.assignee.searchPlaceholder", { defaultValue: "Search responsible..." })}
                                    ...
                                    <div className="max-h-48 overflow-y-auto overscroll-contain">
                                      ...
                                      >
                                        {t("issuesList.assignee.none", { defaultValue: "No responsible" })}
                                      </button>
                                      {currentUserId && (
                                        <button
                                          className={cn(
                                        ...
                                        >
                                          ...
                                          <span>{t("issuesList.assignee.me", { defaultValue: "Me" })}</span>
                                        </button>
                                      )}
                                      {(agents ?? [])
                                        .filter((agent) => {
          ...
          {(remainingIssueRowCount > 0 || hasMoreIssues || isLoadingMoreIssues) && (
            <div className="py-2" data-testid="issues-load-more-sentinel">
              <p className="text-xs text-muted-foreground">
                {isLoadingMoreIssues
                  ? t("issuesList.loadMore.loading", { defaultValue: "Loading more tasks..." })
                  : remainingIssueRowCount > 0
                    ? t("issuesList.loadMore.rendering", { defaultValue: "Rendering {{rendered}} of {{total}} tasks", rendered: Math.min(renderedIssueRowLimit, filtered.length), total: filtered.length })
                    : t("issuesList.loadMore.scroll", { defaultValue: "Scroll to load more tasks" })}
              </p>
            </div>
          )}
        </>
