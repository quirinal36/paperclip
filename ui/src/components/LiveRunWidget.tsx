    ...
    <div className="overflow-hidden rounded-xl border border-blue-500/25 bg-background/80 shadow-(--shadow-extract-11)">
      <div className="border-b border-border/60 bg-blue-500/[0.04] px-4 py-3">
        <div className="text-xs font-semibold uppercase tracking-(--tracking-caps) text-blue-700 dark:text-blue-300">
          {t("liveRunWidget.title", { defaultValue: "Live Runs" })}
        </div>
            ...
            <section key={run.id} className="px-4 py-4">
              <div className="mb-3 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                ...
                <div className="flex items-center gap-2">
                  {isActive && (
                    ...
                    >
                      ...
                      {cancellingRunIds.has(run.id)
                        ? t("liveRunWidget.actions.stopping", { defaultValue: "Stopping…" })
                        : t("liveRunWidget.actions.stop", { defaultValue: "Stop" })}
                    </button>
                  )}
                  <Link
                    to={`/agents/${run.agentId}/runs/${run.id}`}
                  ...
                  >
                    {t("liveRunWidget.actions.openRun", { defaultValue: "Open run" })}
