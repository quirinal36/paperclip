      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-border">
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <span>{newGoalDefaults.parentId ? t("newGoalDialog.header.newSubGoal", { defaultValue: "New sub-goal" }) : t("newGoalDialog.header.newGoal", { defaultValue: "New goal" })}</span>
          </div>
        ...
        <div className="px-4 pt-4 pb-2 shrink-0">
          <input
            ...
            placeholder={t("newGoalDialog.fields.titlePlaceholder", { defaultValue: "Goal title" })}
            value={title}
        ...
        <div className="px-4 pb-2 overflow-y-auto max-h-(--sz-50vh)">
          <MarkdownEditor
            ...
            placeholder={t("newGoalDialog.fields.descriptionPlaceholder", { defaultValue: "Add description..." })}
            bordered={false}
          ...
          <Popover open={levelOpen} onOpenChange={setLevelOpen}>
            <PopoverTrigger asChild>
              <button className="inline-flex items-center gap-1.5 rounded-md border border-border px-2 py-1 text-xs hover:bg-accent/50 transition-colors">
                ...
                {t(`newGoalDialog.levels.${level}`, { defaultValue: levelLabels[level] ?? level })}
              </button>
            </PopoverTrigger>
            <PopoverContent className="w-40 p-1" align="start">
              {GOAL_LEVELS.map((l) => (
                ...
                >
                  {t(`newGoalDialog.levels.${l}`, { defaultValue: levelLabels[l] ?? l })}
                </button>
              ))}
            </PopoverContent>
          </Popover>
          ...
          <Popover open={parentOpen} onOpenChange={setParentOpen}>
            <PopoverTrigger asChild>
              <button className="inline-flex items-center gap-1.5 rounded-md border border-border px-2 py-1 text-xs hover:bg-accent/50 transition-colors">
                ...
                {currentParent ? currentParent.title : t("newGoalDialog.parent.placeholder", { defaultValue: "Parent goal" })}
              </button>
            </PopoverTrigger>
            <PopoverContent className="w-48 p-1" align="start">
              ...
              >
                {t("newGoalDialog.parent.none", { defaultValue: "No parent" })}
              </button>
              {(goals ?? []).map((g) => (
                <button
                  key={g.id}
        ...
        <div className="flex items-center justify-end px-4 py-2.5 border-t border-border">
          ...
          >
            {createGoal.isPending
              ? t("newGoalDialog.actions.creating", { defaultValue: "Creating…" })
              : newGoalDefaults.parentId
              ? t("newGoalDialog.actions.createSubGoal", { defaultValue: "Create sub-goal" })
              : t("newGoalDialog.actions.createGoal", { defaultValue: "Create goal" })}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
