...
const ISSUE_THINKING_EFFORT_OPTIONS = {
  claude_local: [
    { value: "", label: t("newIssueDialog.thinkingEffort.default", { defaultValue: "Default" }) },
    { value: "low", label: t("newIssueDialog.thinkingEffort.low", { defaultValue: "Low" }) },
    { value: "medium", label: t("newIssueDialog.thinkingEffort.medium", { defaultValue: "Medium" }) },
    { value: "high", label: t("newIssueDialog.thinkingEffort.high", { defaultValue: "High" }) },
  ],
  codex_local: [
    { value: "", label: t("newIssueDialog.thinkingEffort.default", { defaultValue: "Default" }) },
    { value: "minimal", label: t("newIssueDialog.thinkingEffort.minimal", { defaultValue: "Minimal" }) },
    { value: "low", label: t("newIssueDialog.thinkingEffort.low", { defaultValue: "Low" }) },
    { value: "medium", label: t("newIssueDialog.thinkingEffort.medium", { defaultValue: "Medium" }) },
    { value: "high", label: t("newIssueDialog.thinkingEffort.high", { defaultValue: "High" }) },
    { value: "xhigh", label: t("newIssueDialog.thinkingEffort.xhigh", { defaultValue: "X-High" }) },
  ],
  opencode_local: [
    { value: "", label: t("newIssueDialog.thinkingEffort.default", { defaultValue: "Default" }) },
    { value: "minimal", label: t("newIssueDialog.thinkingEffort.minimal", { defaultValue: "Minimal" }) },
    ...
    { value: "high", label: t("newIssueDialog.thinkingEffort.high", { defaultValue: "High" }) },
    { value: "xhigh", label: t("newIssueDialog.thinkingEffort.xhigh", { defaultValue: "X-High" }) },
    { value: "max", label: t("newIssueDialog.thinkingEffort.max", { defaultValue: "Max" }) },
  ],
} as const;
...
function buildStatusOptions(): ReadonlyArray<{ value: string; label: string; color: string; description?: string }> {
  ...
  return [
    {
      value: "backlog",
      label: t("newIssueDialog.status.backlog.label", { defaultValue: "Backlog" }),
      color: palette.backlog ?? issueStatusTextDefault,
      description: t("newIssueDialog.status.backlog.description", { defaultValue: "Parked - assignee will not be woken" }),
    },
    {
      value: "todo",
      label: t("newIssueDialog.status.todo.label", { defaultValue: "Todo" }),
      color: palette.todo ?? issueStatusTextDefault,
      description: t("newIssueDialog.status.todo.description", { defaultValue: "Executable - assignee will be woken" }),
    },
    { value: "in_progress", label: t("newIssueDialog.status.inProgress.label", { defaultValue: "In Progress" }), color: palette.in_progress ?? issueStatusTextDefault },
    { value: "in_review", label: t("newIssueDialog.status.inReview.label", { defaultValue: "In Review" }), color: palette.in_review ?? issueStatusTextDefault },
    { value: "done", label: t("newIssueDialog.status.done.label", { defaultValue: "Done" }), color: palette.done ?? issueStatusTextDefault },
  ];
}
...
const EXECUTION_WORKSPACE_MODES = [
  { value: "shared_workspace", label: t("newIssueDialog.executionWorkspace.mode.sharedWorkspace", { defaultValue: "Project default" }) },
  { value: "isolated_workspace", label: t("newIssueDialog.executionWorkspace.mode.isolatedWorkspace", { defaultValue: "New isolated workspace" }) },
  { value: "reuse_existing", label: t("newIssueDialog.executionWorkspace.mode.reuseExisting", { defaultValue: "Reuse existing workspace" }) },
] as const;
...
const IssueTitleTextarea = memo(function IssueTitleTextarea({
  ...
  return (
    <textarea
      ...
      placeholder={t("newIssueDialog.title.placeholder", { defaultValue: "Task title" })}
      rows={1}
      value={draftValue}
      onChange={(e) => {
...
const IssueDescriptionEditor = memo(function IssueDescriptionEditor({
  ...
  return (
    <MarkdownEditor
      ...
      placeholder={t("newIssueDialog.description.placeholder", { defaultValue: "Add description..." })}
      bordered={false}
      mentions={mentions}
...
export function NewIssueDialog() {
  ...
  const createIssue = useMutation({
    ...
    onSuccess: ({ issue, companyId, failures }) => {
      ...
      if (failures.length > 0) {
        ...
        const stagedFileNoun =
          failures.length === 1
            ? t("newIssueDialog.toast.uploadWarnings.fileNoun.one", { defaultValue: "file" })
            : t("newIssueDialog.toast.uploadWarnings.fileNoun.other", { defaultValue: "files" });
        pushToast({
          title: t("newIssueDialog.toast.uploadWarnings.title", { defaultValue: "Created {{issueRef}} with upload warnings", issueRef }),
          body: t("newIssueDialog.toast.uploadWarnings.body", {
            defaultValue: "{{fileCount}} staged {{noun}} could not be added.",
            fileCount: failures.length,
            noun: stagedFileNoun,
          }),
          tone: "warn",
          action: prefix
            ? { label: t("newIssueDialog.toast.uploadWarnings.openAction", { defaultValue: "Open {{issueRef}}", issueRef }), href: `/${prefix}/issues/${issueRef}` }
            : undefined,
        });
      }
      clearDraft();
  ...
  const assigneeOptionsTitle =
    assigneeAdapterType === "claude_local"
      ? t("newIssueDialog.assigneeOptions.claudeTitle", { defaultValue: "Claude options" })
      : assigneeAdapterType === "codex_local"
        ? t("newIssueDialog.assigneeOptions.codexTitle", { defaultValue: "Codex options" })
        : assigneeAdapterType === "opencode_local"
          ? t("newIssueDialog.assigneeOptions.opencodeTitle", { defaultValue: "OpenCode options" })
        : t("newIssueDialog.assigneeOptions.agentTitle", { defaultValue: "Agent options" });
  ...
  const createIssueErrorMessage =
    createIssue.error instanceof Error ? createIssue.error.message : t("newIssueDialog.errors.createFailed", { defaultValue: "Failed to create task. Try again." });
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-border shrink-0">
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <span>{isSubIssueMode ? t("newIssueDialog.header.newSubTask", { defaultValue: "New sub-task" }) : t("newIssueDialog.header.newTask", { defaultValue: "New task" })}</span>
          </div>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain">
          ...
          <div className="px-4 pb-2">
            <div className="overflow-x-auto overscroll-x-contain">
              ...
              <span className="w-6 shrink-0 text-center">{t("newIssueDialog.assignee.forLabel", { defaultValue: "For" })}</span>
              <InlineEntitySelector
                ref={assigneeSelectorRef}
                value={assigneeValue}
                options={assigneeOptions}
                recentOptionIds={recentAssigneeOptionIds}
                placeholder={t("newIssueDialog.assignee.placeholder", { defaultValue: "Assignee" })}
                disablePortal
                noneLabel={t("newIssueDialog.assignee.noneLabel", { defaultValue: "No assignee" })}
                searchPlaceholder={t("newIssueDialog.assignee.searchPlaceholder", { defaultValue: "Search assignees..." })}
                emptyMessage={t("newIssueDialog.assignee.emptyMessage", { defaultValue: "No assignees found." })}
                onChange={(value) => {
                ...
                renderTriggerValue={(option) =>
                  option ? (
                    ...
                    <span className="text-muted-foreground">{t("newIssueDialog.assignee.triggerPlaceholder", { defaultValue: "Assignee" })}</span>
                  )
                }
                renderOption={(option) => {
                  ...
                  return (
                    <>
                      ...
                      {assignee && getTrustPreset(assignee.permissions) === "low_trust_review" ? (
                        <ShieldAlert className="ml-auto h-3.5 w-3.5 shrink-0 text-amber-600 dark:text-amber-300" aria-label={t("newIssueDialog.assignee.lowTrustAriaLabel", { defaultValue: "Low-trust review agent" })} />
                      ) : null}
                    </>
                  );
                }}
              />
              <span>{t("newIssueDialog.project.inLabel", { defaultValue: "in" })}</span>
              <InlineEntitySelector
                ref={projectSelectorRef}
                value={projectId}
                options={projectOptions}
                recentOptionIds={recentProjectIds}
                placeholder={t("newIssueDialog.project.placeholder", { defaultValue: "Project" })}
                disablePortal
                noneLabel={t("newIssueDialog.project.noneLabel", { defaultValue: "No project" })}
                searchPlaceholder={t("newIssueDialog.project.searchPlaceholder", { defaultValue: "Search projects..." })}
                emptyMessage={t("newIssueDialog.project.emptyMessage", { defaultValue: "No projects found." })}
                ...
                renderTriggerValue={(option) =>
                  option && currentProject ? (
                    ...
                    <span className="text-muted-foreground">{t("newIssueDialog.project.triggerPlaceholder", { defaultValue: "Project" })}</span>
                  )
                }
                renderOption={(option) => {
                  if (!option.id) return <span className="truncate">{option.label}</span>;
              ...
              <Popover open={participantMenuOpen} onOpenChange={setParticipantMenuOpen}>
                <PopoverTrigger asChild>
                  <button
                    ...
                    title={taskWatchdogsEnabled ? t("newIssueDialog.participants.menuTitleWithWatchdog", { defaultValue: "Add reviewer, approver, or watchdog" }) : t("newIssueDialog.participants.menuTitle", { defaultValue: "Add reviewer or approver" })}
                  >
                ...
                <PopoverContent className="w-44 p-1" align="start">
                  ...
                  >
                    ...
                    {t("newIssueDialog.participants.reviewer", { defaultValue: "Reviewer" })}
                  </button>
                  <button
                    className={cn(
                      "flex items-center gap-2 w-full px-2 py-1.5 text-xs rounded hover:bg-accent/50",
                  ...
                  >
                    ...
                    {t("newIssueDialog.participants.approver", { defaultValue: "Approver" })}
                  </button>
                  {taskWatchdogsEnabled && (
                    ...
                    >
                      ...
                      {t("newIssueDialog.participants.watchdog", { defaultValue: "Watchdog" })}
                    </button>
                  )}
                </PopoverContent>
              </Popover>
            ...
            {showReviewerRow && (
              <div className="flex items-center gap-2 text-sm text-muted-foreground mt-1">
                ...
                options={assigneeOptions}
                recentOptionIds={recentAssigneeOptionIds}
                placeholder={t("newIssueDialog.reviewer.placeholder", { defaultValue: "Reviewer" })}
                disablePortal
                noneLabel={t("newIssueDialog.reviewer.noneLabel", { defaultValue: "No reviewer" })}
                searchPlaceholder={t("newIssueDialog.reviewer.searchPlaceholder", { defaultValue: "Search reviewers..." })}
                emptyMessage={t("newIssueDialog.reviewer.emptyMessage", { defaultValue: "No reviewers found." })}
                ...
                renderTriggerValue={(option) =>
                  option ? (
                    ...
                    <span className="text-muted-foreground">{t("newIssueDialog.reviewer.triggerPlaceholder", { defaultValue: "Reviewer" })}</span>
                  )
                }
                renderOption={(option) => {
                  if (!option.id) return <span className="truncate">{option.label}</span>;
            ...
            {showApproverRow && (
              <div className="flex items-center gap-2 text-sm text-muted-foreground mt-1">
                ...
                options={assigneeOptions}
                recentOptionIds={recentAssigneeOptionIds}
                placeholder={t("newIssueDialog.approver.placeholder", { defaultValue: "Approver" })}
                disablePortal
                noneLabel={t("newIssueDialog.approver.noneLabel", { defaultValue: "No approver" })}
                searchPlaceholder={t("newIssueDialog.approver.searchPlaceholder", { defaultValue: "Search approvers..." })}
                emptyMessage={t("newIssueDialog.approver.emptyMessage", { defaultValue: "No approvers found." })}
                ...
                renderTriggerValue={(option) =>
                  option ? (
                    ...
                    <span className="text-muted-foreground">{t("newIssueDialog.approver.triggerPlaceholder", { defaultValue: "Approver" })}</span>
                  )
                }
                renderOption={(option) => {
                  if (!option.id) return <span className="truncate">{option.label}</span>;
                ...
                <Popover open={watchdogEditorOpen} onOpenChange={setWatchdogEditorOpen}>
                  <PopoverTrigger asChild>
                    <button
                      ...
                      title={t("newIssueDialog.watchdog.configureTitle", { defaultValue: "Configure watchdog" })}
                    >
                      {selectedWatchdogAgent ? (
                        ...
                        <span className="text-muted-foreground">{t("newIssueDialog.watchdog.setWatchdog", { defaultValue: "Set watchdog" })}</span>
                      )}
                    </button>
                  </PopoverTrigger>
                  <PopoverContent className="w-80 p-3 space-y-3" align="start">
                    <div className="space-y-1.5">
                      <div className="text-xs font-medium text-foreground">{t("newIssueDialog.watchdog.agentLabel", { defaultValue: "Watchdog agent" })}</div>
                      <InlineEntitySelector
                        value={watchdogAgentId}
                        options={watchdogAgentOptions}
                        placeholder={t("newIssueDialog.watchdog.selectAgentPlaceholder", { defaultValue: "Select agent" })}
                        noneLabel={t("newIssueDialog.watchdog.noneLabel", { defaultValue: "No watchdog agent" })}
                        searchPlaceholder={t("newIssueDialog.watchdog.searchPlaceholder", { defaultValue: "Search agents..." })}
                        emptyMessage={t("newIssueDialog.watchdog.emptyMessage", { defaultValue: "No agents found." })}
                        ...
                        renderTriggerValue={(option) =>
                          option ? (
                            ...
                            <span className="text-muted-foreground">{t("newIssueDialog.watchdog.selectAgentTrigger", { defaultValue: "Select agent" })}</span>
                          )
                        }
                        renderOption={(option) => {
                    ...
                    <div className="space-y-1.5">
                      <div className="text-xs font-medium text-foreground">{t("newIssueDialog.watchdog.instructionsLabel", { defaultValue: "Instructions" })} <span className="font-normal text-muted-foreground">{t("newIssueDialog.watchdog.optional", { defaultValue: "(optional)" })}</span></div>
                      <Textarea
                        value={watchdogInstructions}
                        onChange={(event) => setWatchdogInstructions(event.target.value)}
                        placeholder={t("newIssueDialog.watchdog.instructionsPlaceholder", { defaultValue: "What should the watchdog watch for and how should it keep work moving?" })}
                        rows={4}
                        className="text-xs"
                      />
                    </div>
                    <div className="flex items-center justify-between">
                      ...
                      >
                        {t("newIssueDialog.watchdog.remove", { defaultValue: "Remove" })}
                      </button>
                      <Button type="button" size="sm" className="h-7 text-xs" onClick={() => setWatchdogEditorOpen(false)}>
                        {t("newIssueDialog.watchdog.done", { defaultValue: "Done" })}
                      </Button>
                    </div>
                  </PopoverContent>
                </Popover>
          ...
          {isSubIssueMode ? (
            ...
            <div className="max-w-full rounded-md border border-border bg-muted/30 px-2.5 py-1.5 text-xs text-muted-foreground">
              <div className="flex items-center gap-1.5">
                ...
                <span className="shrink-0">{t("newIssueDialog.subTask.parentPrefix", { defaultValue: "Sub-task of" })}</span>
          ...
          {currentProject && currentProjectSupportsExecutionWorkspace && (
            ...
            <div className="space-y-1.5">
              <div className="text-xs font-medium">{t("newIssueDialog.executionWorkspace.title", { defaultValue: "Execution workspace" })}</div>
              <div className="text-(length:--text-micro) text-muted-foreground">
                {t("newIssueDialog.executionWorkspace.description", { defaultValue: "Control whether this task runs in the shared workspace, a new isolated workspace, or an existing one." })}
              </div>
              <select
              ...
              {executionWorkspaceMode === "reuse_existing" && selectedReusableExecutionWorkspace && (
                <div className="text-(length:--text-micro) text-muted-foreground">
                  {t("newIssueDialog.executionWorkspace.reusing", {
                    defaultValue: "Reusing {{name}} from {{source}}.",
                    name: selectedReusableExecutionWorkspace.name,
                    source: selectedReusableExecutionWorkspace.branchName ?? selectedReusableExecutionWorkspace.cwd ?? t("newIssueDialog.executionWorkspace.existingWorkspaceFallback", { defaultValue: "existing execution workspace" }),
                  })}
                </div>
              )}
              {showParentWorkspaceWarning ? (
                <div className="rounded-md border border-amber-300/60 bg-amber-50 px-2 py-1.5 text-(length:--text-micro) text-amber-900 dark:border-amber-800/70 dark:bg-amber-950/30 dark:text-amber-100">
                  {t("newIssueDialog.executionWorkspace.parentWorkspaceWarning", {
                    defaultValue: "Warning: this sub-task will no longer use the parent task workspace{{suffix}}.",
          ...
          {supportsAssigneeOverrides && (
            ...
            {assigneeOptionsOpen && (
              <div className="mt-2 rounded-md border border-border p-3 bg-muted/20 space-y-3">
                <div className="space-y-1.5">
                  <div className="text-xs text-muted-foreground">{t("newIssueDialog.modelLane.label", { defaultValue: "Model lane" })}</div>
                  <div
                    className="flex w-full overflow-hidden rounded-md border border-border"
                    role="radiogroup"
                    aria-label={t("newIssueDialog.modelLane.ariaLabel", { defaultValue: "Model lane" })}
                  >
                    {(["primary", ...(assigneeSupportsCheapLane ? (["cheap"] as const) : ([] as const)), "custom"] as const).map((lane) => (
                      ...
                      >
                        {lane === "primary"
                          ? t("newIssueDialog.modelLane.primary", { defaultValue: "Primary" })
                          : lane === "cheap"
                            ? t("newIssueDialog.modelLane.cheap", { defaultValue: "Cheap" })
                            : t("newIssueDialog.modelLane.custom", { defaultValue: "Custom" })}
                      </button>
                    ))}
                  </div>
                  {assigneeModelLane === "cheap" && (
                    <p className="text-(length:--text-micro) text-muted-foreground">
                      {t("newIssueDialog.modelLane.cheapExplainer.sends", { defaultValue: "Sends" })} <code>modelProfile: "cheap"</code>{" "}
                      {assigneeCheapProfile?.adapterConfig && typeof (assigneeCheapProfile.adapterConfig as Record<string, unknown>).model === "string"
                        ? <>{t("newIssueDialog.modelLane.cheapExplainer.adapterDefault", { defaultValue: "· adapter default " })}<code>{String((assigneeCheapProfile.adapterConfig as Record<string, unknown>).model)}</code></>
                        : assigneeCheapProfile
                          ? <>{t("newIssueDialog.modelLane.cheapExplainer.configuredProfile", { defaultValue: "· uses the agent's configured cheap profile" })}</>
                          : <>{t("newIssueDialog.modelLane.cheapExplainer.fallback", { defaultValue: "· falls back to the primary model if no cheap profile is configured" })}</>}
                    </p>
                  )}
                  {assigneeModelLane === "primary" && (
                    <p className="text-(length:--text-micro) text-muted-foreground">{t("newIssueDialog.modelLane.primaryExplainer", { defaultValue: "Runs on the agent's primary model." })}</p>
                  )}
                  {assigneeModelLane === "custom" && (
                    <p className="text-(length:--text-micro) text-muted-foreground">{t("newIssueDialog.modelLane.customExplainer", { defaultValue: "Override the model and effort for this task only." })}</p>
                  )}
                </div>
                {assigneeModelLane === "custom" && (
                  <div className="space-y-1.5">
                    <div className="text-xs text-muted-foreground">{t("newIssueDialog.modelOverride.label", { defaultValue: "Model" })}</div>
                    <InlineEntitySelector
                      value={assigneeModelOverride}
                      options={modelOverrideOptions}
                      placeholder={t("newIssueDialog.modelOverride.defaultModel", { defaultValue: "Default model" })}
                      disablePortal
                      noneLabel={t("newIssueDialog.modelOverride.defaultModel", { defaultValue: "Default model" })}
                      searchPlaceholder={t("newIssueDialog.modelOverride.searchPlaceholder", { defaultValue: "Search models..." })}
                      emptyMessage={t("newIssueDialog.modelOverride.emptyMessage", { defaultValue: "No models found." })}
                ...
                {assigneeModelLane === "custom" && (
                  <div className="space-y-1.5">
                    <div className="text-xs text-muted-foreground">{t("newIssueDialog.thinkingEffort.label", { defaultValue: "Thinking effort" })}</div>
                ...
                {assigneeAdapterType === "claude_local" && assigneeModelLane === "custom" && (
                  <div className="flex items-center justify-between rounded-md border border-border px-2 py-1.5">
                    <div className="text-xs text-muted-foreground">{t("newIssueDialog.chrome.enableLabel", { defaultValue: "Enable Chrome (--chrome)" })}</div>
                    <ToggleSwitch
            ...
            {stagedFiles.length > 0 ? (
              ...
              {stagedDocuments.length > 0 ? (
                <div className="space-y-2">
                  <div className="text-xs font-medium text-muted-foreground">{t("newIssueDialog.stagedFiles.documentsHeading", { defaultValue: "Documents" })}</div>
                  <div className="space-y-2">
                    {stagedDocuments.map((file) => (
                      <div key={file.id} className="flex items-start justify-between gap-3 rounded-md border border-border/70 px-3 py-2">
                        ...
                        <Button
                          ...
                          disabled={createIssue.isPending}
                          title={t("newIssueDialog.stagedFiles.removeDocument", { defaultValue: "Remove document" })}
                        >
              ...
              {stagedAttachments.length > 0 ? (
                <div className="space-y-2">
                  <div className="text-xs font-medium text-muted-foreground">{t("newIssueDialog.stagedFiles.attachmentsHeading", { defaultValue: "Attachments" })}</div>
                  <div className="space-y-2">
                    {stagedAttachments.map((file) => (
                      <div key={file.id} className="flex items-start justify-between gap-3 rounded-md border border-border/70 px-3 py-2">
                        ...
                        <Button
                          ...
                          onClick={() => removeStagedFile(file.id)}
                          disabled={createIssue.isPending}
                          title={t("newIssueDialog.stagedFiles.removeAttachment", { defaultValue: "Remove attachment" })}
                        >
                          <X className="h-3.5 w-3.5" />
                        </Button>
                      </div>
        ...
        <div className="flex items-center gap-1.5 px-4 py-2 border-t border-border flex-wrap shrink-0">
              ...
              >
                {currentPriority ? (
                  ...
                  <>
                    ...
                    {t("newIssueDialog.priority.label", { defaultValue: "Priority" })}
                  </>
                )}
              </button>
            </PopoverTrigger>
          ...
          >
            ...
            {t("newIssueDialog.actions.upload", { defaultValue: "Upload" })}
          </button>
          ...
          <Popover open={moreOpen} onOpenChange={setMoreOpen}>
            ...
            <PopoverContent className="w-44 p-1" align="start" data-testid="new-issue-more-menu">
              ...
              <button className="flex items-center gap-2 w-full px-2 py-1.5 text-xs rounded hover:bg-accent/50 text-muted-foreground">
                ...
                {t("newIssueDialog.dates.startDate", { defaultValue: "Start date" })}
              </button>
              <button className="flex items-center gap-2 w-full px-2 py-1.5 text-xs rounded hover:bg-accent/50 text-muted-foreground">
                <Calendar className="h-3 w-3" />
                {t("newIssueDialog.dates.dueDate", { defaultValue: "Due date" })}
              </button>
            </PopoverContent>
          </Popover>
        </div>
        ...
        {assigneeValue && status === "backlog" ? (
          ...
          >
            ...
            <span className="leading-snug">
              {t("newIssueDialog.backlogNote.part1", { defaultValue: "Assigning implies executable intent - leave status as " })}
              <span className="font-medium">{t("newIssueDialog.status.backlog.label", { defaultValue: "Backlog" })}</span>
              {t("newIssueDialog.backlogNote.part2", { defaultValue: " only to deliberately park this. The assignee will not be woken until status moves to " })}
              <span className="font-medium">{t("newIssueDialog.status.todo.label", { defaultValue: "Todo" })}</span>
              {t("newIssueDialog.backlogNote.part3", { defaultValue: " or " })}
              <span className="font-medium">{t("newIssueDialog.status.inProgress.label", { defaultValue: "In Progress" })}</span>
              {t("newIssueDialog.backlogNote.part4", { defaultValue: "." })}
            </span>
          </div>
        ) : null}
        ...
        {currentAssigneeLowTrust ? (
          ...
          >
            ...
            <span className="leading-snug">
              {t("newIssueDialog.lowTrustNote", { defaultValue: "Low-trust review agent. It can only act inside its assigned review boundary; issue, project, or run policy defines the concrete scope." })}
            </span>
          </div>
        ) : null}
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-t border-border shrink-0">
          ...
          >
            {t("newIssueDialog.actions.discardDraft", { defaultValue: "Discard Draft" })}
          </Button>
          <div className="flex items-center gap-3">
            <div className="min-h-5 text-right">
              {createIssue.isPending ? (
                <span className="inline-flex items-center gap-1.5 text-xs font-medium text-muted-foreground">
                  ...
                  {t("newIssueDialog.footer.creatingIssue", { defaultValue: "Creating issue..." })}
                </span>
              ) : createIssue.isError ? (
            ...
            >
              <span className="inline-flex items-center justify-center gap-1.5">
                ...
                <span>{createIssue.isPending ? t("newIssueDialog.actions.creating", { defaultValue: "Creating..." }) : isSubIssueMode ? t("newIssueDialog.actions.createSubTask", { defaultValue: "Create Sub-Task" }) : t("newIssueDialog.actions.createTask", { defaultValue: "Create Task" })}</span>
              </span>
            </Button>
          </div>
        </div>
