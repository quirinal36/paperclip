      ...
      {warning.href ? (
        <>
          ...
          <Link to={warning.href} className="font-medium underline underline-offset-2">
            {warning.hrefLabel ?? t("pipelineHealthWarnings.link.openLabel", { defaultValue: "Open" })}
          </Link>
        </>
      ) : null}
    </>
  ...
  return (
    ...
    >
      <h2 id="pipeline-health-bar-heading" className="flex items-center gap-2 text-sm font-semibold">
        ...
        <span>
          {t("pipelineHealthWarnings.boardBar.title", {
            count: warnings.length,
            defaultValue: "Some steps won't run yet — {{count}} thing to fix",
            defaultValue_other: "Some steps won't run yet — {{count}} things to fix",
          })}
        </span>
      </h2>
          ...
          return (
            <li key={`${warning.stageId}-${warning.code}-${index}`} className="list-disc">
              {warning.href ? (
                ...
                <button
                  type="button"
                  aria-label={t("pipelineHealthWarnings.boardBar.openStageSettings", {
      ...
      {overflow > 0 ? (
        <p className="mt-1.5 pl-6 text-xs text-amber-800/80 dark:text-amber-200/70">
          {t("pipelineHealthWarnings.boardBar.overflow", {
            defaultValue: "+{{overflow}} more in stage settings",
            overflow,
          })}
        </p>
      ) : null}
  ...
  return (
    ...
    >
      ...
      >
        <AlertTriangle className="h-4 w-4 shrink-0" />
        <span>
          {t("pipelineHealthWarnings.stageWarnings.title", {
            count: warnings.length,
            defaultValue: "This step won't run yet",
            defaultValue_other: "This step won't run yet — {{count}} things to fix",
          })}
        </span>
      </h2>
