    ...
    return {
      ...
      name:
        ...
        (revision.createdByUserId === "local-board"
          ? t("pipelineItemBodyDocument.actor.board", { defaultValue: "Board" })
          : revision.createdByUserId.slice(0, 8)),
      imageUrl: profile?.image ?? null,
    };
  }

  return { kind: "system", name: t("pipelineItemBodyDocument.actor.system", { defaultValue: "System" }) };
}
...
export function PipelineItemBodyDocument({
  ...
  const restoreMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      await invalidateAll();
      pushToast({ title: t("pipelineItemBodyDocument.toast.revisionRestored", { defaultValue: "Revision restored" }), tone: "success" });
    },
    onError: () =>
      pushToast({
        title: t("pipelineItemBodyDocument.toast.restoreFailed", { defaultValue: "Could not restore the revision" }),
        tone: "error",
      }),
  });
  ...
  const handleSave = useCallback(async () => {
    try {
      ...
      if (error instanceof ApiError && error.status === 409) {
        ...
        pushToast({
          title: t("pipelineItemBodyDocument.toast.conflictTitle", { defaultValue: "Body changed elsewhere" }),
          body: t("pipelineItemBodyDocument.toast.conflictBody", {
      ...
      pushToast({ title: t("pipelineItemBodyDocument.toast.saveFailed", { defaultValue: "Could not save the body" }), tone: "error" });
    }
  ...
  const handleStartConversationFromAnchor = useCallback(
    async (anchor: PendingAnchor) => {
      ...
      try {
        ...
        setPendingStartAnchor(null);
        pushToast({
          title: t("pipelineItemBodyDocument.toast.startConversationFailed", { defaultValue: "Could not start the conversation" }),
          tone: "error",
        });
      }
    },
  ...
  if (editing) {
      ...
      >
        <div className="rounded-md border border-border bg-background">
          <MarkdownEditor
            ...
            placeholder={t("pipelineItemBodyDocument.editor.placeholder", {
              defaultValue: "Write the item body in Markdown…",
            })}
            bordered={false}
        ...
        <div className="mt-2 flex flex-wrap items-center justify-between gap-2">
          <span className="text-(length:--text-micro) text-muted-foreground">
            {t("pipelineItemBodyDocument.editor.savingHint", {
              defaultValue: "Saving creates rev {{rev}} · ⌘↵ to save · Esc to cancel",
              rev: (doc?.latestRevisionNumber ?? 0) + 1,
            })}
          </span>
          <div className="flex items-center gap-2">
            <Button variant="ghost" size="sm" onClick={cancelEdit} disabled={saveMutation.isPending}>
              {t("pipelineItemBodyDocument.actions.cancel", { defaultValue: "Cancel" })}
            </Button>
            <Button size="sm" onClick={() => void handleSave()} disabled={saveMutation.isPending}>
              ...
              {saveMutation.isPending
                ? t("pipelineItemBodyDocument.actions.saving", { defaultValue: "Saving…" })
                : t("pipelineItemBodyDocument.actions.save", { defaultValue: "Save" })}
            </Button>
          </div>
        </div>
      </div>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div className="space-y-1">
              <p className="text-sm font-medium text-amber-800 dark:text-amber-200">
                {t("pipelineItemBodyDocument.historical.viewingRevision", {
                  defaultValue: "Viewing revision {{revisionNumber}}",
                  revisionNumber: selectedHistoricalRevision.revisionNumber,
                })}
              </p>
              <p className="text-xs text-muted-foreground">
                {t("pipelineItemBodyDocument.historical.description", {
                  defaultValue:
                    "Historical preview. New comments are disabled while previewing a historical revision. Restoring it creates a new latest revision and keeps history append-only.",
                })}
              </p>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <Button variant="outline" size="sm" onClick={() => setSelectedRevisionId(null)}>
                {t("pipelineItemBodyDocument.historical.returnToLatest", { defaultValue: "Return to latest" })}
              </Button>
              <Button
                size="sm"
              ...
              >
                {restoreMutation.isPending
                  ? t("pipelineItemBodyDocument.historical.restoring", { defaultValue: "Restoring…" })
                  : t("pipelineItemBodyDocument.historical.restore", { defaultValue: "Restore this revision" })}
              </Button>
            </div>
          </div>
        </div>
    ...
    bodyContent = (
      <EmptyState
        icon={FileText}
        message={t("pipelineItemBodyDocument.empty.message", {
          defaultValue: "No body yet. Capture the item's details here.",
        })}
        action={t("pipelineItemBodyDocument.empty.action", { defaultValue: "Add the item body" })}
        onAction={beginEdit}
      />
    );
  ...
  return (
    <section
      aria-label={t("pipelineItemBodyDocument.ariaLabel", { defaultValue: "Item body" })}
    ...
    >
      <DocumentFrameHeader
        ...
        documentLabel={t("pipelineItemBodyDocument.documentLabel", { defaultValue: "Item body document" })}
        folded={folded}
        ...
        actionsSlot={editing ? (
          <span className="text-(length:--text-micro) font-medium text-amber-700 dark:text-amber-300">
            ● {t("pipelineItemBodyDocument.header.editingUnsaved", { defaultValue: "Editing · unsaved" })}
          </span>
        ) : (
          <Button variant="ghost" size="sm" className="h-auto gap-1.5 px-2 py-1 text-xs" onClick={beginEdit}>
            <FilePenLine className="h-3.5 w-3.5" />
            {t("pipelineItemBodyDocument.actions.edit", { defaultValue: "Edit" })}
          </Button>
        )}
      />
