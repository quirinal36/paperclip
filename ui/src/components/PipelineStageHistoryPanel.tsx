...
export function PipelineStageHistoryPanel({
  ...
  const restore = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        title: t("pipelineStageHistoryPanel.toast.restored.title", {
          defaultValue: "Restored revision {{number}}",
          number: result.restoredFromRevisionNumber,
        }),
        body: t("pipelineStageHistoryPanel.toast.restored.body", {
    ...
    onError: (error) => {
      pushToast({
        title: t("pipelineStageHistoryPanel.toast.restoreFailed.title", { defaultValue: "Failed to restore revision" }),
        body:
          error instanceof Error
            ? error.message
            : t("pipelineStageHistoryPanel.toast.restoreFailed.body", {
    ...
    <Collapsible open={open} onOpenChange={setOpen} className="overflow-hidden rounded-lg border border-border/70">
      <CollapsibleTrigger className="flex w-full items-center justify-between px-3 py-2 text-left">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-sm font-medium">{t("pipelineStageHistoryPanel.history.title", { defaultValue: "History" })}</p>
            <p className="text-xs text-muted-foreground">{t("pipelineStageHistoryPanel.history.subtitle", { defaultValue: "Past versions of these instructions." })}</p>
          </div>
        </div>
        {open ? (
      ...
      <CollapsibleContent className="border-t border-border/70">
        {!hasDocument ? (
          <p className="px-4 py-3 text-xs text-muted-foreground">
            {t("pipelineStageHistoryPanel.emptyNoDocument", {
              defaultValue: "No history yet. Save the instructions to create the first revision.",
            })}
          </p>
        ) : revisionsQuery.isLoading ? (
          <p className="px-4 py-3 text-xs text-muted-foreground">{t("pipelineStageHistoryPanel.loading", { defaultValue: "Loading revisions…" })}</p>
        ) : revisionsQuery.error ? (
          <p className="px-4 py-3 text-xs text-destructive">
            {revisionsQuery.error instanceof Error
              ? revisionsQuery.error.message
              : t("pipelineStageHistoryPanel.errors.loadFailed", { defaultValue: "Could not load revisions." })}
          </p>
        ) : revisions.length === 0 ? (
          <p className="px-4 py-3 text-xs text-muted-foreground">{t("pipelineStageHistoryPanel.empty", { defaultValue: "No revisions recorded yet." })}</p>
        ) : (
                ...
                >
                  <div className="min-w-0">
                    <p className="text-sm font-medium">
                      {t("pipelineStageHistoryPanel.revision.label", {
                      ...
                      {isCurrent ? (
                        <Badge variant="ghost" className="ml-2 bg-muted text-(length:--text-micro) text-muted-foreground">
                          {t("pipelineStageHistoryPanel.badge.current", { defaultValue: "Current" })}
                        </Badge>
                      ) : null}
                    </p>
                  ...
                  {isCurrent ? null : (
                    ...
                    >
                      ...
                      {t("pipelineStageHistoryPanel.actions.restore", { defaultValue: "Restore" })}
                    </Button>
                  )}
                </li>
              );
