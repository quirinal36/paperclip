...
export function PriorityIcon({ priority, onChange, className, showLabel }: PriorityIconProps) {
  ...
  if (!onChange) return showLabel ? <span className="inline-flex items-center gap-1.5">{icon}<span className="text-sm">{t(`priorityIcon.priorities.${priority}`, { defaultValue: config.label })}</span></span> : icon;
  ...
  const trigger = showLabel ? (
    <button className="inline-flex min-h-5 items-center gap-1.5 cursor-pointer hover:bg-accent/50 rounded px-1 -mx-1 py-0.5 transition-colors">
      {icon}
      <span className="text-sm">{t(`priorityIcon.priorities.${priority}`, { defaultValue: config.label })}</span>
    </button>
  ) : icon;
        ...
        {allPriorities.map((p) => {
          ...
          return (
            ...
            >
              ...
              {t(`priorityIcon.priorities.${p}`, { defaultValue: c.label })}
            </Button>
          );
        })}
      </PopoverContent>
