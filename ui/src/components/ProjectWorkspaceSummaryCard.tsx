...
function workspaceKindLabel(kind: ProjectWorkspaceSummary["kind"]) {
  return kind === "execution_workspace"
    ? t("projectWorkspaceSummaryCard.kind.executionWorkspace", { defaultValue: "Execution workspace" })
    : t("projectWorkspaceSummaryCard.kind.projectWorkspace", { defaultValue: "Project workspace" });
}
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {summary.serviceCount > 0 ? (
                ...
                >
                  ...
                  />
                  {t("projectWorkspaceSummaryCard.serviceCount", { defaultValue: "{{running}}/{{total}} services", running: summary.runningServiceCount, total: summary.serviceCount })}
                </Badge>
              ) : null}
          ...
          >
            {summary.hasRuntimeConfig ? (
              ...
              >
                ...
                {hasRunningServices
                  ? t("projectWorkspaceSummaryCard.actions.stopServices", { defaultValue: "Stop services" })
                  : t("projectWorkspaceSummaryCard.actions.startServices", { defaultValue: "Start services" })}
              </Button>
            ) : null}
            {summary.kind === "execution_workspace" && summary.executionWorkspaceId && summary.executionWorkspaceStatus ? (
              ...
              >
                {summary.executionWorkspaceStatus === "cleanup_failed"
                  ? t("projectWorkspaceSummaryCard.actions.retryClose", { defaultValue: "Retry close" })
                  : t("projectWorkspaceSummaryCard.actions.closeWorkspace", { defaultValue: "Close workspace" })}
              </Button>
            ) : null}
          </div>
        </div>
          ...
          <div className="space-y-2 text-sm">
            {summary.branchName ? (
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0 flex-1">
                  <div className="text-(length:--text-micro) uppercase tracking-(--tracking-eyebrow) text-muted-foreground">{t("projectWorkspaceSummaryCard.fields.branch", { defaultValue: "Branch" })}</div>
                  <div className="flex items-start gap-2">
                    <CopyText
                      text={summary.branchName}
                      containerClassName="min-w-0"
                      className="min-w-0 break-all text-left font-mono text-xs text-foreground"
                      copiedLabel={t("projectWorkspaceSummaryCard.branch.copied", { defaultValue: "Branch copied" })}
                    >
                      {summary.branchName}
                    </CopyText>
                    <CopyText
                      text={summary.branchName}
                      ariaLabel={t("projectWorkspaceSummaryCard.branch.copyAria", { defaultValue: "Copy branch" })}
            ...
            {summary.cwd ? (
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0 flex-1">
                  <div className="text-(length:--text-micro) uppercase tracking-(--tracking-eyebrow) text-muted-foreground">{t("projectWorkspaceSummaryCard.fields.path", { defaultValue: "Path" })}</div>
                  <div className="flex items-start gap-2">
                    <CopyText
                      text={summary.cwd}
                      title={summary.cwd}
                      containerClassName="min-w-0"
                      className="min-w-0 break-all text-left font-mono text-xs text-foreground"
                      copiedLabel={t("projectWorkspaceSummaryCard.path.copied", { defaultValue: "Path copied" })}
                    >
                      {truncatePath(summary.cwd)}
                    </CopyText>
                    <CopyText
                      text={summary.cwd}
                      ariaLabel={t("projectWorkspaceSummaryCard.path.copyAria", { defaultValue: "Copy path" })}
                      className="mt-0.5 shrink-0 text-muted-foreground hover:text-foreground"
                      copiedLabel={t("projectWorkspaceSummaryCard.path.copied", { defaultValue: "Path copied" })}
                    >
                      <Copy className="h-3.5 w-3.5" />
            ...
            {summary.primaryServiceUrl ? (
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0">
                  <div className="text-(length:--text-micro) uppercase tracking-(--tracking-eyebrow) text-muted-foreground">{t("projectWorkspaceSummaryCard.fields.service", { defaultValue: "Service" })}</div>
                  <a
        ...
        {summary.issues.length > 0 ? (
          <div className="space-y-2">
            <div className="text-(length:--text-micro) font-medium uppercase tracking-(--tracking-eyebrow) text-muted-foreground">
              {t("projectWorkspaceSummaryCard.linkedTasks", { defaultValue: "Linked tasks" })}
            </div>
