...
export function ProjectWorkspacesContent({
  ...
  if (summaries.length === 0) {
    return <p className="text-sm text-muted-foreground">{t("projectWorkspacesContent.emptyState", { defaultValue: "No non-default workspace activity yet." })}</p>;
  }
        ...
        {cleanupFailedSummaries.length > 0 ? (
          <div className="space-y-2">
            <div className="text-xs font-medium uppercase tracking-(--tracking-caps) text-muted-foreground">
              {t("projectWorkspacesContent.cleanupAttentionNeeded", { defaultValue: "Cleanup attention needed" })}
            </div>
            <div className="space-y-3">
