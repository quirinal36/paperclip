        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <CardDescription className="text-xs mt-0.5">
              ...
              {t("providerQuotaCard.units.in", { defaultValue: "in" })}
              {" · "}
              <span className="font-mono">{formatTokens(totalOutputTokens)}</span>{" "}
              {t("providerQuotaCard.units.out", { defaultValue: "out" })}
              {(totalApiRuns > 0 || totalSubRuns > 0) && (
                <span className="ml-1.5">
                  ·{" "}
                  {totalApiRuns > 0 &&
                    t("providerQuotaCard.runs.apiCount", {
                      defaultValue: "~{{num}} api",
                      num: totalApiRuns,
                    })}
                  {totalApiRuns > 0 && totalSubRuns > 0 && " / "}
                  {totalSubRuns > 0 &&
                    t("providerQuotaCard.runs.subCount", {
                      defaultValue: "~{{num}} sub",
                      num: totalSubRuns,
                    })}
                  {t("providerQuotaCard.runs.suffix", { defaultValue: " runs" })}
                </span>
              )}
            </CardDescription>
          </div>
      ...
      <CardContent className="px-4 pb-4 pt-3 space-y-4">
        {hasBudget && (
          <div className="space-y-3">
            <QuotaBar
              label={t("providerQuotaCard.budget.periodSpend", { defaultValue: "Period spend" })}
              percentUsed={budgetPct}
              leftLabel={formatCents(totalCostCents)}
              rightLabel={t("providerQuotaCard.budget.allocationPct", {
            ...
            />
            <QuotaBar
              label={t("providerQuotaCard.budget.thisWeek", { defaultValue: "This week" })}
              percentUsed={weekPct}
              leftLabel={formatCents(weekSpendCents)}
              rightLabel={t("providerQuotaCard.budget.perWeek", {
                defaultValue: "~{{amount}} / wk",
          ...
          <>
            ...
            <div className="space-y-2">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                {t("providerQuotaCard.sections.rollingWindows", { defaultValue: "Rolling windows" })}
              </p>
                    ...
                    <div key={w} className="space-y-1">
                      <div className="flex items-center justify-between gap-2 text-xs">
                        ...
                        <span className="text-muted-foreground font-mono flex-1">
                          {t("providerQuotaCard.tokens.withUnit", {
        ...
        {totalSubRuns > 0 && (
          <>
            ...
            <div className="space-y-2">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                {t("providerQuotaCard.sections.subscription", { defaultValue: "Subscription" })}
              </p>
              <p className="text-xs text-muted-foreground">
                <span className="font-mono text-foreground">{totalSubRuns}</span>{" "}
                {t("providerQuotaCard.units.runs", { defaultValue: "runs" })}
                {" · "}
                {totalSubTokens > 0 && (
                  <>
                    <span className="font-mono text-foreground">{formatTokens(totalSubTokens)}</span>{" "}
                    {t("providerQuotaCard.units.total", { defaultValue: "total" })}
                    {" · "}
                  </>
                )}
                <span className="font-mono text-foreground">{formatTokens(totalSubInputTokens)}</span>{" "}
                {t("providerQuotaCard.units.in", { defaultValue: "in" })}
                {" · "}
                <span className="font-mono text-foreground">{formatTokens(totalSubOutputTokens)}</span>{" "}
                {t("providerQuotaCard.units.out", { defaultValue: "out" })}
              </p>
              {subSharePct > 0 && (
                <>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {t("providerQuotaCard.subscription.sharePct", {
                      defaultValue: "{{percent}}% of token usage via subscription",
                  ...
                  <div key={`${row.provider}:${row.model}`} className="space-y-1.5">
                    ...
                    <div className="relative h-2 w-full border border-border overflow-hidden">
                      <div
                        ...
                        title={t("providerQuotaCard.tokenShare.tokensTooltip", {
                          defaultValue: "{{percent}}% of provider tokens",
                      ...
                      <div
                        ...
                        style={{ width: `${costPct}%` }}
                        title={t("providerQuotaCard.tokenShare.costTooltip", {
                          defaultValue: "{{percent}}% of provider cost",
          ...
          <>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between gap-3">
                <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                  {t("providerQuotaCard.sections.subscriptionQuota", { defaultValue: "Subscription quota" })}
                </p>
                      ...
                      return (
                        <div key={qw.label} className="space-y-1">
                          <div className="flex items-center justify-between gap-2 text-xs">
                            ...
                            {qw.valueLabel != null ? (
                              ...
                              <span className="font-medium tabular-nums">
                                {t("providerQuotaCard.quota.percentUsed", {
                          ...
                          {qw.detail ? (
                            ...
                            <p className="text-xs text-muted-foreground">
                              {t("providerQuotaCard.quota.resetsAt", {
                                defaultValue: "resets {{date}}",
                                date: new Date(qw.resetsAt).toLocaleDateString(undefined, {
                                  month: "short",
                                  day: "numeric",
                                }),
                              })}
