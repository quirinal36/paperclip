...
export function ReportsToPicker({
  ...
  const resolvedChooseLabel =
    chooseLabel ?? t("reportsToPicker.chooseLabel", { defaultValue: "Reports to..." });
        ...
        >
          {unknownManager ? (
            ...
            <>
              ...
              >
                {`${t("reportsToPicker.reportsTo", { defaultValue: "Reports to {{name}}", name: current.name })}${terminatedManager ? t("reportsToPicker.terminatedSuffix", { defaultValue: " (terminated)" }) : ""}`}
              </span>
            </>
          ) : (
            <>
      ...
      <PopoverContent className="w-48 p-1" align="start">
        ...
        {unknownManager && (
          <div className="px-2 py-1.5 text-xs text-muted-foreground border-b border-border mb-0.5">
            {t("reportsToPicker.staleManagerHint", { defaultValue: "Saved manager is missing from this company. Choose a new manager or clear." })}
          </div>
        )}
        {rows.map((a) => (
          <button
