      ...
      <div className="mx-auto max-w-2xl space-y-4 px-4 py-10">
        <div>
          <h1 className="text-lg font-semibold">
            {t("routeErrorBoundary.error.title", { defaultValue: "This page hit an error" })}
          </h1>
          <p className="mt-1 text-sm text-muted-foreground">
            {t("routeErrorBoundary.error.description", {
              defaultValue:
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={this.props.onReset}>
            {t("routeErrorBoundary.actions.goBack", { defaultValue: "Go back" })}
          </Button>
          <Button size="sm" onClick={() => window.location.reload()}>
            {t("routeErrorBoundary.actions.reloadPage", { defaultValue: "Reload page" })}
          </Button>
        </div>
      </div>
    );
