      ...
      <DialogContent className="flex h-(--sz-calc-18) max-h-(--sz-calc-18) max-w-xl flex-col gap-0 overflow-hidden p-0 sm:h-auto sm:max-h-(--sz-calc-20)">
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto overscroll-contain px-6 py-4">
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-1.5">
              <Label className="text-xs">{t("routineRunVariablesDialog.fields.agentLabel", { defaultValue: "Agent *" })}</Label>
              <InlineEntitySelector
                value={selection.assigneeAgentId}
                options={assigneeOptions}
                recentOptionIds={recentAssigneeIds}
                placeholder={t("routineRunVariablesDialog.assignee.placeholder", { defaultValue: "Agent" })}
                noneLabel={t("routineRunVariablesDialog.assignee.selectPlaceholder", { defaultValue: "Select an agent" })}
                searchPlaceholder={t("routineRunVariablesDialog.assignee.searchPlaceholder", { defaultValue: "Search agents..." })}
                emptyMessage={t("routineRunVariablesDialog.assignee.emptyMessage", { defaultValue: "No agents found." })}
                disablePortal
                openOnFocus={false}
                ...
                renderTriggerValue={(option) =>
                  option ? (
                    ...
                    <span className="text-muted-foreground">{t("routineRunVariablesDialog.assignee.selectPlaceholder", { defaultValue: "Select an agent" })}</span>
                  )
                }
                renderOption={(option) => {
            ...
            <div className="space-y-1.5">
              <Label className="text-xs">{t("routineRunVariablesDialog.fields.projectLabel", { defaultValue: "Project" })}</Label>
              <InlineEntitySelector
                value={selection.projectId}
                options={projectOptions}
                recentOptionIds={recentProjectIds}
                placeholder={t("routineRunVariablesDialog.project.placeholder", { defaultValue: "Project" })}
                noneLabel={t("routineRunVariablesDialog.project.noneLabel", { defaultValue: "No project" })}
                searchPlaceholder={t("routineRunVariablesDialog.project.searchPlaceholder", { defaultValue: "Search projects..." })}
                emptyMessage={t("routineRunVariablesDialog.project.emptyMessage", { defaultValue: "No projects found." })}
                disablePortal
                openOnFocus={false}
                onChange={(projectId) => {
                ...
                renderTriggerValue={(option) =>
                  option && selectedProject ? (
                    ...
                    <span className="text-muted-foreground">{t("routineRunVariablesDialog.project.noneLabel", { defaultValue: "No project" })}</span>
                  )
                }
                renderOption={(option) => {
                  if (!option.id) return <span className="truncate">{option.label}</span>;
            ...
            <div key={variable.name} className="space-y-1.5">
              ...
              {isAutoWorkspaceBranchVariable(variable) ? (
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="__unset__">{t("routineRunVariablesDialog.booleanValue.noValue", { defaultValue: "No value" })}</SelectItem>
                    <SelectItem value="true">{t("routineRunVariablesDialog.booleanValue.trueLabel", { defaultValue: "True" })}</SelectItem>
                    <SelectItem value="false">{t("routineRunVariablesDialog.booleanValue.falseLabel", { defaultValue: "False" })}</SelectItem>
                  </SelectContent>
                </Select>
                ...
                >
                  <SelectTrigger>
                    <SelectValue placeholder={t("routineRunVariablesDialog.selectValue.placeholder", { defaultValue: "Choose a value" })} />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="__unset__">{t("routineRunVariablesDialog.selectValue.noValue", { defaultValue: "No value" })}</SelectItem>
        ...
        >
          {!selection.assigneeAgentId ? (
            <p className="mr-auto text-xs text-amber-600">{t("routineRunVariablesDialog.errors.agentRequired", { defaultValue: "Default agent required for this run." })}</p>
          ) : missingRequired.length > 0 ? (
            <p className="mr-auto text-xs text-amber-600">
              {t("routineRunVariablesDialog.errors.missing", { defaultValue: "Missing: {{items}}", items: missingRequired.join(", ") })}
            </p>
          ) : workspaceSelectionEnabled && !workspaceConfigValid ? (
            <p className="mr-auto text-xs text-amber-600">
              {t("routineRunVariablesDialog.errors.chooseWorkspace", { defaultValue: "Choose an existing workspace before running." })}
            </p>
          ) : (
            <span className="mr-auto" />
          )}
          <Button variant="ghost" onClick={() => onOpenChange(false)} disabled={isPending}>
            {t("routineRunVariablesDialog.actions.cancel", { defaultValue: "Cancel" })}
          </Button>
          <Button
            onClick={() => {
          ...
          >
            {isPending
              ? t("routineRunVariablesDialog.actions.running", { defaultValue: "Running..." })
              : t("routineRunVariablesDialog.actions.run", { defaultValue: "Run routine" })}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
