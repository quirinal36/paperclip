        ...
        {!rail ? (
          <>
            <Button
              ...
              aria-label={t("sidebar.actions.openSearch", { defaultValue: "Open search" })}
              title={t("sidebar.actions.openSearch", { defaultValue: "Open search" })}
            >
              <NavLink to="/search">
            ...
            {!isMobile && !collapseLocked ? (
              peeking ? (
                <Button
                  ...
                  size="icon-sm"
                  className="text-muted-foreground shrink-0"
                  aria-label={t("sidebar.actions.keepExpanded", { defaultValue: "Keep sidebar expanded" })}
                  title={t("sidebar.actions.keepExpanded", { defaultValue: "Keep sidebar expanded" })}
                ...
                <Button
                  ...
                  aria-label={collapsed ? t("sidebar.actions.expand", { defaultValue: "Expand sidebar" }) : t("sidebar.actions.collapse", { defaultValue: "Collapse sidebar" })}
                  title={collapsed ? t("sidebar.actions.expand", { defaultValue: "Expand sidebar" }) : t("sidebar.actions.collapse", { defaultValue: "Collapse sidebar" })}
      ...
      <nav className="flex-1 min-h-0 overflow-y-auto scrollbar-auto-hide flex flex-col gap-4 pointer-coarse:gap-3 px-3 py-2">
        <div className="flex flex-col gap-0.5">
          ...
          {(() => {
            const newTaskButton = (
              <button
                ...
                aria-label={rail ? t("sidebar.actions.newTask", { defaultValue: "New Task" }) : undefined}
              ...
              >
                ...
                <span className={rail ? SIDEBAR_RAIL_HIDDEN_LABEL : "truncate"}>{t("sidebar.actions.newTask", { defaultValue: "New Task" })}</span>
              </button>
            );
            return rail ? (
              <Tooltip>
                <TooltipTrigger asChild>{newTaskButton}</TooltipTrigger>
                <TooltipContent side="right">{t("sidebar.actions.newTask", { defaultValue: "New Task" })}</TooltipContent>
              </Tooltip>
            ) : (
              newTaskButton
            );
          ...
          <SidebarNavItem
            ...
            badgeLabel={t("sidebar.badges.unread", { defaultValue: "unread" })}
          ...
          {showDecisions ? (
            <SidebarNavItem
              ...
              badgeLabel={t("sidebar.badges.decisions", { defaultValue: "decisions" })}
            />
          ) : null}
        ...
        <SidebarSection label={t("sidebar.sections.work", { defaultValue: "Work" })} collapsible={{ open: workOpen, onOpenChange: setWorkOpen }}>
          <SidebarNavItem to="/issues" label={t("sidebar.nav.tasks", { defaultValue: "Tasks" })} icon={CircleDot} />
          {showCases ? (
            <SidebarNavItem to="/cases" label={t("sidebar.nav.cases", { defaultValue: "Cases" })} icon={Layers} textBadge={t("sidebar.badges.beta", { defaultValue: "beta" })} />
          ) : null}
        ...
        <SidebarSection label={t("sidebar.sections.company", { defaultValue: "Company" })} collapsible={{ open: companyOpen, onOpenChange: setCompanyOpen }}>
          <SidebarNavItem to="/org" label={t("sidebar.nav.org", { defaultValue: "Org" })} icon={Network} />
          {showApps ? <SidebarNavItem to="/apps" label={t("sidebar.nav.apps", { defaultValue: "Apps" })} icon={AppWindow} /> : null}
