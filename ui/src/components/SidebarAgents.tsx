...
const AGENT_SORT_CHOICES: SidebarSectionRadioChoice[] = [
  { value: "top", label: t("sidebarAgents.sort.top", { defaultValue: "Top" }) },
  { value: "alphabetical", label: t("sidebarAgents.sort.alphabetical", { defaultValue: "Alphabetical" }) },
  { value: "recent", label: t("sidebarAgents.sort.recent", { defaultValue: "Recent" }) },
];
...
function SidebarAgentItem({
  ...
  const pauseResumeLabel = isPaused
    ? t("sidebarAgents.agentItem.resume", { defaultValue: "Resume agent" })
    : t("sidebarAgents.agentItem.pause", { defaultValue: "Pause agent" });
  ...
  const pauseResumeDisabledLabel = disabled
    ? t("sidebarAgents.agentItem.updating", { defaultValue: "Updating..." })
    : isBudgetPaused
      ? t("sidebarAgents.agentItem.budgetPaused", { defaultValue: "Budget paused" })
      : isPaused && hasInvalidOrgChain
        ? t("sidebarAgents.agentItem.invalidOrgChain", { defaultValue: "Invalid org chain" })
      : pauseResumeLabel;
  const showBuiltInLifecycle = builtInStatus === "needs_setup" || builtInStatus === "pending_approval";
  const trailingLabel = [
    showBuiltInLifecycle
      ? t("sidebarAgents.agentItem.builtInStatus", {
          defaultValue: "Built-in agent {{status}}",
          status: builtInStatus.replace(/_/g, " "),
        })
      : null,
    hasInvalidOrgChain
      ? t("sidebarAgents.agentItem.invalidReportingChain", { defaultValue: "Invalid reporting chain" })
      : null,
    ...
    <SidebarNavItem
          ...
          <span className="ml-1 flex shrink-0 items-center gap-1">
            ...
            {hasInvalidOrgChain ? (
              <AlertTriangle
                ...
                aria-label={t("sidebarAgents.agentItem.invalidReportingChain", { defaultValue: "Invalid reporting chain" })}
              />
            ) : null}
          </span>
        ) : undefined
      ...
      liveAccessory={
        agent.pauseReason === "budget" ? (
          <BudgetSidebarMarker title={t("sidebarAgents.agentItem.pausedByBudget", { defaultValue: "Agent paused by budget" })} />
        ) : undefined
      }
    />
  );
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            ...
            aria-label={t("sidebarAgents.agentItem.openActions", { defaultValue: "Open actions for {{name}}", name: agent.name })}
          >
        ...
        <DropdownMenuContent align="end" className="w-48">
          {onToggleStar ? (
            <>
              ...
              >
                ...
                <span>
                  {starred
                    ? t("sidebarAgents.agentItem.removeFromStarred", { defaultValue: "Remove from starred" })
                    : t("sidebarAgents.agentItem.starAgent", { defaultValue: "Star agent" })}
                </span>
              </DropdownMenuItem>
              <DropdownMenuSeparator />
            </>
          ...
          <DropdownMenuItem asChild>
            ...
            >
              ...
              <span>{t("sidebarAgents.agentItem.editAgent", { defaultValue: "Edit agent" })}</span>
            </Link>
          </DropdownMenuItem>
          <DropdownMenuSeparator />
          <DropdownMenuItem
            ...
            title={isBudgetPaused ? t("sidebarAgents.agentItem.budgetPausedTooltip", { defaultValue: "Agent was paused by budget limits" }) : undefined}
          >
          ...
          >
            ...
            <span>
              {leaving
                ? t("sidebarAgents.agentItem.leaving", { defaultValue: "Leaving..." })
                : t("sidebarAgents.agentItem.leaveAgent", { defaultValue: "Leave agent" })}
            </span>
          </DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
...
export function SidebarAgents({ streamlined = false }: { streamlined?: boolean } = {}) {
  ...
  const pauseResumeAgent = useMutation({
    ...
    onSuccess: async (_agent, { agent, action }) => {
      ...
      pushToast({
        title:
          action === "pause"
            ? t("sidebarAgents.toast.paused", { defaultValue: "Agent paused" })
            : t("sidebarAgents.toast.resumed", { defaultValue: "Agent resumed" }),
        body: agent.name,
        tone: "success",
      });
    },
    onError: (error, { agent, action }) => {
      pushToast({
        title:
          action === "pause"
            ? t("sidebarAgents.toast.pauseError", { defaultValue: "Could not pause agent" })
            : t("sidebarAgents.toast.resumeError", { defaultValue: "Could not resume agent" }),
  ...
  return (
    <SidebarSection
      label={t("sidebarAgents.section.label", { defaultValue: "Agents" })}
      collapsible={{ open, onOpenChange: setOpen }}
      headerAction={{
        ariaLabel: t("sidebarAgents.actions.newAgent", { defaultValue: "New agent" }),
        icon: Plus,
        onClick: openNewAgent,
      }}
      menu={{
        ariaLabel: t("sidebarAgents.section.menuAriaLabel", { defaultValue: "Agents section actions" }),
        actions: [
          { type: "item", label: t("sidebarAgents.actions.browseAgents", { defaultValue: "Browse agents" }), icon: Users, href: "/agents/all" },
          { type: "separator" },
        ],
        radioLabel: t("sidebarAgents.sort.label", { defaultValue: "Agent sort" }),
      ...
      {showSeeAllLink && (() => {
        ...
        const seeAllLink = (
          <Link
            ...
            aria-label={rail ? t("sidebarAgents.seeAllAgents", { defaultValue: "See all agents" }) : undefined}
            onClick={() => {
              if (isMobile) setSidebarOpen(false);
            }}
          ...
          >
            ...
            <span className={rail ? SIDEBAR_RAIL_HIDDEN_LABEL : undefined}>{t("sidebarAgents.seeAllAgents", { defaultValue: "See all agents" })}</span>
          </Link>
        );
        return rail ? (
          <Tooltip>
            <TooltipTrigger asChild>{seeAllLink}</TooltipTrigger>
            <TooltipContent side="right">{t("sidebarAgents.seeAllAgents", { defaultValue: "See all agents" })}</TooltipContent>
          </Tooltip>
        ) : (
          seeAllLink
        );
