...
function revisionLabel(revision: SummarySlotRevision) {
  return t("summarySlotCard.revision.label", {
...
function latestRevisionOptionLabel(
  ...
  return `${t("summarySlotCard.revision.latestOption", {
    defaultValue: "Latest (Rev {{number}})",
...
export function resolveGenerationStatusLine(status: LiveGenerationStatus | null): string | null {
  ...
  if (status.currentToolName) {
    return t("summarySlotCard.generation.workingWith", {
...
export function SummarySlotCard({
    ...
    onError: (error) => {
      setActionError(
        error instanceof Error
          ? error.message
          : t("summarySlotCard.errors.generateFailed", {
  ...
  const latestSelectLabel = latestDocument
    ...
    : t("summarySlotCard.revision.latest", { defaultValue: "Latest" });
  ...
  return (
    <section className={cn("space-y-4", className)}>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {isGenerating ? (
              <Badge variant="secondary">
                {t("summarySlotCard.badges.generating", { defaultValue: "Generating" })}
              </Badge>
            ) : null}
            {displayingHistoricalRevision ? (
              <Badge variant="outline">
                {t("summarySlotCard.badges.historicalRevision", { defaultValue: "Historical revision" })}
              </Badge>
            ) : null}
            {latestDocument && !displayingHistoricalRevision ? (
              <Badge variant="outline">
                {t("summarySlotCard.badges.latestRevision", { defaultValue: "Latest revision" })}
              </Badge>
            ) : null}
          </div>
        ...
        <div className="flex shrink-0 flex-wrap items-center gap-2">
          {displayingHistoricalRevision ? (
            ...
            >
              {t("summarySlotCard.actions.latest", { defaultValue: "Latest" })}
            </Button>
          ) : null}
          {latestDocument && !generationFailed ? (
            ...
            >
              ...
              {t("summarySlotCard.actions.refresh", { defaultValue: "Refresh" })}
            </Button>
          ) : null}
        </div>
      </div>
          ...
          <div className="flex flex-col items-start gap-3 rounded-lg border border-border bg-muted/30 p-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-1 text-sm">
                <p className="font-medium text-foreground">
                  {needsSetup.status === "pending_approval"
                    ? t("summarySlotCard.setup.pendingApprovalTitle", {
                        defaultValue: "Summarizer setup is pending approval",
                      })
                    : t("summarySlotCard.setup.title", { defaultValue: "Set up the Summarizer" })}
                </p>
            ...
            {needsSetup.status === "pending_approval" ? null : (
              <Button type="button" size="sm" onClick={() => setConfigureOpen(true)}>
                {t("summarySlotCard.setup.action", { defaultValue: "Set up Summarizer" })}
              </Button>
            )}
          </div>
          {companyId ? (
      ...
      {!needsSetup && summarizerState?.status === "paused" && summarizerState.agent ? (
        <InlineBanner
          tone="warning"
          title={t("summarySlotCard.paused.title", { defaultValue: "Summarizer is paused" })}
          actions={
            <Button
              type="button"
              size="sm"
            ...
            >
              {resumeSummarizer.isPending
                ? t("summarySlotCard.paused.resuming", { defaultValue: "Resuming..." })
                : t("summarySlotCard.paused.resume", { defaultValue: "Resume agent" })}
            </Button>
          }
        >
          {t("summarySlotCard.paused.body", {
            defaultValue:
      ...
      {actionError ? (
        <InlineBanner
          tone="warning"
          title={t("summarySlotCard.errors.requestFailedTitle", { defaultValue: "Summary request failed" })}
        >
          {actionError}
        </InlineBanner>
      ) : null}
      ...
      {slotQuery.isError ? (
        <InlineBanner
          tone="warning"
          title={t("summarySlotCard.errors.loadFailedTitle", { defaultValue: "Summary could not be loaded" })}
          actions={
        ...
        >
          {slotQuery.error instanceof Error
            ...
            : t("summarySlotCard.errors.loadFailedBody", { defaultValue: "Try loading the summary again." })}
        </InlineBanner>
      ) : null}
      ...
      {!slotQuery.isError && generationFailed ? (
        <InlineBanner
          tone="danger"
          title={t("summarySlotCard.errors.generationFailedTitle", { defaultValue: "Summary generation failed" })}
          actions={
            <Button
              type="button"
              size="sm"
            ...
            >
              {generateMutation.isPending
                ? t("summarySlotCard.actions.retrying", { defaultValue: "Retrying..." })
                : t("summarySlotCard.actions.retry", { defaultValue: "Retry" })}
            </Button>
          }
        >
          {slotQuery.data?.slot?.failureReason
            ?? t("summarySlotCard.errors.generationFailedBody", {
              defaultValue: "The generation task ended before writing a summary.",
            })}
        </InlineBanner>
      ) : null}
      ...
      {!slotQuery.isError && isGenerating && generatingIssue ? (
        <div className="flex items-start gap-3 text-sm">
          ...
          <div className="min-w-0 space-y-1">
            <p className="font-medium text-foreground">
              {t("summarySlotCard.generation.title", { defaultValue: "Generating summary" })}
            </p>
            ...
            <p className="text-muted-foreground">
              {t("summarySlotCard.generation.workingIn", { defaultValue: "Summarizer is working in" })}{" "}
      ...
      {!slotQuery.isError && !latestDocument && !isGenerating && !generationFailed && canGenerateFirstSummary ? (
        <div className="flex flex-col items-start gap-3 rounded-lg border border-border bg-muted/30 p-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-1 text-sm">
            <p className="font-medium text-foreground">
              {t("summarySlotCard.empty.title", { defaultValue: "No summary yet" })}
            </p>
            <p className="text-muted-foreground">
              {t("summarySlotCard.empty.description", {
                defaultValue: "Generate a concise status snapshot for this surface.",
              })}
            </p>
          </div>
          <Button
          ...
          >
            ...
            {generateMutation.isPending
              ? t("summarySlotCard.actions.generating", { defaultValue: "Generating..." })
              : t("summarySlotCard.actions.generate", { defaultValue: "Generate summary" })}
          </Button>
        </div>
      ) : null}
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-col gap-3 text-xs text-muted-foreground sm:flex-row sm:items-center sm:justify-between">
            <span title={formatDateTime(historicalRevision?.createdAt ?? latestRevision?.createdAt ?? latestDocument.updatedAt)}>
              {t("summarySlotCard.meta.updated", {
                defaultValue: "Updated {{time}}",
                time: relativeTime(historicalRevision?.createdAt ?? latestRevision?.createdAt ?? latestDocument.updatedAt),
              })}
            </span>
            ...
            {revisions.length > 1 ? (
              ...
              >
                <SelectTrigger
                  ...
                  aria-label={t("summarySlotCard.revision.selectLabel", { defaultValue: "Select summary revision" })}
                  ...
                  <SelectValue>
                    ...
                    <span>
                      {t("summarySlotCard.revision.count", {
                        defaultValue: "{{revisionCount}} revisions",
