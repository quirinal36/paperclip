...
export function ThemeToggle({ className, variant = "icon", onAfterToggle }: ThemeToggleProps) {
  ...
  const label = isDark
    ? t("theme.toggle.toLight", { defaultValue: "Switch to light mode" })
    : t("theme.toggle.toDark", { defaultValue: "Switch to dark mode" });
  const description = t("theme.toggle.description", { defaultValue: "Toggle the app appearance." });
