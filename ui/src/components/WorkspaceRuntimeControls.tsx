...
function buildJobItem(
  ...
  return {
    ...
    title: command.name,
    kind: "job",
    statusLabel: t("workspaceRuntimeControls.status.runOnce", { defaultValue: "run once" }),
    lifecycle: null,
    healthStatus: null,
    ...
    serviceIndex: null,
    disabledReason: command.disabledReason ?? (!command.command ? t("workspaceRuntimeControls.disabledReason.jobMissingCommand", { defaultValue: "This job is missing a command." }) : null),
  };
}
...
export function buildWorkspaceRuntimeControlSections(input: {
  ...
  const otherServices = runtimeServices
    ...
    .map((runtimeService) => ({
      ...
      disabledReason: t("workspaceRuntimeControls.disabledReason.untrackedService", { defaultValue: "This runtime service no longer matches a configured workspace command." }),
    }));
...
export function buildWorkspaceServiceControlEntries(input: {
  ...
  return [...input.sections.services, ...input.sections.otherServices].map((item) => {
    ...
    const failureDetail = state === "failed"
      ? `${t("workspaceRuntimeControls.status.serviceFailed", { defaultValue: "Service failed" })}${runtimeService?.stoppedAt ? ` · ${timeAgo(runtimeService.stoppedAt)}` : ""}`
      : null;
    ...
    <div className={cn("flex gap-2", iconOnly ? "w-auto flex-row flex-wrap" : "w-full flex-col sm:w-auto sm:flex-row sm:flex-wrap")}>
      {actions.map((action) => {
        ...
        const label = action === "run"
          ? t("workspaceRuntimeControls.actions.run", { defaultValue: "Run" })
          : action === "start"
            ? t("workspaceRuntimeControls.actions.start", { defaultValue: "Start" })
            : action === "stop"
              ? t("workspaceRuntimeControls.actions.stop", { defaultValue: "Stop" })
              : t("workspaceRuntimeControls.actions.restart", { defaultValue: "Restart" });
            ...
            <div key={item.key} className={cn("border border-border/80 bg-background px-3 py-3", square ? "rounded-none" : "rounded-xl")}>
              <div className="flex flex-col gap-3">
                ...
                <div className="space-y-1 text-xs text-muted-foreground">
                  ...
                  {item.port ? <div>{t("workspaceRuntimeControls.fields.port", { defaultValue: "Port {{port}}", port: item.port })}</div> : null}
...
export function WorkspaceRuntimeControls({
  ...
  const resolvedServiceEmptyMessage = emptyMessage ?? serviceEmptyMessage ?? t("workspaceRuntimeControls.empty.services", { defaultValue: "No services are configured for this workspace." });
  const resolvedJobEmptyMessage = jobEmptyMessage ?? t("workspaceRuntimeControls.empty.jobs", { defaultValue: "No one-shot jobs are configured for this workspace." });
  ...
  return (
    <div className={cn("space-y-4", className)}>
      <div className={cn("border border-border/70 bg-background p-3", square ? "rounded-none" : "rounded-xl")}>
        <div className="space-y-1">
          <div className="text-xs font-medium uppercase tracking-(--tracking-eyebrow) text-muted-foreground">{t("workspaceRuntimeControls.header.title", { defaultValue: "Workspace commands" })}</div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              ...
              {runningCount > 0
                ? t("workspaceRuntimeControls.summary.servicesRunning", { defaultValue: "{{runningCount}} services running", runningCount })
                : t("workspaceRuntimeControls.summary.noServicesRunning", { defaultValue: "No services running" })}
            </Badge>
            <span className="text-xs text-muted-foreground">
              {resolvedSections.jobs.length > 0
                ? (resolvedSections.jobs.length === 1
                    ? t("workspaceRuntimeControls.summary.jobsAvailableOne", { defaultValue: "{{jobCount}} job available to run on demand.", jobCount: resolvedSections.jobs.length })
                    : t("workspaceRuntimeControls.summary.jobsAvailableOther", { defaultValue: "{{jobCount}} jobs available to run on demand.", jobCount: resolvedSections.jobs.length }))
                : t("workspaceRuntimeControls.summary.controlledIndependently", { defaultValue: "Each command can be controlled independently." })}
            </span>
          </div>
      ...
      <CommandSection
        title={t("workspaceRuntimeControls.services.title", { defaultValue: "Services" })}
        description={t("workspaceRuntimeControls.services.description", { defaultValue: "Long-running commands that Paperclip can supervise for this workspace." })}
      ...
      <CommandSection
        title={t("workspaceRuntimeControls.jobs.title", { defaultValue: "Jobs" })}
        description={t("workspaceRuntimeControls.jobs.description", { defaultValue: "One-shot commands that run now and exit when they finish." })}
      ...
      {resolvedSections.otherServices.length > 0 ? (
        <CommandSection
          title={t("workspaceRuntimeControls.untracked.title", { defaultValue: "Untracked services" })}
          description={t("workspaceRuntimeControls.untracked.description", { defaultValue: "Running services that no longer match the current workspace command config." })}
