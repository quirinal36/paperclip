...
function statusMeta(entry: WorkspaceServiceControlEntry): { label: string; unhealthy: boolean } {
  switch (entry.state) {
    case "starting":
      return { label: t("workspaceServiceControlBar.status.starting", { defaultValue: "Starting…" }), unhealthy: false };
    case "stopping":
      return { label: t("workspaceServiceControlBar.status.stopping", { defaultValue: "Stopping…" }), unhealthy: false };
    case "restarting":
      return { label: t("workspaceServiceControlBar.status.restarting", { defaultValue: "Restarting…" }), unhealthy: false };
    case "failed":
      return { label: t("workspaceServiceControlBar.status.failed", { defaultValue: "Failed" }), unhealthy: false };
    case "running":
      return entry.healthStatus === "unhealthy"
        ? { label: t("workspaceServiceControlBar.status.unhealthy", { defaultValue: "Unhealthy" }), unhealthy: true }
        : { label: t("workspaceServiceControlBar.status.running", { defaultValue: "Running" }), unhealthy: false };
    default:
      return { label: t("workspaceServiceControlBar.status.stopped", { defaultValue: "Stopped" }), unhealthy: false };
  }
}
...
function CopyUrlButton({ url, disabled }: { url: string; disabled?: boolean }) {
  ...
  const copyLabel =
    copyState === "copied"
      ? t("workspaceServiceControlBar.copyUrl.copied", { defaultValue: "URL copied" })
      : copyState === "failed"
        ? t("workspaceServiceControlBar.copyUrl.failed", { defaultValue: "Copy failed" })
        : t("workspaceServiceControlBar.copyUrl.label", { defaultValue: "Copy URL" });
  return (
    <Button
      variant="ghost"
      size="icon-xs"
...
function UrlSegment({ entry, compact }: { entry: WorkspaceServiceControlEntry; compact?: boolean }) {
  ...
  if (!displayUrl) {
    return <span className="font-mono text-xs text-muted-foreground/70">{t("workspaceServiceControlBar.url.none", { defaultValue: "no url" })}</span>;
  }
  return (
    <>
      {live ? (
      ...
      <span className={cn("flex items-center", live ? null : "invisible")} aria-hidden={live ? undefined : true}>
        ...
        <Button
          ...
          disabled={!live}
          className="text-muted-foreground hover:text-foreground"
          title={t("workspaceServiceControlBar.actions.openInNewTab", { defaultValue: "Open in new tab" })}
        >
          {live ? (
            <a href={entry.url ?? undefined} target="_blank" rel="noreferrer" aria-label={t("workspaceServiceControlBar.actions.openInNewTab", { defaultValue: "Open in new tab" })}>
  ...
  if (entry.state === "stopped") {
    return (
      <Button
        ...
        aria-label={t("workspaceServiceControlBar.actions.start", { defaultValue: "Start" })}
        title={t("workspaceServiceControlBar.actions.start", { defaultValue: "Start" })}
      >
        <Play className="size-3" />
        {t("workspaceServiceControlBar.actions.start", { defaultValue: "Start" })}
      </Button>
    );
  }
    ...
    return (
      <>
        ...
        <Button
          ...
          aria-label={t("workspaceServiceControlBar.actions.restart", { defaultValue: "Restart" })}
          title={t("workspaceServiceControlBar.actions.restart", { defaultValue: "Restart" })}
  ...
  return (
    <>
      <Button
        ...
        aria-label={t("workspaceServiceControlBar.actions.stop", { defaultValue: "Stop" })}
        title={t("workspaceServiceControlBar.actions.stop", { defaultValue: "Stop" })}
        className="border border-border text-foreground"
      >
    ...
    <div className="flex items-center gap-1 text-xs text-muted-foreground">
      ...
      {onViewLogs ? (
        <>
          ...
          >
            {t("workspaceServiceControlBar.actions.viewLogs", { defaultValue: "View logs" })}
          </button>
        </>
      ) : null}
    </div>
...
function ServicePopoverRow({
  ...
  const secondary = live
    ...
    : entry.state === "starting" && entry.port
      ? t("workspaceServiceControlBar.popover.startingOnPort", { defaultValue: "starting on :{{port}}…", port: entry.port })
        ...
        <div className="flex h-9 items-center pl-3 pr-1.5">
          <Popover open={open} onOpenChange={setOpen}>
            <PopoverTrigger asChild>
              <button
                ...
                aria-label={t("workspaceServiceControlBar.aggregate.ariaLabel", { defaultValue: "{{running}} of {{total}} services running — show services", running: runningCount, total: services.length })}
              >
                <StatusIndicator entry={aggregateEntry} />
                <span className="whitespace-nowrap">{t("workspaceServiceControlBar.aggregate.runningCount", { defaultValue: "{{running}}/{{total}} running", running: runningCount, total: services.length })}</span>
            ...
            <PopoverContent align="end" className="w-96 p-0" onOpenAutoFocus={(event) => event.preventDefault()}>
              <div className="px-4 pb-1 pt-3 text-xs font-medium uppercase tracking-wide text-muted-foreground">
                {t("workspaceServiceControlBar.popover.servicesHeading", { defaultValue: "Services · {{count}}", count: services.length })}
              </div>
              ...
              <div className="flex items-center gap-1 border-t border-border px-4 py-2">
                <Button variant="ghost" size="xs" onClick={() => onAction("start", null)}>{t("workspaceServiceControlBar.actions.startAll", { defaultValue: "Start all" })}</Button>
                <Button variant="ghost" size="xs" onClick={() => onAction("stop", null)}>{t("workspaceServiceControlBar.actions.stopAll", { defaultValue: "Stop all" })}</Button>
                <Button variant="ghost" size="xs" onClick={() => onAction("restart", null)}>{t("workspaceServiceControlBar.actions.restartAll", { defaultValue: "Restart all" })}</Button>
                {onManageServices ? (
                  <Button
                    variant="link"
                    size="xs"
                  ...
                  >
                    {t("workspaceServiceControlBar.actions.manageServices", { defaultValue: "Manage in Services tab →" })}
                  </Button>
                ) : null}
              </div>
            </PopoverContent>
          ...
          <div className="hidden min-w-0 items-center gap-0.5 sm:flex">
            {primary ? (
              ...
              <span className="font-mono text-xs text-muted-foreground/70">{t("workspaceServiceControlBar.url.none", { defaultValue: "no url" })}</span>
            )}
          </div>
          <div className="mx-3 hidden h-5 w-px bg-border sm:block" />
          <div className="ml-auto flex items-center gap-1 pl-3 sm:pl-0">
