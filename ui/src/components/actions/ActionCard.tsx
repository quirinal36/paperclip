...
function bindingRows(binding: ActionCardBinding, isStale: boolean): BindingRow[] {
  const catalogValue = isStale && binding.previousCatalogSha256 ? (
    <span className="inline-flex flex-wrap items-center gap-1.5">
      <span className="text-muted-foreground line-through decoration-amber-500" title={t("actionCard.bindings.previousCatalogHash", { defaultValue: "Previous catalog hash" })}>
        {shortSha(binding.previousCatalogSha256)}
      </span>
      <span className="text-amber-600 dark:text-amber-400" title={t("actionCard.bindings.currentCatalogHash", { defaultValue: "Current catalog hash" })}>
  ...
  return [
    {
      label: t("actionCard.bindings.application", { defaultValue: "Application" }),
      value: (
        <span>
          {binding.application}
          <span className="ml-1.5 text-xs text-muted-foreground">{t("actionCard.bindings.manifestVersion", { defaultValue: "manifest v{{version}}", version: binding.manifestVersion })}</span>
        </span>
      ),
    },
    { label: t("actionCard.bindings.connection", { defaultValue: "Connection" }), value: binding.connection, mono: true },
    { label: t("actionCard.bindings.catalog", { defaultValue: "Catalog" }), value: catalogValue, mono: !isStale },
    {
      label: t("actionCard.bindings.payload", { defaultValue: "Payload" }),
      value: (
        <span className="inline-flex items-center gap-1.5">
          ...
          <span className="font-sans text-(length:--text-micro) uppercase tracking-normal text-muted-foreground">{t("actionCard.bindings.signed", { defaultValue: "signed" })}</span>
        </span>
      ),
      mono: true,
    },
...
export function ActionCard({
  ...
  const approveButton = (
    <Button
      ...
      title={isStale ? t("actionCard.actions.approveStaleTooltip", { defaultValue: "Re-issue the request before approving — the catalog hash changed." }) : undefined}
    >
      {t("actionCard.actions.approve", { defaultValue: "Approve" })}
    </Button>
  );
  const denyButton = (
    <Button size="sm" variant="outline" onClick={onDeny} className={mobile ? "w-full" : undefined}>
      {t("actionCard.actions.deny", { defaultValue: "Deny" })}
    </Button>
  );
  const editButton = (
    <Button size="sm" variant="outline" onClick={onEditResign} className={mobile ? "w-full" : undefined}>
      <Pencil className="mr-1 h-3.5 w-3.5" />
      {t("actionCard.actions.editResign", { defaultValue: "Edit & re-sign" })}
    </Button>
  );
    ...
    >
      <CardContent className="space-y-3 p-4">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-sm text-foreground">
              ...
              {t("actionCard.request.callLine", { defaultValue: "requested approval to call" })}
            </p>
        ...
        {isStale ? (
          <EnforcementBanner
            tone="warning"
            title={t("actionCard.stale.title", { defaultValue: "Catalog changed since this request was signed." })}
            body={t("actionCard.stale.body", { defaultValue: "The application's tool catalog hash no longer matches the one this approval was issued against. Approval is disabled — the agent must edit & re-sign to request again." })}
          />
        ) : null}
        ...
        <div className="space-y-1">
          <p className="text-xs font-medium uppercase tracking-normal text-muted-foreground">{t("actionCard.sections.input", { defaultValue: "Input" })}</p>
        ...
        {/* Why I'm asking */}
        <div className="space-y-1">
          <p className="text-xs font-medium uppercase tracking-normal text-muted-foreground">{t("actionCard.sections.whyAsking", { defaultValue: "Why I'm asking" })}</p>
          <p className="text-sm text-muted-foreground">
            {reason}
            {policyNumber != null ? (
              <>
                {" "}
                <span className="font-medium text-foreground">{t("actionCard.policy.label", { defaultValue: "Policy #{{number}}", number: policyNumber })}</span>{" "}
                {t("actionCard.policy.requiresApproval", { defaultValue: "requires approval here." })}
              </>
            ) : null}
          </p>
        </div>
      ...
      >
        <span className="flex items-center gap-1.5 text-xs text-muted-foreground">
          ...
          {expiresInLabel ?? t("actionCard.expiry.none", { defaultValue: "no expiry set" })}
        </span>
        {mobile ? (
