...
function ImagePreview({ artifact }: { artifact: CompanyArtifact }) {
  ...
  if (errored || !artifact.contentPath) {
    return <PlaceholderPreview label={t("artifactCard.preview.image", { defaultValue: "Image" })} />;
  }
  return (
    <PreviewFrame>
      <img
      ...
      <PlaceholderPreview
        label={
          artifact.source === "document"
            ? t("artifactCard.preview.document", { defaultValue: "Document" })
            : t("artifactCard.preview.text", { defaultValue: "Text" })
        }
      />
    );
  }
...
export function ArtifactPreview({ artifact }: { artifact: CompanyArtifact }) {
  ...
  switch (artifact.mediaKind) {
    ...
    case "file":
      return <PlaceholderPreview label={t("artifactCard.preview.file", { defaultValue: "File" })} />;
    case "empty":
    default:
          ...
          <div className="flex shrink-0 items-center gap-0.5 opacity-0 transition-opacity group-hover:opacity-100 focus-within:opacity-100">
            {artifact.openPath ? (
              <SecondaryAction
                ...
                title={t("artifactCard.actions.openInNewTab", { defaultValue: "Open file in new tab" })}
              >
            ...
            {artifact.downloadPath ? (
              <SecondaryAction
                ...
                download
                title={t("artifactCard.actions.download", { defaultValue: "Download file" })}
              >
