...
export function SecretPopoverForm({
  ...
  const nameError = (() => {
    if (!trimmedName)
      return touched ? t("createSecretPopover.errors.nameRequired", { defaultValue: "Name is required" }) : null;
    if (!SECRET_NAME_RE.test(trimmedName))
      return t("createSecretPopover.errors.nameFormat", { defaultValue: "Use lowercase letters, digits and _" });
    if (existingSecretNames?.some((existing) => existing.toLowerCase() === trimmedName)) {
      return t("createSecretPopover.errors.nameExists", { defaultValue: "A secret with this name already exists" });
    }
    return null;
  })();
  const valueError =
    value.length === 0
      ? touched
        ? t("createSecretPopover.errors.valueRequired", { defaultValue: "Value is required" })
        : null
      : null;
    ...
    try {
      ...
      setError(
        submitError instanceof Error
          ? submitError.message
          : t("createSecretPopover.errors.createFailed", { defaultValue: "Failed to create secret" }),
      );
      setSubmitting(false);
    }
  }
  ...
  const ctaLabel =
    mode === "create"
      ? t("createSecretPopover.actions.createAndBind", { defaultValue: "Create & bind" })
      : t("createSecretPopover.actions.storeAndBind", { defaultValue: "Store & bind" });
  const heading =
    mode === "create"
      ? t("createSecretPopover.heading.create", { defaultValue: "Create secret" })
      : t("createSecretPopover.heading.store", { defaultValue: "Store value as secret" });
    ...
    <div className="w-72 space-y-3">
      <div className="space-y-1">
        ...
        {mode === "store" ? (
          <PopoverDescription className="text-(length:--text-micro) text-muted-foreground">
            {t("createSecretPopover.storeDescription.before", {
              defaultValue: "Moves the typed value into an encrypted company secret and binds",
            })}{" "}
            <span className="font-mono">
              {initialName ||
                t("createSecretPopover.storeDescription.thisVariable", { defaultValue: "this variable" })}
            </span>
            {t("createSecretPopover.storeDescription.after", { defaultValue: " to it." })}
          </PopoverDescription>
        ) : null}
      </div>
      ...
      <label className="block space-y-1">
        <span className="text-(length:--text-micro) font-medium text-muted-foreground">
          {t("createSecretPopover.fields.name", { defaultValue: "Name" })}
        </span>
        <input
          ...
          aria-label={t("createSecretPopover.fields.nameAriaLabel", { defaultValue: "Secret name" })}
      ...
      <label className="block space-y-1">
        <span className="text-(length:--text-micro) font-medium text-muted-foreground">
          {t("createSecretPopover.fields.value", { defaultValue: "Value" })}
        </span>
        <div className="relative">
          <input
            ...
            spellCheck={false}
            placeholder={
              mode === "create" ? t("createSecretPopover.fields.valuePlaceholder", { defaultValue: "value" }) : undefined
            }
            aria-label={t("createSecretPopover.fields.valueAriaLabel", { defaultValue: "Secret value" })}
          ...
          <button
            ...
            aria-label={
              reveal
                ? t("createSecretPopover.actions.hideValue", { defaultValue: "Hide value" })
                : t("createSecretPopover.actions.showValue", { defaultValue: "Show value" })
            }
      ...
      <div className="flex items-center justify-end gap-2 pt-0.5">
        <Button type="button" variant="ghost" size="sm" onClick={onCancel} disabled={submitting}>
          {t("createSecretPopover.actions.cancel", { defaultValue: "Cancel" })}
        </Button>
