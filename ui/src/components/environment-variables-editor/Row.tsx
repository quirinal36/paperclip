...
export function EnvironmentVariableRow({
  ...
  async function submitSecretPopover(name: string, value: string) {
    ...
    onToast(t("row.toast.secretCreated", { defaultValue: "Secret {{name}} created", name: created.name }));
  ...
  const sourceLabel =
    row.source === "text"
      ? t("row.source.text.tooltip", { defaultValue: "Text value" })
      : row.source === "secret"
        ? t("row.source.secret.tooltip", { defaultValue: "Company secret reference" })
        : t("row.source.userSecret.tooltip", { defaultValue: "User secret reference" });
  ...
  const versionTagLabel = row.version === "latest" ? t("row.version.latest", { defaultValue: "latest" }) : `v${row.version}`;
    ...
    >
      ...
      <div className="col-start-1 row-start-1 min-w-0 @[40rem]/env:row-start-1 @[40rem]/env:self-start">
        <input
          ...
          placeholder={t("row.name.placeholder", { defaultValue: "KEY" })}
          value={row.name}
          spellCheck={false}
          disabled={disabled}
          aria-label={t("row.name.ariaLabel", { defaultValue: "Variable name" })}
      ...
      <div className="col-span-2 col-start-1 row-start-2 min-w-0 @[40rem]/env:col-span-1 @[40rem]/env:col-start-2 @[40rem]/env:row-start-1">
              ...
              <DropdownMenu>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <DropdownMenuTrigger asChild disabled={disabled}>
                      <button
                        type="button"
                        aria-label={t("row.source.ariaLabel", { defaultValue: "Value source" })}
                ...
                <DropdownMenuContent align="start" className="w-56">
                  <DropdownMenuItem className="flex-col items-start gap-0.5" onSelect={() => switchSource("text")}>
                    <span className="text-sm">{t("row.source.text.label", { defaultValue: "Text value" })}</span>
                    <span className="text-(length:--text-micro) text-muted-foreground">{t("row.source.text.description", { defaultValue: "Store the value inline as plain text." })}</span>
                  </DropdownMenuItem>
                  <DropdownMenuItem className="flex-col items-start gap-0.5" onSelect={() => switchSource("secret")}>
                    <span className="text-sm">{t("row.source.secret.label", { defaultValue: "Company secret" })}</span>
                    <span className="text-(length:--text-micro) text-muted-foreground">{t("row.source.secret.description", { defaultValue: "Resolve a stored company secret at run start." })}</span>
                  </DropdownMenuItem>
                  <DropdownMenuItem className="flex-col items-start gap-0.5" onSelect={() => switchSource("user_secret")}>
                    <span className="text-sm">{t("row.source.userSecret.label", { defaultValue: "User secret" })}</span>
                    <span className="text-(length:--text-micro) text-muted-foreground">
                      {t("row.source.userSecret.description", { defaultValue: "Resolve the responsible user's own value at run start." })}
                    </span>
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
              ...
              {row.source === "text" ? (
                <>
                  <input
                    ...
                    placeholder={t("row.value.placeholder", { defaultValue: "value" })}
                    value={row.textValue}
                    type={sensitive ? "password" : "text"}
                    spellCheck={false}
                    disabled={disabled}
                    aria-label={t("row.value.ariaLabel", { defaultValue: "Variable value" })}
                  ...
                  {sensitive ? (
                    <div className="flex shrink-0 items-stretch border-l border-border">
                      <button
                        ...
                        title={t("row.sensitive.storeTooltip", { defaultValue: "This value looks sensitive — store it as a secret" })}
                      >
                        <ShieldAlert className="size-3.5" />
                        <span className="hidden @[30rem]/env:inline">{t("row.sensitive.storeButton", { defaultValue: "Store as secret" })}</span>
                      </button>
                      <button
                        type="button"
                        onClick={() => onPatch({ sensitiveDismissed: true })}
                        disabled={disabled}
                        aria-label={t("row.sensitive.dismissAriaLabel", { defaultValue: "Dismiss sensitive-value suggestion" })}
                        title={t("row.sensitive.dismissTooltip", { defaultValue: "Dismiss — keep this value as plain text" })}
                    ...
                    <Popover open={versionOpen} onOpenChange={setVersionOpen}>
                      <PopoverAnchor asChild>
                        <button
                          ...
                          aria-label={t("row.version.ariaLabel", { defaultValue: "Version" })}
                          className={cn(
                      ...
                      <PopoverContent align="end" className="w-44 p-1" role="radiogroup" aria-label={t("row.version.groupAriaLabel", { defaultValue: "Secret version" })}>
                        <button
                          type="button"
                          role="radio"
                        ...
                        >
                          {t("row.version.latest", { defaultValue: "latest" })} <span className="text-(length:--text-micro) text-muted-foreground">{t("row.version.recommended", { defaultValue: "(recommended)" })}</span>
                        </button>
                ...
                <div className="grid min-w-0 flex-1 grid-cols-(--gtc-13)">
                  {userSecretsEnabled ? (
                    <select
                      aria-label={t("row.userSecret.selectAriaLabel", { defaultValue: "User secret" })}
                    ...
                    >
                      <option value="">{t("row.userSecret.selectPlaceholder", { defaultValue: "Select user secret..." })}</option>
                      {row.userSecretKey && !userSecretDefinitions?.some((definition) => definition.key === row.userSecretKey) ? (
                        <option value={row.userSecretKey}>{t("row.userSecret.unknownOption", { defaultValue: "Unknown ({{key}})", key: row.userSecretKey })}</option>
                      ) : null}
                    ...
                    <input
                      className={valueTextInputClass}
                      placeholder={t("row.userSecret.keyPlaceholder", { defaultValue: "user-secret key" })}
                      value={row.userSecretKey}
                      spellCheck={false}
                      disabled={disabled}
                      aria-label={t("row.userSecret.keyAriaLabel", { defaultValue: "User secret key" })}
                      onChange={(event) => onPatch({ userSecretKey: event.target.value })}
                    />
                  )}
                  <select
                    aria-label={t("row.requirement.ariaLabel", { defaultValue: "Requirement" })}
                  ...
                  >
                    <option value="required">{t("row.requirement.required", { defaultValue: "Required" })}</option>
                    <option value="optional">{t("row.requirement.optional", { defaultValue: "Optional" })}</option>
                  </select>
                </div>
              )}
            </div>
        ...
        {undoPrev ? (
          <p className="mt-0.5 inline-flex items-center gap-2 text-(length:--text-micro) text-muted-foreground">
            {t("row.undo.reverted", { defaultValue: "Reverted to text —" })}{" "}
            <button
              type="button"
            ...
            >
              {t("row.undo.action", { defaultValue: "Undo" })}
            </button>
          </p>
        ) : null}
      </div>
      ...
      <div className="col-start-2 row-start-1 flex items-center justify-end gap-0.5 self-start @[40rem]/env:col-start-3 @[40rem]/env:self-center">
        {row.source === "text" && !sensitive && (row.name.trim() || row.textValue) ? (
          <DropdownMenu>
            <DropdownMenuTrigger asChild disabled={disabled}>
              <button
                type="button"
                aria-label={t("row.actions.moreAriaLabel", { defaultValue: "More actions" })}
            ...
            <DropdownMenuContent align="end">
              ...
              >
                {t("row.actions.storeAsSecret", { defaultValue: "Store as secret…" })}
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        ) : null}
        <button
          ...
          onClick={onRemove}
          disabled={disabled}
          aria-label={t("row.actions.removeAriaLabel", {
            defaultValue: "Remove {{name}}",
            name: row.name.trim() || t("row.variableFallback", { defaultValue: "variable" }),
          })}
