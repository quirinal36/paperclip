...
function formatChangedNames(names: readonly string[]): string {
  ...
  return names.length > CHANGE_SUMMARY_MAX_NAMES
    ? t("index.changeSummary.more", {
        defaultValue: "{{shown}} +{{count}} more",
        shown,
...
export const EnvironmentVariablesEditor = forwardRef<EnvironmentVariablesEditorHandle, EnvironmentVariablesEditorProps>(function EnvironmentVariablesEditor({
  ...
  const changeSummaryText = useMemo(() => {
    ...
    if (changeSummary.added.length > 0)
      parts.push(t("index.changeSummary.new", { defaultValue: "New: {{names}}", names: formatChangedNames(changeSummary.added) }));
    if (changeSummary.changed.length > 0)
      parts.push(t("index.changeSummary.edited", { defaultValue: "Edited: {{names}}", names: formatChangedNames(changeSummary.changed) }));
    if (changeSummary.removed.length > 0)
      parts.push(t("index.changeSummary.removed", { defaultValue: "Removed: {{names}}", names: formatChangedNames(changeSummary.removed) }));
    return parts.join(" · ");
  }, [changeSummary, t]);
  ...
  function bulkImport(text: string, targetRowId: string): boolean {
    ...
    const importedTitle =
      pairs.length === 1
        ? t("index.import.toastOne", { defaultValue: "Imported {{count}} variable", count: pairs.length })
        : t("index.import.toastOther", { defaultValue: "Imported {{count}} variables", count: pairs.length });
    toast?.pushToast({ title: importedTitle, tone: "success" });
    return true;
  }
  ...
  const hint =
    footerHint === undefined ? t("index.hint.default", { defaultValue: DEFAULT_HINT }) : footerHint;
    ...
    <TooltipProvider>
      ...
      {hasRows ? (
        <>
          ...
          <div className="hidden gap-x-1.5 @[40rem]/env:grid @[40rem]/env:grid-cols-(--gtc-14)">
            <span className="text-(length:--text-micro) font-medium uppercase tracking-wide text-muted-foreground">{t("index.columns.name", { defaultValue: "Name" })}</span>
            <span className="text-(length:--text-micro) font-medium uppercase tracking-wide text-muted-foreground">{t("index.columns.value", { defaultValue: "Value" })}</span>
            <span />
          </div>
        ...
        <p className="text-sm text-muted-foreground">{t("index.empty", { defaultValue: "No environment variables" })}</p>
      )}
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 pt-1">
        ...
        >
          ...
          {t("index.actions.addVariable", { defaultValue: "Add variable" })}
        </button>
        ...
        {quickBind.length > 0 && !disabled ? (
          <div className="flex flex-wrap items-center gap-1.5">
            <span className="inline-flex items-center gap-1 text-(length:--text-micro) text-muted-foreground/70">
              ...
              {t("index.recentlyUsed.label", { defaultValue: "Recently used:" })}
            </span>
            {quickBind.map((secret) => (
              <button
                ...
                title={t("index.recentlyUsed.bindTooltip", { defaultValue: "Bind {{name}}", name: secret.name })}
              >
                + {secret.name}
              </button>
            ))}
        ...
        >
          <div className="flex min-w-0 flex-col gap-0.5">
            <div className="flex items-center gap-2 text-sm font-medium">
              ...
              <span>{t("index.unsavedChanges", { defaultValue: "Unsaved changes" })}</span>
            </div>
            {changeSummaryText ? (
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              {t("index.actions.revert", { defaultValue: "Revert" })}
            </button>
            <button
              type="button"
              onClick={saveDraft}
            ...
            >
              ...
              {t("index.actions.save", { defaultValue: "Save" })}
            </button>
          </div>
        </div>
      ) : null}
