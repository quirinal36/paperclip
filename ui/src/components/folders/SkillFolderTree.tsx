...
export function SkillFolderRail({
  ...
  return (
    <div className="relative hidden h-full shrink-0 md:flex" style={{ width: `${width}px` }}>
      <nav
        aria-label={t("skillFolderTree.nav.ariaLabel", { defaultValue: "Skill folders" })}
      ...
      <div className="mb-2 flex items-center justify-between gap-2 pt-0.5">
        <div className="text-(length:--text-micro) font-medium uppercase tracking-wide text-muted-foreground">
          {t("skillFolderTree.foldersHeading", { defaultValue: "Folders" })}
        </div>
        <Button
          variant="ghost"
          size="icon-sm"
          title={t("skillFolderTree.actions.newFolder", { defaultValue: "New folder" })}
      ...
      {loading ? (
        ...
        <div className="space-y-0.5">
          <VirtualRow
            ...
            label={t("skillFolderTree.allSkills", { defaultValue: "All skills" })}
            count={allCount}
          ...
          {model.my ? (
            <TreeBranch
              ...
              rootLabel={t("skillFolderTree.mySkills", { defaultValue: "My Skills" })}
            ...
            <VirtualRow
              active={false}
              label={t("skillFolderTree.mySkills", { defaultValue: "My Skills" })}
              count={0}
          ...
          {model.company.length > 0 ? (
            ...
            <div className="px-2 py-1 text-xs text-muted-foreground">
              {t("skillFolderTree.emptyCompany", { defaultValue: "No company folders yet." })}
            </div>
          )}
          ...
          {model.projects ? (
            <TreeBranch
              ...
              renamingId={renamingId}
              renameDraft={renameDraft}
              rootLabel={t("skillFolderTree.projects", { defaultValue: "Projects" })}
            ...
            <VirtualRow
              active={false}
              label={t("skillFolderTree.projects", { defaultValue: "Projects" })}
              count={0}
          ...
          {model.bundled ? (
            <TreeBranch
              ...
              renamingId={renamingId}
              renameDraft={renameDraft}
              rootLabel={t("skillFolderTree.bundled", { defaultValue: "Bundled" })}
              rootIcon={<Boxes className="h-3.5 w-3.5" />}
              onToggle={toggle}
              onSelect={onSelect}
              onCreateFolder={onCreateFolder}
          ...
          <div className="px-2 pb-1 pt-3 text-(length:--text-micro) font-medium uppercase tracking-wide text-muted-foreground">
            {t("skillFolderTree.systemHeading", { defaultValue: "System" })}
          </div>
          <VirtualRow
            active={selection === "unfiled"}
            label={t("skillFolderTree.unfiled", { defaultValue: "Unfiled" })}
            count={unfiledCount}
      ...
      {tags.length > 0 ? (
        <div className="mt-4 border-t border-border pt-3">
          <div className="mb-1.5 flex items-center gap-1.5 px-2 text-(length:--text-micro) font-medium uppercase tracking-wide text-muted-foreground">
            ...
            {t("skillFolderTree.tagsHeading", { defaultValue: "Tags" })}
          </div>
      ...
      <div
        role="separator"
        aria-label={t("skillFolderTree.resize.ariaLabel", { defaultValue: "Resize skill folders" })}
  ...
  return (
    <div className="group/heading flex items-center justify-between px-2 pb-0.5 pt-3">
      <span className="text-(length:--text-micro) font-medium uppercase tracking-wide text-muted-foreground">
        {t("skillFolderTree.company.heading", { defaultValue: label })}
      </span>
      <button
        type="button"
        onClick={onCreate}
        title={t("skillFolderTree.company.newFolderTitle", {
          defaultValue: `New ${label.toLowerCase()} folder`,
        })}
    ...
    <div>
      ...
      >
        <button
          ...
          aria-label={
            isOpen
              ? t("skillFolderTree.actions.collapseFolder", { defaultValue: "Collapse folder" })
              : t("skillFolderTree.actions.expandFolder", { defaultValue: "Expand folder" })
          }
          className={cn(
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                ...
                aria-label={t("skillFolderTree.actions.folderActions", {
            ...
            <DropdownMenuContent align="end">
              {canNest ? (
                <DropdownMenuItem onSelect={() => onCreateFolder(folder.id)}>
                  ...
                  {t("skillFolderTree.actions.newSubfolder", { defaultValue: "New subfolder" })}
                </DropdownMenuItem>
              ) : null}
              {editable ? (
                <>
                  <DropdownMenuItem onSelect={() => onStartRename(folder)}>
                    {t("skillFolderTree.actions.rename", { defaultValue: "Rename" })}
                  </DropdownMenuItem>
                  <DropdownMenuItem onSelect={() => onEditFolder(folder)}>
                    {t("skillFolderTree.actions.editColor", { defaultValue: "Edit color" })}
                  </DropdownMenuItem>
                  <DropdownMenuItem onSelect={() => onMoveFolder(folder, isInMySkills ? "company" : "my")}>
                    <MoveRight className="h-3.5 w-3.5" />
                    {isInMySkills
                      ? t("skillFolderTree.actions.moveToCompany", { defaultValue: "Move to Company" })
                      : t("skillFolderTree.actions.moveToMySkills", { defaultValue: "Move to My Skills" })}
                  </DropdownMenuItem>
                  <DropdownMenuSeparator />
                  <DropdownMenuItem variant="destructive" onSelect={() => onDeleteFolder(folder)}>
                    <Trash2 className="h-3.5 w-3.5" />
                    {t("skillFolderTree.actions.delete", { defaultValue: "Delete" })}
                  </DropdownMenuItem>
                </>
              ) : null}
            </DropdownMenuContent>
...
export function FolderBreadcrumb({
  ...
  return (
    <nav aria-label={t("skillFolderTree.breadcrumb.ariaLabel", { defaultValue: "Folder path" })} className="flex flex-wrap items-center gap-1 text-sm">
      <button
        type="button"
      ...
      >
        ...
        {t("skillFolderTree.allSkills", { defaultValue: "All skills" })}
      </button>
      {selection === "unfiled" ? (
        <>
          <ChevronRight className="h-3.5 w-3.5 text-muted-foreground/60" />
          <span className="rounded px-1.5 py-0.5 font-medium text-foreground">
            {t("skillFolderTree.unfiled", { defaultValue: "Unfiled" })}
          </span>
        </>
      ) : null}
        ...
        >
          ...
          {isCurrent ? (
            <span className="text-xs text-muted-foreground">
              {t("skillFolderTree.labels.current", { defaultValue: "current" })}
            </span>
          ) : null}
          {bundled ? (
            <span className="text-xs text-muted-foreground">
              {t("skillFolderTree.labels.readOnly", { defaultValue: "read-only" })}
            </span>
          ) : null}
          ...
          {nestable ? (
            <button
              type="button"
              title={t("skillFolderTree.actions.newFolderInside", { defaultValue: "New folder inside…" })}
      ...
      <DialogContent className="sm:max-w-lg">
        ...
        <div className="flex items-center gap-2 rounded-md border border-border px-2.5">
          ...
          <input
            ...
            placeholder={t("skillFolderTree.search.placeholder", { defaultValue: "Search folders" })}
        ...
        <div className="max-h-72 overflow-y-auto rounded-md border border-border p-1">
          ...
          <div className="mt-1 border-t border-border pt-1">
            {creatingParent === null ? (
              ...
              >
                <Plus className="h-3.5 w-3.5" />
                {t("skillFolderTree.actions.newTopLevelFolder", { defaultValue: "New top-level folder…" })}
              </button>
            )}
          </div>
        </div>
        ...
        <div className="min-h-5 text-xs text-muted-foreground">
          {previewPath ? (
            <span>
              {t("skillFolderTree.move.movingToPrefix", { defaultValue: "Moving to " })}
              <span className="font-mono text-foreground">{previewPath}</span>
            </span>
          ) : (
            <span>{t("skillFolderTree.move.pickDestination", { defaultValue: "Pick a destination folder." })}</span>
          )}
        </div>

        <DialogFooter>
          <Button variant="ghost" onClick={() => onOpenChange(false)} disabled={pending}>
            {t("skillFolderTree.actions.cancel", { defaultValue: "Cancel" })}
          </Button>
          <Button
          ...
          >
            {pending
              ? t("skillFolderTree.actions.moving", { defaultValue: "Moving…" })
              : t("skillFolderTree.actions.moveHere", { defaultValue: "Move here" })}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  ...
  return (
    <div className="flex items-center gap-2 py-1" style={{ paddingLeft: `${0.5 + depth * 0.9}rem` }}>
      ...
      <Input
        ...
        placeholder={t("skillFolderTree.fields.folderName", { defaultValue: "Folder name" })}
        autoFocus
      ...
      <Button size="sm" onClick={onSubmit} disabled={pending || !value.trim()}>
        {t("skillFolderTree.actions.add", { defaultValue: "Add" })}
      </Button>
    </div>
  );
}
