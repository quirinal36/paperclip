...
function userLabel(userId: string, resolvers: HandoffChipResolvers): string {
  ...
  const base = label ?? t("interruptHandoffViews.assignee.boardFallback", { defaultValue: "Board" });
  return resolvers.currentUserId && resolvers.currentUserId === userId
    ? t("interruptHandoffViews.assignee.youSuffix", { defaultValue: "{{base}} (you)", base })
    : base;
}
...
export function AssigneeChip({
  ...
  if (assignee.agentId) {
    return (
      <span className={cn(CHIP_CLASS, className)} data-testid="handoff-assignee-chip" data-kind="agent">
        <span className="sr-only">{t("interruptHandoffViews.assignee.agentSrPrefix", { defaultValue: "Agent " })}</span>
  ...
  if (assignee.userId) {
    return (
      <span className={cn(CHIP_CLASS, className)} data-testid="handoff-assignee-chip" data-kind="user">
        <span className="sr-only">{t("interruptHandoffViews.assignee.userSrPrefix", { defaultValue: "User " })}</span>
  ...
  return (
    ...
    >
      <span className="sr-only">{t("interruptHandoffViews.assignee.noResponsibleSrPrefix", { defaultValue: "No responsible — " })}</span>
      {t("interruptHandoffViews.assignee.unassigned", { defaultValue: "Unassigned" })}
    </span>
  );
}
...
export function HandoffWakeRow({
  ...
  return (
    <div
      ...
      data-kind={info.kind}
    >
      <span className="text-xs font-medium uppercase tracking-wider text-muted-foreground">{t("interruptHandoffViews.wakeRow.label", { defaultValue: "Wake" })}</span>
  ...
  return (
    ...
    >
      ...
      <span className="min-w-0 flex-1">
        {t("interruptHandoffViews.mentionCoach.didYouMeanPrefix", { defaultValue: "Did you mean " })}
        <span className="font-medium">@{candidate.matchedText}</span>
        {t("interruptHandoffViews.mentionCoach.plainTextSuffix", {
      ...
      <button
        ...
        aria-label={t("interruptHandoffViews.mentionCoach.insertAriaLabel", {
          defaultValue: "Insert mention for {{agentDisplayName}} into your comment",
          agentDisplayName,
        })}
      >
        {t("interruptHandoffViews.mentionCoach.insertAction", { defaultValue: "Insert mention" })}
      </button>
      <button
        type="button"
        onClick={onDismiss}
        className="shrink-0 rounded p-0.5 hover:bg-amber-100/60 dark:hover:bg-amber-500/20"
        aria-label={t("interruptHandoffViews.mentionCoach.dismissAriaLabel", { defaultValue: "Dismiss suggestion" })}
      >
      ...
      <div className="flex items-start gap-1.5">
        ...
        <div className="min-w-0 flex-1 space-y-1">
          ...
          <p className="flex flex-wrap items-center gap-1 text-amber-700/90 dark:text-amber-300/90">
            <span>{t("interruptHandoffViews.interruptAssign.handOffTo", { defaultValue: "Hand off to" })}</span>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1.5 text-xs font-medium uppercase tracking-wider text-muted-foreground">
        ...
        {t("interruptHandoffViews.pauseAffects.heading", { defaultValue: "What this affects" })}
      </div>
      {summary.nothingLive ? (
        <p role="status" className="text-xs text-muted-foreground" data-testid="pause-nothing-live">
          {t("interruptHandoffViews.pauseAffects.nothingLive", {
            defaultValue:
      ...
      {visibleBuckets.length > 0 ? (
        ...
        <p className="text-xs text-muted-foreground">{t("interruptHandoffViews.pauseAffects.noneAffected", { defaultValue: "No tasks are affected." })}</p>
      )}
    </div>
  );
}
