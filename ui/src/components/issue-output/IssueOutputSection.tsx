...
function OutputMediaPreview({
  ...
  if (onMediaClick) {
    return (
      <button
        type="button"
        className={className}
        aria-label={t("issueOutputSection.media.browseInGallery", { defaultValue: "Browse {{filename}} in gallery", filename })}
  ...
  return (
    <a
      ...
      rel="noreferrer"
      className={className}
      aria-label={t("issueOutputSection.media.open", { defaultValue: "Open {{filename}}", filename })}
    >
      {preview}
    </a>
  );
...
export function IssueOutputSection({ workProducts, resolveCreatorName, onMediaClick }: IssueOutputSectionProps) {
  ...
  return (
    <section className="space-y-3" aria-label={t("issueOutputSection.sectionLabel", { defaultValue: "Task outputs" })}>
      <div className="flex items-center gap-2">
        <Play className="h-3.5 w-3.5 text-muted-foreground" aria-hidden="true" />
        <h3 className="text-sm font-medium text-muted-foreground">{t("issueOutputSection.heading", { defaultValue: "Output" })}</h3>
      ...
      {rest.length > 0 ? (
        <div className="space-y-2">
          <p className="text-(length:--text-micro) font-medium uppercase tracking-wide text-muted-foreground">{t("issueOutputSection.alsoProduced", { defaultValue: "Also produced" })}</p>
          {mediaRest.length > 0 ? (
