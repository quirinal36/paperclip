  ...
  return (
    <Card className="flex-row items-center gap-2.5 p-2">
      ...
      <div className="min-w-0 flex-1">
        ...
        >
          {item.degraded
            ? t("outputRow.fileDetailsUnavailable", { defaultValue: "File details unavailable" })
            : metaBits.join(" · ")}
        </p>
      </div>
      {meta ? (
        <div className="flex shrink-0 items-center gap-1">
          <Button
            ...
            variant="ghost"
            size="icon-sm"
            title={t("outputRow.actions.openInNewTab", { defaultValue: "Open in new tab" })}
          >
            <a
              href={meta.openPath}
              target="_blank"
              rel="noreferrer"
              aria-label={t("outputRow.actions.openFile", { defaultValue: "Open {{filename}}", filename })}
            >
          ...
          <Button
            ...
            variant="ghost"
            size="icon-sm"
            title={t("outputRow.actions.download", { defaultValue: "Download" })}
          >
            <a
              href={meta.downloadPath}
              aria-label={t("outputRow.actions.downloadFile", { defaultValue: "Download {{filename}}", filename })}
            >
