    ...
    <div className="space-y-6">
      ...
      <div className="overflow-x-auto overscroll-x-contain">
        <div className="inline-flex min-w-full flex-wrap items-center gap-2 text-sm text-muted-foreground sm:min-w-max sm:flex-nowrap">
          <span>{t("editableSections.overview.for", { defaultValue: "For" })}</span>
          <InlineEntitySelector
            ref={assigneeSelectorRef}
            value={editDraft.assigneeAgentId}
            options={assigneeOptions}
            recentOptionIds={recentAssigneeIds}
            placeholder={t("editableSections.overview.assignee.placeholder", { defaultValue: "Responsible" })}
            noneLabel={t("editableSections.overview.assignee.none", { defaultValue: "No responsible" })}
            searchPlaceholder={t("editableSections.overview.assignee.searchPlaceholder", { defaultValue: "Search responsible..." })}
            emptyMessage={t("editableSections.overview.assignee.empty", { defaultValue: "No responsible found." })}
            ...
            renderTriggerValue={(option) =>
              option ? (
                ...
                <span className="text-muted-foreground">{t("editableSections.overview.assignee.placeholder", { defaultValue: "Responsible" })}</span>
              )
            }
            renderOption={(option) => {
          ...
          />
          <span>{t("editableSections.overview.in", { defaultValue: "in" })}</span>
          <InlineEntitySelector
            ref={projectSelectorRef}
            value={editDraft.projectId}
            options={projectOptions}
            recentOptionIds={recentProjectIds}
            placeholder={t("editableSections.overview.project.placeholder", { defaultValue: "Project" })}
            noneLabel={t("editableSections.overview.project.none", { defaultValue: "No project" })}
            searchPlaceholder={t("editableSections.overview.project.searchPlaceholder", { defaultValue: "Search projects..." })}
            emptyMessage={t("editableSections.overview.project.empty", { defaultValue: "No projects found." })}
            ...
            renderTriggerValue={(option) =>
              option && currentProject ? (
                ...
                <span className="text-muted-foreground">{t("editableSections.overview.project.placeholder", { defaultValue: "Project" })}</span>
              )
            }
            renderOption={(option) => {
              if (!option.id) return <span className="truncate">{option.label}</span>;
      ...
      {!routine.assigneeAgentId ? (
        <div className="rounded-lg border border-amber-500/30 bg-amber-500/5 p-4 text-sm text-amber-900 dark:text-amber-200">
          {t("editableSections.overview.defaultAgentRequired", {
            defaultValue:
        ...
        {routine.descriptionDocument ? (
          ...
          >
            <MarkdownEditor
              ...
              placeholder={t("editableSections.overview.instructionsPlaceholder", { defaultValue: "Add instructions..." })}
              bordered={false}
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <SummaryCard
          icon={Clock3}
          label={t("editableSections.overview.summary.triggers.label", { defaultValue: "Triggers" })}
          value={
            activeTriggers === 0
              ? t("editableSections.overview.summary.none", { defaultValue: "None" })
              : t("editableSections.overview.summary.triggers.active", {
                  defaultValue: "{{total}} active",
                  total: activeTriggers,
                })
          }
          hint={
            nextFire
              ? t("editableSections.overview.summary.triggers.nextFire", {
                  defaultValue: "Next fire {{time}}",
                  time: nextFire,
                })
              : t("editableSections.overview.summary.triggers.noSchedule", { defaultValue: "No schedule" })
          }
          to={() => navigateToSection("triggers")}
          ariaLabel={t("editableSections.overview.summary.triggers.aria", {
            defaultValue: "{{total}} triggers. Open triggers.",
            total: activeTriggers,
          })}
        />
        <SummaryCard
          icon={KeyRound}
          label={t("editableSections.overview.summary.secrets.label", { defaultValue: "Secrets" })}
          value={
            boundSecrets === 0
              ? t("editableSections.overview.summary.none", { defaultValue: "None" })
              : t("editableSections.overview.summary.secrets.bound", {
                  defaultValue: "{{total}} bound",
                  total: boundSecrets,
                })
          }
          hint={t("editableSections.overview.summary.secrets.hint", { defaultValue: "Manage bound secrets" })}
          to={() => navigateToSection("secrets")}
          ariaLabel={t("editableSections.overview.summary.secrets.aria", {
            defaultValue: "{{total}} secrets bound. Open secrets.",
            total: boundSecrets,
          })}
        />
        <SummaryCard
          icon={Play}
          label={t("editableSections.overview.summary.lastRun.label", { defaultValue: "Last run" })}
          value={lastRun ? lastRun.status.replaceAll("_", " ") : t("editableSections.overview.summary.lastRun.noRuns", { defaultValue: "No runs" })}
          hint={lastRun ? timeAgo(lastRun.triggeredAt) : t("editableSections.overview.summary.lastRun.hint", { defaultValue: "Trigger a run" })}
          to={() => navigateToSection("runs")}
          ariaLabel={
            lastRun
              ? t("editableSections.overview.summary.lastRun.aria", {
                  defaultValue: "Last run {{status}}. Open runs.",
                  status: lastRun.status,
                })
              : t("editableSections.overview.summary.lastRun.ariaNone", { defaultValue: "No runs. Open runs." })
          }
        />
      </div>
      ...
      <div className="space-y-2">
        <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">
          {t("editableSections.overview.recentActivity", { defaultValue: "Recent activity" })}
        </p>
        {recentActivity.length === 0 ? (
          <p className="text-xs text-muted-foreground">{t("editableSections.overview.noActivity", { defaultValue: "No activity yet." })}</p>
        ) : (
          <div className="divide-y divide-border/60">
            ...
            >
              {t("editableSections.overview.viewAllActivity", { defaultValue: "View all activity " })}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm font-medium text-muted-foreground">
          {routine.triggers.length === 0
            ? t("editableSections.triggers.count.none", { defaultValue: "No triggers yet" })
            : routine.triggers.length === 1
              ? t("editableSections.triggers.count.one", {
                  defaultValue: "{{total}} trigger",
                  total: routine.triggers.length,
                })
              : t("editableSections.triggers.count.other", {
        ...
        >
          {addOpen ? (
            <>
              ...
              {t("editableSections.actions.cancel", { defaultValue: "Cancel" })}
            </>
          ) : (
            <>
              <Plus className="mr-1.5 h-3.5 w-3.5" />
              {t("editableSections.triggers.newTrigger", { defaultValue: "New trigger" })}
            </>
          )}
        </Button>
      </div>
      ...
      <div className="space-y-3 rounded-lg border border-border p-4">
        <p className="text-sm font-medium">{t("editableSections.triggers.addTrigger", { defaultValue: "Add trigger" })}</p>
        <div className="grid gap-3 md:grid-cols-2">
          <div className="space-y-1.5">
            <Label className="text-xs">{t("editableSections.triggers.fields.kind", { defaultValue: "Kind" })}</Label>
            <Select
              ...
              <SelectContent>
                {triggerKinds.map((kind) => (
                  <SelectItem key={kind} value={kind} disabled={kind === "webhook"}>
                    {kind}
                    {kind === "webhook"
                      ? t("editableSections.triggers.comingSoon", { defaultValue: " — COMING SOON" })
                      : ""}
                  </SelectItem>
                ))}
              </SelectContent>
          ...
          {newTrigger.kind === "schedule" && (
            <div className="space-y-1.5 md:col-span-2">
              <Label className="text-xs">{t("editableSections.triggers.fields.schedule", { defaultValue: "Schedule" })}</Label>
              <ScheduleEditor
          ...
          {newTrigger.kind === "webhook" && (
            <>
              <div className="space-y-1.5">
                <Label className="text-xs">{t("editableSections.triggers.fields.signingMode", { defaultValue: "Signing mode" })}</Label>
                <Select
              ...
              {!SIGNING_MODES_WITHOUT_REPLAY_WINDOW.has(newTrigger.signingMode) && (
                <div className="space-y-1.5">
                  <Label className="text-xs">{t("editableSections.triggers.fields.replayWindow", { defaultValue: "Replay window (seconds)" })}</Label>
                  <Input
        ...
        <div className="flex items-center justify-end gap-2">
          ...
          >
            {createTrigger.isPending
              ? t("editableSections.triggers.adding", { defaultValue: "Adding..." })
              : t("editableSections.triggers.addTrigger", { defaultValue: "Add trigger" })}
          </Button>
        </div>
      </div>
      ) : null}
      ...
      {routine.triggers.length === 0 ? (
        <EmptyState
          icon={Clock3}
          message={t("editableSections.triggers.empty.message", { defaultValue: "No triggers yet." })}
          action={t("editableSections.triggers.empty.action", { defaultValue: "Add a schedule" })}
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-3 rounded-md border border-border bg-muted/20 px-4 py-3 text-xs">
        <span className="flex-1 text-muted-foreground">
          {t("editableSections.variables.hint.before", { defaultValue: "Variables are auto-detected from " })}
          <code className="font-mono">{"{{placeholders}}"}</code>
          {t("editableSections.variables.hint.after", {
            defaultValue:
        ...
        <Button variant="secondary" size="sm" onClick={() => navigateToSection("overview")}>
          ...
          {t("editableSections.actions.editInstructions", { defaultValue: "Edit instructions" })}
        </Button>
      </div>
      ...
      {hasVariables ? (
        ...
        <EmptyState
          icon={Braces}
          message={t("editableSections.variables.empty.message", {
            defaultValue: "No variables yet. Add a {{placeholder}} in the title or instructions to create one.",
          })}
          action={t("editableSections.actions.editInstructions", { defaultValue: "Edit instructions" })}
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-md border border-border bg-muted/20 px-4 py-3 text-xs text-muted-foreground">
        {t("editableSections.secrets.reservedNamesBefore", {
          defaultValue:
        ...
        {t("editableSections.secrets.reservedNamesAfter", { defaultValue: " names are reserved." })}
      </div>
      ...
      {secretMessage ? (
        <div className="space-y-3 rounded-lg border border-blue-500/30 bg-blue-500/5 p-4 text-sm">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              {t("editableSections.secrets.saveNow", {
            ...
            {secretMessage.entries.map((entry, index) => (
              <div key={`${entry.webhookUrl}-${index}`} className="space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <Button variant="outline" size="sm" onClick={() => copySecretValue(t("editableSections.secrets.webhookUrlLabel", { defaultValue: "Webhook URL" }), entry.webhookUrl)}>
                    {t("editableSections.secrets.copyUrl", { defaultValue: "URL" })}
                  </Button>
                </div>
                <div className="flex items-center gap-2">
                  <Input value={entry.webhookSecret} readOnly className="flex-1" />
                  <Button variant="outline" size="sm" onClick={() => copySecretValue(t("editableSections.secrets.webhookSecretLabel", { defaultValue: "Webhook secret" }), entry.webhookSecret)}>
                    {t("editableSections.secrets.copySecret", { defaultValue: "Secret" })}
                  </Button>
                </div>
              </div>
            ))}
...
export function DeliverySection() {
  ...
  const concurrencyPolicyOptions = [
    {
      ...
      title: t("editableSections.delivery.concurrencyOptions.coalesceIfActive.title", {
        defaultValue: "Coalesce if active",
      }),
      description: t("editableSections.delivery.concurrencyOptions.coalesceIfActive.description", {
        defaultValue: "Keep one follow-up run queued while an active run is still working.",
      }),
    },
    {
      value: "always_enqueue",
      title: t("editableSections.delivery.concurrencyOptions.alwaysEnqueue.title", {
        defaultValue: "Always enqueue",
      }),
      description: t("editableSections.delivery.concurrencyOptions.alwaysEnqueue.description", {
        defaultValue: "Queue every trigger occurrence, even if several runs stack up.",
      }),
    },
    {
      value: "skip_if_active",
      title: t("editableSections.delivery.concurrencyOptions.skipIfActive.title", {
        defaultValue: "Skip if active",
      }),
      description: t("editableSections.delivery.concurrencyOptions.skipIfActive.description", {
  ...
  const catchUpPolicyOptions = [
    {
      value: "skip_missed",
      title: t("editableSections.delivery.catchUpOptions.skipMissed.title", { defaultValue: "Skip missed" }),
      description: t("editableSections.delivery.catchUpOptions.skipMissed.description", {
        defaultValue: "Ignore schedule windows that were missed while paused.",
      }),
    },
    {
      value: "enqueue_missed_with_cap",
      title: t("editableSections.delivery.catchUpOptions.enqueueMissedWithCap.title", {
        defaultValue: "Enqueue missed with cap",
      }),
      description: t("editableSections.delivery.catchUpOptions.enqueueMissedWithCap.description", {
        defaultValue:
    ...
    <div className="space-y-6">
      <div className="space-y-3">
        <p className="text-xs font-medium uppercase tracking-(--tracking-caps) text-muted-foreground">
          {t("editableSections.delivery.concurrency", { defaultValue: "Concurrency" })}
        </p>
        <RadioCardGroup
          ariaLabel={t("editableSections.delivery.concurrencyPolicyAria", { defaultValue: "Concurrency policy" })}
      ...
      <div className="space-y-3">
        <p className="text-xs font-medium uppercase tracking-(--tracking-caps) text-muted-foreground">
          {t("editableSections.delivery.catchUp", { defaultValue: "Catch-up" })}
        </p>
        <RadioCardGroup
          ariaLabel={t("editableSections.delivery.catchUpPolicyAria", { defaultValue: "Catch-up policy" })}
...
function NextFiresPreview({
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs font-medium uppercase tracking-(--tracking-caps) text-muted-foreground">
        {t("editableSections.nextFires.title", { defaultValue: "Next 5 fires" })}
      </p>
      {preview ? (
        <>
          ...
          <p className="text-(length:--text-micro) text-muted-foreground/60">
            {t("editableSections.nextFires.previewNote", {
              defaultValue:
        ...
        <p className="rounded-lg border border-dashed border-border p-3 text-xs text-muted-foreground">
          {t("editableSections.nextFires.empty", {
            defaultValue:
