  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5" data-testid="search-filter-chips">
      {chips.map((chip) => (
        <Badge key={chip.id} variant="secondary" className="gap-1 pr-1 font-normal">
          ...
          <button
            ...
            aria-label={t("searchFilterChips.removeFilter.ariaLabel", { defaultValue: "Remove filter {{label}}", label: chip.label })}
          >
            <X className="h-3 w-3" />
          </button>
        </Badge>
      ...
      <button
        ...
        onClick={onClearAll}
      >
        {t("searchFilterChips.clearAll", { defaultValue: "Clear all" })}
      </button>
    </div>
  );
}
