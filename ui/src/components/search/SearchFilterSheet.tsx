...
export function SearchFilterSheet({
  ...
  const applyLabel =
    previewTotal === null
      ? t("searchFilterSheet.actions.showResults", { defaultValue: "Show results" })
      : previewTotal === 1
        ? t("searchFilterSheet.actions.showResultSingular", {
            defaultValue: "Show {{total}} result",
            total: previewTotal,
          })
        : t("searchFilterSheet.actions.showResultPlural", {
            defaultValue: "Show {{total}} results",
            total: previewTotal,
          });
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="bottom" className="max-h-(--sz-85vh) gap-0 rounded-t-xl p-0" data-testid="search-filter-sheet">
        <SheetHeader className="flex-row items-center justify-between border-b border-border">
          <SheetTitle className="text-base">{t("searchFilterSheet.title", { defaultValue: "Filters" })}</SheetTitle>
          <button
            type="button"
            className={cn("text-xs text-muted-foreground hover:text-foreground", activeCount === 0 && "invisible")}
            onClick={() => update({})}
          >
            {t("searchFilterSheet.actions.clearAll", { defaultValue: "Clear all" })}
          </button>
        </SheetHeader>

        <div className="flex-1 space-y-4 overflow-y-auto p-4">
          <ChipToggleGroup
            title={t("searchFilterSheet.groups.status", { defaultValue: "Status" })}
            options={options.status}
            selected={draft.status ?? []}
            onToggle={(value) => toggleMulti("status", value)}
          />
          <ChipToggleGroup
            title={t("searchFilterSheet.groups.priority", { defaultValue: "Priority" })}
            options={options.priority}
            selected={draft.priority ?? []}
            onToggle={(value) => toggleMulti("priority", value)}
          />
          <ChipToggleGroup
            title={t("searchFilterSheet.groups.assignee", { defaultValue: "Assignee" })}
            options={options.assignee}
            selected={selectedAssignee ? [selectedAssignee] : []}
            onToggle={toggleAssignee}
          />
          <ChipToggleGroup
            title={t("searchFilterSheet.groups.project", { defaultValue: "Project" })}
            options={options.project}
            selected={draft.projectId ? [draft.projectId] : []}
            onToggle={(value) => toggleSingle("projectId", value)}
          />
          <ChipToggleGroup
            title={t("searchFilterSheet.groups.label", { defaultValue: "Label" })}
            options={options.label}
            selected={draft.labelId ? [draft.labelId] : []}
            onToggle={(value) => toggleSingle("labelId", value)}
          />
          <ChipToggleGroup
            title={t("searchFilterSheet.groups.updated", { defaultValue: "Updated" })}
            options={options.updated}
            selected={draft.updatedWithin ? [draft.updatedWithin] : []}
            onToggle={(value) => toggleSingle("updatedWithin", value)}
          />
          <div className="space-y-1.5">
            <div className="text-xs font-medium text-muted-foreground">{t("searchFilterSheet.groups.sortBy", { defaultValue: "Sort by" })}</div>
            <div className="flex flex-wrap gap-1.5">
        ...
        <SheetFooter className="flex-row gap-2 border-t border-border">
          <SheetClose asChild>
            <Button variant="outline" className="flex-1">
              {t("searchFilterSheet.actions.cancel", { defaultValue: "Cancel" })}
            </Button>
          </SheetClose>
          <Button
            className="flex-1"
...
export function SearchFilterSheetTrigger({
  ...
  return (
    <Button variant="outline" size="sm" className="h-8 gap-1.5 text-xs font-normal" onClick={onClick}>
      ...
      {t("searchFilterSheet.trigger.label", { defaultValue: "Filters" })}
      {activeCount > 0 ? (
