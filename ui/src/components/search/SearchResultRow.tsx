...
const SNIPPET_STYLES: Record<string, SnippetStyle> = {
  comment: { Icon: MessageSquare, labelKey: "searchResultRow.snippetLabels.comment", labelDefault: "Comment" },
  document: { Icon: FileText, labelKey: "searchResultRow.snippetLabels.document", labelDefault: "Doc" },
  artifact: { Icon: Paperclip, labelKey: "searchResultRow.snippetLabels.artifact", labelDefault: "Artifact" },
  description: { Icon: Quote, labelKey: "searchResultRow.snippetLabels.description", labelDefault: "Description" },
};
...
function formatRelativeTime(input: string | null): string {
  ...
  if (seconds < 60) return t("searchResultRow.time.justNow", { defaultValue: "just now" });
  const minutes = Math.round(seconds / 60);
  if (minutes < 60) return t("searchResultRow.time.minutes", { defaultValue: "{{value}}m", value: minutes });
  const hours = Math.round(minutes / 60);
  if (hours < 24) return t("searchResultRow.time.hours", { defaultValue: "{{value}}h", value: hours });
  const days = Math.round(hours / 24);
  if (days < 7) return t("searchResultRow.time.days", { defaultValue: "{{value}}d", value: days });
  const weeks = Math.round(days / 7);
  if (weeks < 5) return t("searchResultRow.time.weeks", { defaultValue: "{{value}}w", value: weeks });
  const months = Math.round(days / 30);
  if (months < 12) return t("searchResultRow.time.months", { defaultValue: "{{value}}mo", value: months });
  const years = Math.round(days / 365);
  return t("searchResultRow.time.years", { defaultValue: "{{value}}y", value: years });
}
        ...
        <div className="min-w-0 flex-1">
          ...
          {result.snippet ? (
            <SnippetLine
              ...
              field="agent"
              fallbackLabel={result.sourceLabel ?? t("searchResultRow.sourceLabels.agent", { defaultValue: "Agent" })}
            />
          ) : null}
        </div>
      </Link>
        ...
        <div className="min-w-0 flex-1">
          ...
          {result.snippet ? (
            <SnippetLine
              ...
              highlights={result.snippets[0]?.highlights}
              field="project"
              fallbackLabel={result.sourceLabel ?? t("searchResultRow.sourceLabels.project", { defaultValue: "Project" })}
            />
          ) : null}
        </div>
      </Link>
        ...
        <div className="min-w-0 flex-1">
          ...
          {result.snippet ? (
            <SnippetLine
              ...
              highlights={result.snippets[0]?.highlights}
              field="artifact"
              fallbackLabel={result.sourceLabel ?? t("searchResultRow.sourceLabels.artifact", { defaultValue: "Artifact" })}
              multiline
            />
          ) : null}
...
function SnippetLine({ text, highlights, field, fallbackLabel, multiline = false }: SnippetLineProps) {
  ...
  const label = style ? t(style.labelKey, { defaultValue: style.labelDefault }) : fallbackLabel;
  return (
    <div
      className={cn(
