...
export function SkillLineageChip({
  ...
  const label = original
    ...
    : t("skillProvenance.lineage.originalSkillFallback", { defaultValue: "the original skill" });
  ...
  return (
    <Link
      ...
      title={t("skillProvenance.lineage.forkedFromTitle", { defaultValue: "Forked from {{label}}", label })}
    >
      <GitFork className="h-3 w-3 shrink-0" />
      <span className="truncate">
        {t("skillProvenance.lineage.forkedFrom", { defaultValue: "Forked from " })}
...
export function ProjectScanNotice({
  ...
  const location =
    skill.sourcePath ??
    skill.sourceLabel ??
    t("skillProvenance.projectScan.workingTreeFallback", { defaultValue: "the project working tree" });
  ...
  return (
    <div className="flex flex-wrap items-start gap-2 border-b border-border bg-muted/40 px-3 py-2 text-xs text-muted-foreground">
      ...
      <div className="min-w-0 flex-1">
        <span>
          {t("skillProvenance.projectScan.livesIn", { defaultValue: "This skill lives in " })}
          <span className="font-mono text-foreground">{location}</span>
          {t("skillProvenance.projectScan.savesNotice", {
            defaultValue: ". Saves write to the project working tree and are not committed.",
          })}
        </span>{" "}
        <Button
          type="button"
          ...
          onClick={onEditACopy}
        >
          {t("skillProvenance.projectScan.editCopy", { defaultValue: "Edit a copy instead" })}
        </Button>
      </div>
    </div>
  );
