...
export function ApprovalDetail() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("approvalDetail.breadcrumb.approvals", { defaultValue: "Approvals" }), href: "/approvals" },
      {
        label:
          approval?.id?.slice(0, 8) ??
          approvalId ??
          t("approvalDetail.breadcrumb.fallback", { defaultValue: "Approval" }),
      },
    ]);
    ...
    onError: (err) =>
      setError(
        err instanceof Error
          ? err.message
          : t("approvalDetail.errors.approveFailed", { defaultValue: "Approve failed" }),
      ),
  });
  ...
  const rejectMutation = useMutation({
    ...
    onError: (err) =>
      setError(
        err instanceof Error
          ? err.message
          : t("approvalDetail.errors.rejectFailed", { defaultValue: "Reject failed" }),
      ),
  });
  ...
  const revisionMutation = useMutation({
    ...
    onError: (err) =>
      setError(
        err instanceof Error
          ? err.message
          : t("approvalDetail.errors.revisionFailed", {
  ...
  const resubmitMutation = useMutation({
    ...
    onError: (err) =>
      setError(
        err instanceof Error
          ? err.message
          : t("approvalDetail.errors.resubmitFailed", { defaultValue: "Resubmit failed" }),
      ),
  });
  ...
  const addCommentMutation = useMutation({
    ...
    onError: (err) =>
      setError(
        err instanceof Error
          ? err.message
          : t("approvalDetail.errors.commentFailed", { defaultValue: "Comment failed" }),
      ),
  });
  ...
  const deleteAgentMutation = useMutation({
    ...
    onError: (err) =>
      setError(
        err instanceof Error
          ? err.message
          : t("approvalDetail.errors.deleteFailed", { defaultValue: "Delete failed" }),
      ),
  });
  ...
  if (!approval)
    return (
      <p className="text-sm text-muted-foreground">
        {t("approvalDetail.notFound", { defaultValue: "Approval not found." })}
      </p>
    );
      ...
      ? {
          label:
            (linkedIssues?.length ?? 0) > 1
              ? t("approvalDetail.cta.reviewLinkedTasks", { defaultValue: "Review linked tasks" })
              : t("approvalDetail.cta.reviewLinkedTask", { defaultValue: "Review linked task" }),
    ...
    <div className="space-y-6 max-w-3xl">
            ...
            <div className="flex items-start gap-2">
              ...
              <div>
                <p className="text-sm text-green-800 dark:text-green-100 font-medium">
                  {t("approvalDetail.banner.confirmedTitle", { defaultValue: "Approval confirmed" })}
                </p>
                <p className="text-xs text-green-700 dark:text-green-200/90">
                  {t("approvalDetail.banner.confirmedBody", {
                    defaultValue:
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
          ...
          {approval.requestedByAgentId && (
            <div className="flex items-center gap-2">
              <span className="text-muted-foreground text-xs">
                {t("approvalDetail.requestedBy", { defaultValue: "Requested by" })}
              </span>
              <Identity
        ...
        {linkedIssues && linkedIssues.length > 0 && (
          <div className="pt-2 border-t border-border/60">
            <p className="text-xs text-muted-foreground mb-1.5">
              {t("approvalDetail.linkedTasks.heading", { defaultValue: "Linked Tasks" })}
            </p>
            ...
            <p className="text-(length:--text-micro) text-muted-foreground mt-2">
              {t("approvalDetail.linkedTasks.hint", {
                defaultValue:
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {isBudgetApproval && approval.status === "pending" && (
            <p className="text-sm text-muted-foreground">
              {t("approvalDetail.budget.resolveHint", {
          ...
          {approval.status === "rejected" && approval.type === "hire_agent" && linkedAgentId && (
            <Button
              ...
              onClick={() => {
                if (
                  !window.confirm(
                    t("approvalDetail.deleteAgent.confirm", {
            ...
            >
              {t("approvalDetail.deleteAgent.button", { defaultValue: "Delete disapproved agent" })}
            </Button>
          )}
        </div>
      </div>
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        <h3 className="text-sm font-medium">
          {t("approvalDetail.comments.heading", {
        ...
        <div className="flex justify-end">
          ...
          >
            {addCommentMutation.isPending
              ? t("approvalDetail.comments.posting", { defaultValue: "Posting…" })
              : t("approvalDetail.comments.post", { defaultValue: "Post comment" })}
          </Button>
        </div>
      </div>
    </div>
