...
export function artifactGroupByLabel(value: ArtifactGroupBy): string {
  const label = ARTIFACT_GROUP_OPTIONS.find((option) => option.value === value)?.label ?? "None";
  return t(`artifacts.groupOptions.${value}`, { defaultValue: label });
}
...
export function Artifacts() {
  ...
  useEffect(() => {
    if (viewingSelectedStack && selectedGroup) {
      setBreadcrumbs([
        { label: t("artifacts.breadcrumb", { defaultValue: "Artifacts" }), href: "/artifacts" },
        { label: `${selectedGroup.issue.identifier} · ${selectedGroup.title}` },
      ]);
    } else {
      setBreadcrumbs([{ label: t("artifacts.breadcrumb", { defaultValue: "Artifacts" }) }]);
    }
  ...
  if (!selectedCompanyId) {
    return (
      <EmptyState
        icon={Package}
        message={t("artifacts.empty.company", { defaultValue: "Select a company to view artifacts." })}
      />
    );
  }
  ...
  const emptyMessage = showGroupCards
    ? searching
      ? t("artifacts.empty.stacksSearch", { defaultValue: "No artifact stacks match this search." })
      : t("artifacts.empty.stacks", { defaultValue: "No artifact stacks yet." })
    : searching
      ? t("artifacts.empty.artifactsSearch", { defaultValue: "No artifacts match this search." })
      : viewingSelectedStack
        ? t("artifacts.empty.stackFiltered", {
            defaultValue: "No artifacts in this stack match the current filters.",
          })
        : kind === "all"
          ? t("artifacts.empty.none", {
              defaultValue: "No artifacts yet. Outputs attached to issues will appear here.",
            })
          : t("artifacts.empty.kindNone", { defaultValue: "No artifacts of this type yet." });
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="relative w-full sm:max-w-sm">
          ...
          <Input
            ...
            placeholder={t("artifacts.search.placeholder", { defaultValue: "Search artifacts..." })}
            aria-label={t("artifacts.search.ariaLabel", { defaultValue: "Search artifacts" })}
          ...
          {draftQuery.length > 0 ? (
            <button
              ...
              aria-label={t("artifacts.search.clearAriaLabel", { defaultValue: "Clear artifact search" })}
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                ...
                variant="outline"
                size="icon"
                aria-label={t("artifacts.groupControl.ariaLabel", {
                  defaultValue: "Group artifacts (currently {{label}})",
                  label: artifactGroupByLabel(groupBy),
                })}
                title={t("artifacts.groupControl.title", { defaultValue: "Group artifacts" })}
            ...
            <DropdownMenuContent align="end" className="w-44">
              <DropdownMenuLabel>
                {t("artifacts.groupByLabel", { defaultValue: "Group by" })}
              </DropdownMenuLabel>
              {ARTIFACT_GROUP_OPTIONS.map((option) => (
                ...
                >
                  {t(`artifacts.groupOptions.${option.value}`, { defaultValue: option.label })}
          ...
          <div
            ...
            role="tablist"
            aria-label={t("artifacts.filterByTypeAriaLabel", { defaultValue: "Filter artifacts by type" })}
          >
            {ARTIFACT_KIND_FILTERS.map((filter) => (
              ...
              >
                {t(`artifacts.kinds.${filter.value}`, { defaultValue: filter.label })}
              </button>
            ))}
          </div>
        </div>
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div ref={loadMoreRef} className="flex min-h-10 items-center justify-center pb-2 text-xs text-muted-foreground">
            {isFetchingNextPage
              ? t("artifacts.loadingMore", { defaultValue: "Loading more artifacts..." })
              : hasNextPage
                ? null
                : isFetching
                  ? t("artifacts.updating", { defaultValue: "Updating artifacts..." })
                  : null}
          </div>
        </>
      )}
