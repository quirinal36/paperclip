...
const FIXTURE_LABELS: Record<LabFixtureKey, string> = {
  "signed-out-private": t("bootstrapSetupUxLab.fixtures.signedOutPrivate", {
    defaultValue: "1 · authenticated/private — signed out (browser claim available)",
  }),
  "signed-in-private": t("bootstrapSetupUxLab.fixtures.signedInPrivate", {
    defaultValue: "2 · authenticated/private — signed in (claim CTA primary)",
  }),
  claiming: t("bootstrapSetupUxLab.fixtures.claiming", {
    defaultValue: "3 · authenticated/private — claim in flight",
  }),
  "claim-error": t("bootstrapSetupUxLab.fixtures.claimError", {
    defaultValue: "4 · authenticated/private — claim error (e.g. 409 already claimed)",
  }),
  "claim-success": t("bootstrapSetupUxLab.fixtures.claimSuccess", {
    defaultValue: "5 · authenticated/private — claim succeeded, redirect pending",
  }),
  "public-invite-only": t("bootstrapSetupUxLab.fixtures.publicInviteOnly", {
  ...
  return (
    <div className="mt-6 border-t border-border pt-5">
      <div className="flex items-center gap-2 text-sm font-medium">
        ...
        <span>
          {t("bootstrapSetupUxLab.cli.prompt", { defaultValue: "Prefer to finish setup from the host?" })}
        </span>
      </div>
      <p className="mt-2 text-sm text-muted-foreground">
        {hasActiveInvite
          ? t("bootstrapSetupUxLab.cli.activeInvite", {
              defaultValue:
          ...
          : t("bootstrapSetupUxLab.cli.noInvite", {
              defaultValue:
  ...
  return (
    <StateChrome>
      <h1 className="text-xl font-semibold">
        {t("bootstrapSetupUxLab.heading.finishSetup", { defaultValue: "Finish setting up this Paperclip" })}
      </h1>
      <p className="mt-2 text-sm text-muted-foreground">
        {t("bootstrapSetupUxLab.signedOut.body", {
          defaultValue:
      ...
      <div className="mt-5">
        <Button asChild>
          <a href="/auth?next=/">
            {t("bootstrapSetupUxLab.signedOut.signInCreate", { defaultValue: "Sign in / Create account" })}
          </a>
        </Button>
      </div>
...
function SignedInPrivate() {
  ...
  return (
    <StateChrome>
      ...
      <p className="mt-2 text-sm text-muted-foreground">
        {t("bootstrapSetupUxLab.body.claimToBecomeAdmin", {
          defaultValue:
            "No admin has claimed this instance yet. Claim it now to become the first admin and start onboarding.",
        })}
      </p>
      <div className="mt-5 flex flex-wrap items-center gap-3">
        <Button>{t("bootstrapSetupUxLab.actions.claimInstance", { defaultValue: "Claim this instance" })}</Button>
        <span className="text-sm text-muted-foreground">
          {t("bootstrapSetupUxLab.status.signedInAs", { defaultValue: "Signed in as " })}
          <span className="font-medium text-foreground">jane@appliance.local</span>
        </span>
      </div>
      <p className="mt-3 text-xs text-muted-foreground">
        {t("bootstrapSetupUxLab.signedIn.wrongAccount", { defaultValue: "Wrong account?" })}{" "}
        <a href="/auth?next=/" className="underline underline-offset-2">
          {t("bootstrapSetupUxLab.signedIn.switchAccount", { defaultValue: "Switch account" })}
        </a>
        .
      </p>
      <CliFallback hasActiveInvite={false} />
    ...
    <StateChrome>
      ...
      <div className="mt-5 flex flex-wrap items-center gap-3">
        <Button disabled>
          ...
          {t("bootstrapSetupUxLab.claiming.button", { defaultValue: "Claiming…" })}
        </Button>
        <span className="text-sm text-muted-foreground">
          {t("bootstrapSetupUxLab.status.signedInAs", { defaultValue: "Signed in as " })}
          <span className="font-medium text-foreground">jane@appliance.local</span>
      ...
      >
        ...
        <div>
          <p className="font-medium">
            {t("bootstrapSetupUxLab.claimError.alertTitle", {
              defaultValue: "Someone else has already claimed this instance.",
            })}
          </p>
          <p className="mt-1 text-destructive/90">
            {t("bootstrapSetupUxLab.claimError.alertBody", {
              defaultValue: "Refresh to sign in, or ask the existing admin to invite you from",
            })}{" "}
            <span className="font-mono">
              {t("bootstrapSetupUxLab.claimError.settingsPath", { defaultValue: "Instance settings → Access" })}
            </span>
            .
          </p>
        </div>
  ...
  return (
    <StateChrome>
      <div className="flex items-start gap-3">
        ...
        <div>
          <h1 className="text-xl font-semibold">
            {t("bootstrapSetupUxLab.claimSuccess.title", { defaultValue: "You’re the instance admin" })}
          </h1>
          <p className="mt-2 text-sm text-muted-foreground">
            {t("bootstrapSetupUxLab.claimSuccess.body", {
      ...
      <div className="mt-5 flex items-center gap-3">
        ...
        <span className="text-sm text-muted-foreground">
          {t("bootstrapSetupUxLab.claimSuccess.redirecting", { defaultValue: "Redirecting…" })}
        </span>
      </div>
      <div className="mt-5">
        <Button asChild variant="outline">
          <a href="/">
            {t("bootstrapSetupUxLab.claimSuccess.continue", { defaultValue: "Continue to dashboard" })}
          </a>
        </Button>
      </div>
    </StateChrome>
...
function PublicInviteOnly() {
  ...
  return (
    <StateChrome>
      <h1 className="text-xl font-semibold">
        {t("bootstrapSetupUxLab.publicInvite.title", {
          defaultValue: "This Paperclip is waiting on its first admin",
        })}
      </h1>
      <p className="mt-2 text-sm text-muted-foreground">
        {t("bootstrapSetupUxLab.publicInvite.body", {
          defaultValue:
      ...
      <p className="mt-4 text-xs text-muted-foreground">
        {t("bootstrapSetupUxLab.publicInvite.note", {
          defaultValue:
