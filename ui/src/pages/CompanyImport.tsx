...
function ImportPreviewPane({
  ...
  if (!selectedFile || content === null) {
    return (
      <EmptyState icon={Package} message={t("companyImport.preview.selectFilePrompt", { defaultValue: "Select a file to preview its contents." })} />
    );
  }
      ...
      <div className="rounded-md border border-border">
        <div className="flex items-center gap-2 border-b border-border px-4 py-2.5">
          ...
          <span className="text-xs text-muted-foreground">
            {t("companyImport.conflicts.itemCount", { count: conflicts.length, defaultValue: "{{count}} item", defaultValue_other: "{{count}} items" })}
          </span>
        </div>
          ...
          {conflicts.map((item) => {
            ...
            return (
              ...
              >
                ...
                >
                  {isSkipped
                    ? t("companyImport.conflicts.skipButton.skipped", { defaultValue: "skipped" })
                    : t("companyImport.conflicts.skipButton.skip", { defaultValue: "skip" })}
                </button>
                  ...
                  >
                    {isConfirmed ? (
                      <>
                        ...
                        {t("companyImport.conflicts.confirmButton.confirmed", { defaultValue: "confirmed" })}
                      </>
                    ) : (
                      t("companyImport.conflicts.confirmButton.confirm", { defaultValue: "confirm rename" })
                    )}
                  </button>
                )}
              </div>
    ...
    <div className="mx-5 mt-3">
      <div className="rounded-md border border-border">
        <div className="flex items-center gap-2 border-b border-border px-4 py-2.5">
          ...
          <span className="text-xs text-muted-foreground">
            {t("companyImport.adapters.agentCount", { count: agents.length, defaultValue: "{{count}} agent", defaultValue_other: "{{count}} agents" })}
          </span>
        </div>
        <div className="divide-y divide-border">
          {agents.map((agent) => {
              ...
              <div key={agent.slug}>
                <div className="flex items-center gap-3 px-4 py-2.5 text-sm">
                  ...
                  >
                    ...
                    {t("companyImport.adapters.configureButton", { defaultValue: "configure adapter" })}
                  </button>
                </div>
                {isExpanded && (
...
async function readLocalPackageZip(file: File): Promise<{
  ...
  if (!/\.zip$/i.test(file.name)) {
    throw new Error(t("companyImport.errors.selectZip", { defaultValue: "Select a .zip company package." }));
  }
  const archive = await readZipArchive(await file.arrayBuffer());
  if (Object.keys(archive.files).length === 0) {
    throw new Error(t("companyImport.errors.noPackageFiles", { defaultValue: "No package files were found in the selected zip archive." }));
  }
  return {
    name: file.name,
...
export function CompanyImport() {
  ...
  const localZipHelpText =
    t("companyImport.source.local.helpText", { defaultValue: "Upload a .zip exported directly from Paperclip. Re-zipped archives created by Finder, Explorer, or other zip tools may not import correctly." });

  useEffect(() => {
    setBreadcrumbs([
      { label: t("companyImport.breadcrumbs.orgChart", { defaultValue: "Org Chart" }), href: "/org" },
      { label: t("companyImport.breadcrumbs.import", { defaultValue: "Import" }) },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  const previewMutation = useMutation({
    mutationFn: () => {
      ...
      if (!source) throw new Error(t("companyImport.errors.noSource", { defaultValue: "No source configured." }));
    ...
    onError: (err) => {
      pushToast({
        tone: "error",
        title: t("companyImport.toasts.previewFailed.title", { defaultValue: "Preview failed" }),
        body: err instanceof Error ? err.message : t("companyImport.toasts.previewFailed.body", { defaultValue: "Failed to preview import." }),
      });
    },
  });
  ...
  const importMutation = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        tone: "success",
        title: t("companyImport.toasts.importComplete.title", { defaultValue: "Import complete" }),
        body: t("companyImport.toasts.importComplete.body", {
          count: result.agents.length,
          name: result.company.name,
          defaultValue: "{{name}}: {{count}} agent processed.",
          defaultValue_other: "{{name}}: {{count}} agents processed.",
        }),
      });
    ...
    onError: (err) => {
      pushToast({
        tone: "error",
        title: t("companyImport.toasts.importFailed.title", { defaultValue: "Import failed" }),
        body: err instanceof Error ? err.message : t("companyImport.toasts.importFailed.body", { defaultValue: "Failed to apply import." }),
      });
    },
  });
  ...
  async function handleChooseLocalPackage(e: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      pushToast({
        tone: "error",
        title: t("companyImport.toasts.packageReadFailed.title", { defaultValue: "Package read failed" }),
        body: err instanceof Error ? err.message : t("companyImport.toasts.packageReadFailed.body", { defaultValue: "Failed to read folder." }),
      });
    }
  }
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Download} message={t("companyImport.selectCompanyPrompt", { defaultValue: "Select a company to import into." })} />;
  }
      ...
      <div className="border-b border-border px-5 py-5 space-y-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            {t("companyImport.source.subtitle", { defaultValue: "Choose a GitHub repo or upload a local Paperclip zip package." })}
          </p>
        </div>
        ...
        <div className="grid gap-2 md:grid-cols-2">
          {(
            [
              { key: "github", icon: Github, label: t("companyImport.source.github", { defaultValue: "GitHub repo" }) },
        ...
        {sourceMode === "local" ? (
          ...
          <Field
            label={t("companyImport.fields.githubUrl.label", { defaultValue: "GitHub URL" })}
            hint={t("companyImport.fields.githubUrl.hint", { defaultValue: "Repo tree path or blob URL to COMPANY.md (e.g. github.com/owner/repo/tree/main/company)." })}
          >
            <input
        ...
        <Field
          label={t("companyImport.fields.target.label", { defaultValue: "Target" })}
          hint={t("companyImport.fields.target.hint", { defaultValue: "Import into this company or create a new one." })}
        >
          <select
            className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
            value={targetMode}
          ...
          >
            <option value="new">{t("companyImport.fields.target.optionNew", { defaultValue: "Create new company" })}</option>
            <option value="existing">
              {t("companyImport.fields.target.optionExisting", { defaultValue: "Existing company: {{name}}", name: selectedCompany?.name ?? "" })}
            </option>
          </select>
        </Field>
        ...
        {targetMode === "new" && (
          <Field
            label={t("companyImport.fields.newCompanyName.label", { defaultValue: "New company name" })}
            hint={t("companyImport.fields.newCompanyName.hint", { defaultValue: "Optional override. Leave blank to use the package name." })}
          >
            <input
              className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
              type="text"
              ...
              placeholder={t("companyImport.fields.newCompanyName.placeholder", { defaultValue: "Imported Company" })}
            />
          </Field>
        )}

        <Field
          label={t("companyImport.fields.collisionStrategy.label", { defaultValue: "Collision strategy" })}
          hint={t("companyImport.fields.collisionStrategy.hint", { defaultValue: "Board imports can rename, skip, or replace matching company content." })}
        >
          <select
            className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
          ...
          >
            <option value="rename">{t("companyImport.fields.collisionStrategy.optionRename", { defaultValue: "Rename on conflict" })}</option>
            <option value="skip">{t("companyImport.fields.collisionStrategy.optionSkip", { defaultValue: "Skip on conflict" })}</option>
            <option value="replace">{t("companyImport.fields.collisionStrategy.optionReplace", { defaultValue: "Replace existing" })}</option>
          </select>
        </Field>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            {previewMutation.isPending
              ...
              : t("companyImport.actions.preview", { defaultValue: "Preview import" })}
          </Button>
        </div>
      </div>
          ...
          <div className="sticky top-0 z-10 border-b border-border bg-background px-5 py-3">
            <div className="flex flex-wrap items-center gap-4 text-sm">
              ...
              <span className="text-muted-foreground">
                {t("companyImport.previewBar.filesSelected", {
                  count: totalFiles,
                  selected: selectedCount,
                  total: totalFiles,
                  defaultValue: "{{selected}} / {{total}} file selected",
                  defaultValue_other: "{{selected}} / {{total}} files selected",
                })}
              </span>
              {conflicts.length > 0 && (
                <span className="text-amber-500">
                  {t("companyImport.previewBar.conflictCount", { count: conflicts.length, defaultValue: "{{count}} conflict", defaultValue_other: "{{count}} conflicts" })}
                </span>
              )}
              {importPreview.errors.length > 0 && (
                <span className="text-destructive">
                  {t("companyImport.previewBar.errorCount", { count: importPreview.errors.length, defaultValue: "{{count}} error", defaultValue_other: "{{count}} errors" })}
                </span>
              )}
            </div>
          </div>
          ...
          <div className="mx-5 mt-3 flex flex-wrap justify-end gap-2">
            ...
            >
              ...
              {importMutation.isPending
                ...
                : t("companyImport.actions.importCount", { count: selectedCount, defaultValue: "Import {{count}} file", defaultValue_other: "Import {{count}} files" })}
            </Button>
          </div>
          ...
          <div className="grid gap-4 xl:h-(--sz-calc-31) xl:grid-cols-(--gtc-25) xl:gap-0">
            <aside className="flex max-h-(--sz-24rem) flex-col overflow-hidden border-b border-border xl:max-h-none xl:border-b-0 xl:border-r">
              <div className="border-b border-border px-4 py-3 shrink-0">
                <h2 className="text-base font-semibold">{t("companyImport.packageFiles.title", { defaultValue: "Package files" })}</h2>
              </div>
