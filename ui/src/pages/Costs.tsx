  ...
  return (
    <Card>
      <CardHeader className="px-5 pt-5 pb-2">
        <CardTitle className="text-base">{t("costs.finance.summary.title", { defaultValue: "Finance ledger" })}</CardTitle>
        <CardDescription>
          {t("costs.finance.summary.description", { defaultValue: "Account-level charges that do not map to a single inference request." })}
        </CardDescription>
      </CardHeader>
      <CardContent className="grid gap-3 px-5 pb-5 pt-2 sm:grid-cols-2 xl:grid-cols-4">
        <MetricTile
          ...
          subtitle={t("costs.finance.debits.subtitle", {
...
export function Costs() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("costs.title", { defaultValue: "Costs" }) }]);
  }, [setBreadcrumbs, t]);
    ...
    return [
      {
        ...
        label: (
          <span className="flex items-center gap-1.5">
            <span>{t("costs.providers.all", { defaultValue: "All providers" })}</span>
    ...
    return [
      {
        ...
        label: (
          <span className="flex items-center gap-1.5">
            <span>{t("costs.billers.all", { defaultValue: "All billers" })}</span>
            {billerKeys.length > 0 ? (
              <>
                <span className="font-mono text-xs text-muted-foreground">{formatTokens(allTokens)}</span>
                <span className="text-xs text-muted-foreground">{formatCents(allCents)}</span>
    ...
    <div className="space-y-6">
            ...
            <MetricTile
              ...
              value={activeBudgetIncidents.length > 0 ? String(activeBudgetIncidents.length) : (
                spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                  ...
                  : t("costs.metrics.budget.valueOpen", { defaultValue: "Open" })
              )}
              subtitle={
                activeBudgetIncidents.length > 0
                  ? t("costs.metrics.budget.subtitleIncidents", {
                  ...
                  : spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                    ? t("costs.metrics.budget.subtitleUtilization", {
                    ...
                    : t("costs.metrics.budget.subtitleNoCap", { defaultValue: "No monthly cap configured" })
              }
              icon={Coins}
            />
            <MetricTile
      ...
      <Tabs value={mainTab} onValueChange={(value) => setMainTab(value as typeof mainTab)}>
        ...
        <TabsContent value="overview" className="mt-4 space-y-4">
          {showCustomPrompt ? (
            <p className="text-sm text-muted-foreground">{t("costs.customPrompt", { defaultValue: "Select a start and end date to load data." })}</p>
          ) : showOverviewLoading ? (
                    ...
                    <div className="flex flex-wrap items-end justify-between gap-3">
                      <div>
                        ...
                        <div className="mt-1 text-sm text-muted-foreground">
                          {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                            ? t("costs.overview.inferenceLedger.budgetAmount", {
                                defaultValue: "Budget {{amount}}",
                                amount: formatCents(spendData.summary.budgetCents),
                              })
                            : t("costs.overview.inferenceLedger.unlimitedBudget", { defaultValue: "Unlimited budget" })}
                        </div>
                      </div>
                      <div className="border border-border px-4 py-3 text-right">
                        <div className="text-(length:--text-micro) uppercase tracking-(--tracking-eyebrow) text-muted-foreground">{t("costs.overview.inferenceLedger.usageLabel", { defaultValue: "usage" })}</div>
                    ...
                    {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0 ? (
                      <div className="space-y-2">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {t("costs.overview.inferenceLedger.consumed", {
                            ...
                            >
                              ...
                              <div className="text-right text-sm tabular-nums">
                                ...
                                {(row.apiRunCount > 0 || row.subscriptionRunCount > 0) ? (
                                  <div className="text-xs text-muted-foreground">
                                    {row.apiRunCount > 0
                                      ? t("costs.overview.byAgent.apiRuns", { defaultValue: "{{n}} api", n: row.apiRunCount })
                                      : t("costs.overview.byAgent.apiRuns", { defaultValue: "{{n}} api", n: 0 })}
                                    {" · "}
                                    {row.subscriptionRunCount > 0
                                      ? t("costs.overview.byAgent.subscriptionRuns", { defaultValue: "{{n}} subscription", n: row.subscriptionRunCount })
                                      : t("costs.overview.byAgent.subscriptionRuns", { defaultValue: "{{n}} subscription", n: 0 })}
                                  </div>
                                ) : null}
                              </div>
                            </div>
                                    ...
                                    >
                                      ...
                                      <div className="text-right tabular-nums">
                                        ...
                                        <div className="text-muted-foreground">
                                          {t("costs.overview.byAgent.modelTokens", {
                                            defaultValue: "{{tokens}} tok",
                ...
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      ...
                      <CardDescription>{t("costs.overview.byProject.description", { defaultValue: "Run costs attributed through project-linked tasks." })}</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-2 px-5 pb-5 pt-2">
                  ...
                  <FinanceTimelineCard rows={topFinanceEvents.slice(0, 6)} emptyMessage={t("costs.overview.financeTimeline.empty", { defaultValue: "No finance events yet. Add account-level charges once biller invoices or credits land." })} />
                </div>
              </div>
            </>
          )}
                ...
                {(["company", "agent", "project"] as const).map((scopeType) => {
                  ...
                  return (
                    <section key={scopeType} className="space-y-3">
                      <div>
                        <h2 className="text-lg font-semibold capitalize">{t("costs.budgets.scope.heading", { defaultValue: "{{scope}} budgets", scope: scopeType })}</h2>
                        <p className="text-sm text-muted-foreground">
                          {scopeType === "company"
                            ? t("costs.budgets.scope.description.company", { defaultValue: "Company-wide monthly policy." })
                            : scopeType === "agent"
                              ? t("costs.budgets.scope.description.agent", { defaultValue: "Recurring monthly spend policies for individual agents." })
                              : t("costs.budgets.scope.description.project", { defaultValue: "Lifetime spend policies for execution-bound projects." })}
                        </p>
                      </div>
                      <div className="grid gap-4 xl:grid-cols-2">
                        {rows.map((summary) => (
                ...
                {budgetPolicies.length === 0 ? (
                  <Card>
                    <CardContent className="px-5 py-8 text-sm text-muted-foreground">
                      {t("costs.budgets.empty", { defaultValue: "No budget policies yet. Set agent and project budgets from their detail pages, or use the existing company monthly budget control." })}
                    </CardContent>
                  </Card>
                ) : null}
              </div>
