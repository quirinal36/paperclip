...
function buildExecutionWorkspaceBaseTabItems(
  t: ReturnType<typeof useTranslation>["t"],
): OrderedExecutionWorkspaceTabItem[] {
  return [
    { value: "issues", label: t("executionWorkspaceDetail.tabs.tasks", { defaultValue: "Tasks" }), order: 10 },
    { value: "services", label: t("executionWorkspaceDetail.tabs.services", { defaultValue: "Services" }), order: 20 },
    { value: "configuration", label: t("executionWorkspaceDetail.tabs.configuration", { defaultValue: "Configuration" }), order: 30 },
    { value: "runtime_logs", label: t("executionWorkspaceDetail.tabs.runtimeLogs", { defaultValue: "Runtime logs" }), order: 40 },
    { value: "routines", label: t("executionWorkspaceDetail.tabs.routines", { defaultValue: "Routines" }), order: 60 },
  ];
}
...
function formatOptionalDateTime(value: Date | string | null | undefined) {
  return value ? formatDateTime(value) : t("executionWorkspaceDetail.values.never", { defaultValue: "Never" });
}
...
function parseWorkspaceRuntimeJson(value: string) {
  ...
  try {
    ...
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
      return {
        ok: false as const,
        error: t("executionWorkspaceDetail.errors.runtimeJsonNotObject", { defaultValue: "Workspace commands JSON must be a JSON object." }),
      };
    }
    ...
    return {
      ok: false as const,
      error: error instanceof Error ? error.message : t("executionWorkspaceDetail.errors.invalidJson", { defaultValue: "Invalid JSON." }),
    };
  }
}
  ...
  const addServices = (collection: ConfiguredRuntimeServicePort["collection"], services: unknown, commandsRequireServiceKind: boolean) => {
    ...
    services.forEach((service, index) => {
      ...
      entries.push({
        collection,
        index,
        name: typeof config.name === "string" && config.name.trim() ? config.name : t("executionWorkspaceDetail.services.defaultName", { defaultValue: "Service {{number}}", number: index + 1 }),
...
export function getConfiguredRuntimeServicePortWarnings(services: ConfiguredRuntimeServicePort[]) {
  ...
  return Array.from(servicesByPort.entries())
    ...
    .map(([port, servicesForPort]) =>
      t("executionWorkspaceDetail.warnings.portConflict", {
        defaultValue: "Port {{port}} is assigned to multiple services: {{services}}.",
        port,
...
function validateForm(form: WorkspaceFormState) {
  ...
  if (repoUrl) {
    try {
      ...
      return t("executionWorkspaceDetail.errors.invalidRepoUrl", { defaultValue: "Repo URL must be a valid URL." });
    }
  }
  ...
  if (!form.inheritRuntime) {
    ...
    if (invalidPort) return t("executionWorkspaceDetail.errors.invalidFixedPort", { defaultValue: "{{name}} has an invalid fixed port.", name: invalidPort.name });
  }
...
function workspaceOperationPhaseLabel(phase: string) {
  switch (phase) {
    case "worktree_prepare":
      return t("executionWorkspaceDetail.operationPhase.worktreePrepare", { defaultValue: "Worktree setup" });
    case "workspace_config_freshness":
      return t("executionWorkspaceDetail.operationPhase.configFreshness", { defaultValue: "Config freshness" });
    case "workspace_provision":
      return t("executionWorkspaceDetail.operationPhase.provision", { defaultValue: "Provision" });
    case "workspace_teardown":
      return t("executionWorkspaceDetail.operationPhase.teardown", { defaultValue: "Teardown" });
    case "worktree_cleanup":
      return t("executionWorkspaceDetail.operationPhase.worktreeCleanup", { defaultValue: "Worktree cleanup" });
    case "workspace_finalize":
      return t("executionWorkspaceDetail.operationPhase.finalize", { defaultValue: "Finalize" });
    default:
      return phase;
  }
}
...
function MonoValue({ value, copy }: { value: string; copy?: boolean }) {
  ...
  return (
    <div className="inline-flex max-w-full items-start gap-2">
      ...
      {copy ? (
        <CopyText text={value} className="shrink-0 text-muted-foreground hover:text-foreground" copiedLabel={t("executionWorkspaceDetail.copy.copied", { defaultValue: "Copied" })}>
    ...
    <div className="flex flex-col gap-3 border-b border-border px-3 py-3 last:border-b-0 sm:flex-row sm:items-center">
      <div className="min-w-0 flex-1 space-y-1.5">
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
          <span>{routine.assigneeAgentId ? t("executionWorkspaceDetail.routines.defaultAgentSet", { defaultValue: "Default agent set" }) : t("executionWorkspaceDetail.routines.chooseAgentWhenRunning", { defaultValue: "Choose agent when running" })}</span>
          <span>{t("executionWorkspaceDetail.routines.lastRun", { defaultValue: "Last run {{time}}", time: formatOptionalDateTime(routine.lastRun?.triggeredAt ?? routine.lastTriggeredAt) })}</span>
      ...
      >
        ...
        {isRunning ? t("executionWorkspaceDetail.routines.running", { defaultValue: "Running..." }) : t("executionWorkspaceDetail.routines.runNow", { defaultValue: "Run now" })}
      </Button>
    </div>
  );
}
...
function ExecutionWorkspaceRoutinesList({
  ...
  const runRoutine = useMutation({
    ...
    onSuccess: async (_, { id }) => {
      ...
      pushToast({
        title: t("executionWorkspaceDetail.routines.startedToast.title", { defaultValue: "Routine started" }),
        body: t("executionWorkspaceDetail.routines.startedToast.body", { defaultValue: "Paperclip created a run using this execution workspace." }),
        tone: "success",
      });
    },
    onSettled: () => {
    ...
    onError: (mutationError) => {
      pushToast({
        title: t("executionWorkspaceDetail.routines.runFailedToast.title", { defaultValue: "Routine run failed" }),
        body: mutationError instanceof Error ? mutationError.message : t("executionWorkspaceDetail.errors.routineRunFailed", { defaultValue: "Paperclip could not start the routine run." }),
        tone: "error",
      });
    },
  });
  ...
  return (
    <>
      <Card className="rounded-none">
        <CardHeader>
          <CardTitle>{t("executionWorkspaceDetail.routines.card.title", { defaultValue: "Workspace routines" })}</CardTitle>
          <CardDescription>
            {t("executionWorkspaceDetail.routines.card.description", { defaultValue: "Routines that use workspace-specific variables can be run against this execution workspace." })}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            <p className="text-sm text-muted-foreground">{t("executionWorkspaceDetail.routines.loading", { defaultValue: "Loading routines..." })}</p>
          ) : error ? (
            <p className="text-sm text-destructive">
              {error instanceof Error ? error.message : t("executionWorkspaceDetail.errors.loadRoutines", { defaultValue: "Failed to load routines." })}
            </p>
          ) : workspaceRoutines.length === 0 ? (
            <div className="flex flex-col items-center gap-2 py-10 text-center">
              <Repeat className="h-5 w-5 text-muted-foreground" />
              <p className="text-sm text-muted-foreground">
                {t("executionWorkspaceDetail.routines.empty", { defaultValue: "No routines use workspace-specific variables yet." })}
              </p>
            </div>
          ) : (
...
export function ExecutionWorkspaceDetail() {
  ...
  const workspaceTabItems = useMemo(
    () => orderExecutionWorkspaceTabItems([...buildExecutionWorkspaceBaseTabItems(t), ...workspacePluginTabItems]),
  ...
  useEffect(() => {
    if (!workspace) return;
    const crumbs = [
      { label: t("executionWorkspaceDetail.breadcrumbs.projects", { defaultValue: "Projects" }), href: "/projects" },
      ...(project ? [{ label: project.name, href: `/projects/${projectRef}` }] : []),
      ...(project ? [{ label: t("executionWorkspaceDetail.breadcrumbs.workspaces", { defaultValue: "Workspaces" }), href: `/projects/${projectRef}/workspaces` }] : []),
  ...
  const updateWorkspace = useMutation({
    ...
    onError: (error) => {
      setErrorMessage(error instanceof Error ? error.message : t("executionWorkspaceDetail.errors.saveFailed", { defaultValue: "Failed to save execution workspace." }));
    },
  });
  ...
  const controlRuntimeServices = useMutation({
    ...
    onSuccess: (result, request) => {
      ...
      setRuntimeActionMessage(
        request.action === "run"
          ? t("executionWorkspaceDetail.runtimeActions.jobCompleted", { defaultValue: "Workspace job completed." })
          : request.action === "stop"
            ? t("executionWorkspaceDetail.runtimeActions.serviceStopped", { defaultValue: "Workspace service stopped." })
            : request.action === "restart"
              ? t("executionWorkspaceDetail.runtimeActions.serviceRestarted", { defaultValue: "Workspace service restarted." })
              : t("executionWorkspaceDetail.runtimeActions.serviceStarted", { defaultValue: "Workspace service started." }),
      );
    },
    onError: (error) => {
      setRuntimeActionMessage(null);
      setRuntimeActionErrorMessage(error instanceof Error ? error.message : t("executionWorkspaceDetail.errors.controlFailed", { defaultValue: "Failed to control workspace commands." }));
    },
  ...
  if (workspaceQuery.error) {
    return (
      <p className="text-sm text-destructive">
        {workspaceQuery.error instanceof Error ? workspaceQuery.error.message : t("executionWorkspaceDetail.errors.loadWorkspace", { defaultValue: "Failed to load workspace" })}
      </p>
    );
  }
  ...
  const saveChanges = () => {
    ...
    try {
      ...
      setErrorMessage(error instanceof Error ? error.message : t("executionWorkspaceDetail.errors.buildUpdateFailed", { defaultValue: "Failed to build workspace update." }));
      return;
    }
      ...
      <div className="space-y-4 overflow-hidden sm:space-y-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0 space-y-2">
            <div className="text-xs font-medium uppercase tracking-(--tracking-eyebrow) text-muted-foreground">
              {t("executionWorkspaceDetail.eyebrow", { defaultValue: "Execution workspace" })}
            </div>
        ...
        {activeTab === "services" ? (
          <WorkspaceRuntimeControls
            ...
            serviceEmptyMessage={
              effectiveRuntimeConfig
                ? t("executionWorkspaceDetail.servicesTab.serviceEmptyWithConfig", { defaultValue: "No services have been started for this execution workspace yet." })
                : t("executionWorkspaceDetail.servicesTab.serviceEmptyNoConfig", { defaultValue: "No workspace command config is defined for this execution workspace yet." })
            }
            jobEmptyMessage={t("executionWorkspaceDetail.servicesTab.jobEmpty", { defaultValue: "No one-shot jobs are configured for this execution workspace yet." })}
            disabledHint={
              canStartRuntimeServices
                ? null
                : t("executionWorkspaceDetail.servicesTab.disabledHint", { defaultValue: "Execution workspaces need a working directory before local commands can run, and services also need runtime config." })
            }
          ...
          <div className="space-y-4 sm:space-y-6">
            <Card className="rounded-none">
              <CardHeader>
                <CardTitle>{t("executionWorkspaceDetail.settings.card.title", { defaultValue: "Workspace settings" })}</CardTitle>
                <CardDescription>
                  {t("executionWorkspaceDetail.settings.card.description", { defaultValue: "Edit the concrete path, repo, branch, provisioning, teardown, and runtime overrides attached to this execution workspace. Saved changes affect future runs; Paperclip may refresh or replace a reused workspace when config changes." })}
                </CardDescription>
                <CardAction>
                  <Button
                    variant="destructive"
                  ...
                  >
                    {workspace.status === "cleanup_failed" ? t("executionWorkspaceDetail.settings.retryClose", { defaultValue: "Retry close" }) : t("executionWorkspaceDetail.settings.closeWorkspace", { defaultValue: "Close workspace" })}
                  </Button>
                </CardAction>
              </CardHeader>
              ...
              <div className="space-y-6">
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("executionWorkspaceDetail.settings.sections.general", { defaultValue: "General" })}</div>
                  <Field label={t("executionWorkspaceDetail.settings.fields.name.label", { defaultValue: "Workspace name" })}>
                    <Input
                      value={form.name}
                      onChange={(event) => setForm((current) => current ? { ...current, name: event.target.value } : current)}
                      placeholder={t("executionWorkspaceDetail.settings.fields.name.placeholder", { defaultValue: "Execution workspace name" })}
                    />
                  </Field>
                </div>
                ...
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("executionWorkspaceDetail.settings.sections.sourceControl", { defaultValue: "Source control" })}</div>
                  <div className="grid gap-4 sm:grid-cols-2">
                    <Field label={t("executionWorkspaceDetail.settings.fields.branchName.label", { defaultValue: "Branch name" })} hint={t("executionWorkspaceDetail.settings.fields.branchName.hint", { defaultValue: "Useful for isolated worktrees" })}>
                      <Input
                        className="font-mono"
                    ...
                    <Field label={t("executionWorkspaceDetail.settings.fields.baseRef.label", { defaultValue: "Base ref" })}>
                      <Input
                        className="font-mono"
                        value={form.baseRef}
                  ...
                  <Field label={t("executionWorkspaceDetail.settings.fields.repoUrl.label", { defaultValue: "Repo URL" })}>
                    <Input
                      value={form.repoUrl}
                ...
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("executionWorkspaceDetail.settings.sections.paths", { defaultValue: "Paths" })}</div>
                  <Field label={t("executionWorkspaceDetail.settings.fields.workingDirectory.label", { defaultValue: "Working directory" })}>
                    <Input
                      className="font-mono"
                      value={form.cwd}
                  ...
                  <Field label={t("executionWorkspaceDetail.settings.fields.providerRef.label", { defaultValue: "Provider path / ref" })}>
                    <Input
                      className="font-mono"
                ...
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("executionWorkspaceDetail.settings.sections.lifecycleCommands", { defaultValue: "Lifecycle commands" })}</div>
                  <Field label={t("executionWorkspaceDetail.settings.fields.provisionCommand.label", { defaultValue: "Provision command" })} hint={t("executionWorkspaceDetail.settings.fields.provisionCommand.hint", { defaultValue: "Runs when Paperclip prepares this execution workspace" })}>
                    <Textarea
                  ...
                  <Field label={t("executionWorkspaceDetail.settings.fields.teardownCommand.label", { defaultValue: "Teardown command" })} hint={t("executionWorkspaceDetail.settings.fields.teardownCommand.hint", { defaultValue: "Runs when the execution workspace is archived or cleaned up" })}>
                    <Textarea
                      className="min-h-20 font-mono"
                  ...
                  <Field label={t("executionWorkspaceDetail.settings.fields.cleanupCommand.label", { defaultValue: "Cleanup command" })} hint={t("executionWorkspaceDetail.settings.fields.cleanupCommand.hint", { defaultValue: "Workspace-specific cleanup before teardown" })}>
                    <Textarea
                ...
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("executionWorkspaceDetail.settings.sections.runtimeConfig", { defaultValue: "Runtime config" })}</div>
                  <div className="rounded-md border border-dashed border-border/70 bg-background px-4 py-3">
                    <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
                      <div className="space-y-1">
                        <div className="text-sm font-medium text-foreground">
                          {t("executionWorkspaceDetail.settings.runtimeSource.title", { defaultValue: "Runtime config source" })}
                        </div>
                        <p className="text-sm text-muted-foreground">
                          {runtimeConfigSource === "execution_workspace"
                            ? t("executionWorkspaceDetail.settings.runtimeSource.executionWorkspace", { defaultValue: "This execution workspace currently overrides the project workspace runtime config." })
                            : runtimeConfigSource === "project_workspace"
                              ? t("executionWorkspaceDetail.settings.runtimeSource.projectWorkspace", { defaultValue: "This execution workspace is inheriting the project workspace runtime config." })
                              : t("executionWorkspaceDetail.settings.runtimeSource.none", { defaultValue: "No runtime config is currently defined on this execution workspace or its project workspace." })}
                        </p>
                      </div>
                      <Button
                        variant="outline"
                      ...
                      >
                        {t("executionWorkspaceDetail.settings.resetToInherit", { defaultValue: "Reset to inherit" })}
                      </Button>
                    </div>
                  </div>

                  <details className="rounded-md border border-dashed border-border/70 bg-background px-4 py-3">
                    <summary className="cursor-pointer text-sm font-medium">{t("executionWorkspaceDetail.settings.advancedRuntime.summary", { defaultValue: "Advanced runtime JSON" })}</summary>
                    <p className="mt-2 text-sm text-muted-foreground">
                      {t("executionWorkspaceDetail.settings.advancedRuntime.description", { defaultValue: "Override the inherited workspace command model only when this execution workspace truly needs different service or job behavior." })}
                    </p>
                    <div className="mt-3">
                      <Field label={t("executionWorkspaceDetail.settings.fields.workspaceCommandsJson.label", { defaultValue: "Workspace commands JSON" })} hint={t("executionWorkspaceDetail.settings.fields.workspaceCommandsJson.hint", { defaultValue: "Legacy `services` arrays still work, but `commands` supports both services and jobs." })}>
                        <div className="mb-2 flex items-center gap-2 text-sm text-muted-foreground">
                          ...
                          />
                          <label htmlFor="inherit-runtime-config">{t("executionWorkspaceDetail.settings.inheritRuntimeCheckbox", { defaultValue: "Inherit project workspace runtime config" })}</label>
                        </div>
                        <Textarea
                  ...
                  {configuredRuntimeServicePorts.length > 0 ? (
                    <div className="space-y-3 rounded-md border border-border bg-muted/20 p-4">
                      <div>
                        <div className="text-sm font-medium">{t("executionWorkspaceDetail.settings.servicePorts.title", { defaultValue: "Service ports" })}</div>
                        <p className="mt-1 text-sm text-muted-foreground">
                          {t("executionWorkspaceDetail.settings.servicePorts.description", { defaultValue: "Set a fixed port for a service or leave it blank to use its configured automatic behavior. Editing an inherited service creates an execution-workspace runtime override." })}
                        </p>
                      </div>
                      <div className="grid gap-3 sm:grid-cols-2">
                        {configuredRuntimeServicePorts.map((service) => (
                          <Field key={`${service.collection}-${service.index}`} label={service.name} hint={t("executionWorkspaceDetail.settings.servicePorts.fixedPortHint", { defaultValue: "Fixed port" })}>
                            <Input
                              type="number"
                              min="1"
                              max="65535"
                      ...
                      <p className="text-sm text-muted-foreground">
                        {t("executionWorkspaceDetail.settings.servicePorts.recheckNote", { defaultValue: "Paperclip checks fixed ports again when a service starts and rejects cross-workspace conflicts." })}
                      </p>
                    </div>
                  ) : null}
                </div>
              ...
              <div className="mt-6 flex flex-col items-stretch gap-3 sm:flex-row sm:flex-wrap sm:items-center">
                <Button className="w-full sm:w-auto" disabled={!isDirty || updateWorkspace.isPending} onClick={saveChanges}>
                  ...
                  {t("executionWorkspaceDetail.settings.saveChanges", { defaultValue: "Save changes" })}
                </Button>
                <Button
                  variant="outline"
                  className="w-full sm:w-auto"
                ...
                >
                  {t("executionWorkspaceDetail.settings.reset", { defaultValue: "Reset" })}
                </Button>
                {errorMessage ? <p className="text-sm text-destructive">{errorMessage}</p> : null}
                {!errorMessage && !isDirty ? <p className="text-sm text-muted-foreground">{t("executionWorkspaceDetail.settings.noUnsavedChanges", { defaultValue: "No unsaved changes." })}</p> : null}
              </div>
              </CardContent>
            </Card>
            ...
            <Card className="rounded-none">
              <CardHeader>
                <CardTitle>{t("executionWorkspaceDetail.context.card.title", { defaultValue: "Workspace context" })}</CardTitle>
                <CardDescription>{t("executionWorkspaceDetail.context.card.description", { defaultValue: "Linked objects and relationships" })}</CardDescription>
              </CardHeader>
              <CardContent>
              <DetailRow label={t("executionWorkspaceDetail.context.rows.project", { defaultValue: "Project" })}>
                {project ? <Link to={`/projects/${projectRef}`} className="hover:underline">{project.name}</Link> : <MonoValue value={workspace.projectId} />}
              </DetailRow>
              <DetailRow label={t("executionWorkspaceDetail.context.rows.projectWorkspace", { defaultValue: "Project workspace" })}>
                {project && linkedProjectWorkspace ? (
                  ...
                  t("executionWorkspaceDetail.values.none", { defaultValue: "None" })
                )}
              </DetailRow>
              <DetailRow label={t("executionWorkspaceDetail.context.rows.sourceTask", { defaultValue: "Source task" })}>
                {sourceIssue ? (
              ...
              <DetailRow label={t("executionWorkspaceDetail.context.rows.derivedFrom", { defaultValue: "Derived from" })}>
                {derivedWorkspace ? (
              ...
              <DetailRow label={t("executionWorkspaceDetail.context.rows.workspaceId", { defaultValue: "Workspace ID" })}>
            ...
            <Card className="rounded-none">
              <CardHeader>
                <CardTitle>{t("executionWorkspaceDetail.location.card.title", { defaultValue: "Concrete location" })}</CardTitle>
                <CardDescription>{t("executionWorkspaceDetail.location.card.description", { defaultValue: "Paths and refs" })}</CardDescription>
              </CardHeader>
              <CardContent>
              <DetailRow label={t("executionWorkspaceDetail.location.rows.workingDir", { defaultValue: "Working dir" })}>
                {workspace.cwd ? <MonoValue value={workspace.cwd} copy /> : t("executionWorkspaceDetail.values.none", { defaultValue: "None" })}
              </DetailRow>
              <DetailRow label={t("executionWorkspaceDetail.location.rows.providerRef", { defaultValue: "Provider ref" })}>
                {workspace.providerRef ? <MonoValue value={workspace.providerRef} copy /> : t("executionWorkspaceDetail.values.none", { defaultValue: "None" })}
              </DetailRow>
              <DetailRow label={t("executionWorkspaceDetail.location.rows.repoUrl", { defaultValue: "Repo URL" })}>
                {workspace.repoUrl && isSafeExternalUrl(workspace.repoUrl) ? (
                  <div className="inline-flex max-w-full items-start gap-2">
                    ...
                    <CopyText text={workspace.repoUrl} className="shrink-0 text-muted-foreground hover:text-foreground" copiedLabel={t("executionWorkspaceDetail.copy.copied", { defaultValue: "Copied" })}>
                      <Copy className="h-3.5 w-3.5" />
                    </CopyText>
                  </div>
                ) : workspace.repoUrl ? (
              ...
              <DetailRow label={t("executionWorkspaceDetail.location.rows.baseRef", { defaultValue: "Base ref" })}>
                {workspace.baseRef ? <MonoValue value={workspace.baseRef} copy /> : t("executionWorkspaceDetail.values.none", { defaultValue: "None" })}
              </DetailRow>
              <DetailRow label={t("executionWorkspaceDetail.location.rows.branch", { defaultValue: "Branch" })}>
                {workspace.branchName ? <MonoValue value={workspace.branchName} copy /> : t("executionWorkspaceDetail.values.none", { defaultValue: "None" })}
              </DetailRow>
              <DetailRow label={t("executionWorkspaceDetail.location.rows.opened", { defaultValue: "Opened" })}>{formatDateTime(workspace.openedAt)}</DetailRow>
              <DetailRow label={t("executionWorkspaceDetail.location.rows.lastUsed", { defaultValue: "Last used" })}>{formatDateTime(workspace.lastUsedAt)}</DetailRow>
              <DetailRow label={t("executionWorkspaceDetail.location.rows.cleanup", { defaultValue: "Cleanup" })}>
                {workspace.cleanupEligibleAt
                  ? `${formatDateTime(workspace.cleanupEligibleAt)}${workspace.cleanupReason ? ` · ${workspace.cleanupReason}` : ""}`
                  : t("executionWorkspaceDetail.values.notScheduled", { defaultValue: "Not scheduled" })}
              </DetailRow>
              </CardContent>
            </Card>
          </div>
          ...
          <Card className="rounded-none">
            <CardHeader>
              <CardTitle>{t("executionWorkspaceDetail.logs.card.title", { defaultValue: "Runtime and cleanup logs" })}</CardTitle>
              <CardDescription>{t("executionWorkspaceDetail.logs.card.description", { defaultValue: "Recent operations" })}</CardDescription>
            </CardHeader>
            <CardContent>
            {workspaceOperationsQuery.isLoading ? (
              <p className="text-sm text-muted-foreground">{t("executionWorkspaceDetail.logs.loading", { defaultValue: "Loading workspace operations…" })}</p>
            ) : workspaceOperationsQuery.error ? (
              <p className="text-sm text-destructive">
                {workspaceOperationsQuery.error instanceof Error
                  ? workspaceOperationsQuery.error.message
                  : t("executionWorkspaceDetail.errors.loadOperations", { defaultValue: "Failed to load workspace operations." })}
              </p>
              ...
              <p className="text-sm text-muted-foreground">{t("executionWorkspaceDetail.logs.empty", { defaultValue: "No workspace operations have been recorded yet." })}</p>
            )}
            </CardContent>
          </Card>
        ) : activeTab === "issues" ? (
          <div className="space-y-6">
            {workspace.projectWorkspaceId ? (
              <SummarySlotCard
                ...
                title={t("executionWorkspaceDetail.issues.summaryTitle", { defaultValue: "Workspace summary" })}
                description={t("executionWorkspaceDetail.issues.summaryDescription", { defaultValue: "Summarizer keeps the latest workspace status, next step, and operator-needed items here." })}
              />
            ) : null}
          ...
          <Card>
            <CardContent className="py-6 text-sm text-muted-foreground">{t("executionWorkspaceDetail.plugin.loading", { defaultValue: "Loading workspace plugin..." })}</CardContent>
          </Card>
          ...
          <MissingPluginTabPlaceholder
            ...
            defaultTabLabel={t("executionWorkspaceDetail.plugin.backToTasks", { defaultValue: "Back to tasks" })}
          />
        ) : activeTab === "routines" ? (
