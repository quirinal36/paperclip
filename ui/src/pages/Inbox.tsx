...
function runFailureMessage(run: HeartbeatRun): string {
  return firstNonEmptyLine(run.error) ?? firstNonEmptyLine(run.stderrExcerpt) ?? t("inbox.errors.runExited", { defaultValue: "Run exited with an error." });
}
...
export function formatJoinRequestInboxLabel(
  ...
  if (joinRequest.requestType !== "human") {
    return joinRequest.agentName
      ? t("inbox.joinRequest.agentWithName", { defaultValue: "Agent join request: {{name}}", name: joinRequest.agentName })
      : t("inbox.joinRequest.agent", { defaultValue: "Agent join request" });
  }
  ...
  return t("inbox.joinRequest.human", { defaultValue: "Human join request" });
}
      ...
      <div className="flex items-start gap-2 sm:items-center">
          ...
          <span className="hidden sm:inline-flex h-4 w-4 shrink-0 items-center justify-center self-center">
            {showUnreadDot ? (
              <button
                ...
                aria-label={t("inbox.aria.markAsRead", { defaultValue: "Mark as read" })}
              >
                <span className={cn(
              ...
              <button
                ...
                aria-label={t("inbox.aria.dismissFromInbox", { defaultValue: "Dismiss from inbox" })}
              >
          ...
          <span className="min-w-0 flex-1">
            <span className="line-clamp-2 text-sm font-medium sm:truncate sm:line-clamp-none">
              {issue ? (
                ...
                linkedAgentName
                  ? t("inbox.failedRun.labelWithAgent", { defaultValue: "Failed run — {{agent}}", agent: linkedAgentName })
                  : t("inbox.failedRun.label", { defaultValue: "Failed run" })
              )}
            </span>
        ...
        <div className="hidden shrink-0 items-center gap-2 sm:flex">
          ...
          >
            ...
            {isRetrying ? t("inbox.actions.retrying", { defaultValue: "Retrying…" }) : t("inbox.actions.retry", { defaultValue: "Retry" })}
          </Button>
          {!showUnreadSlot && (
            <button
              type="button"
              ...
              aria-label={t("inbox.aria.dismiss", { defaultValue: "Dismiss" })}
            >
              <X className="h-4 w-4" />
            </button>
          )}
        ...
        {showResolutionButtons ? (
          <div className="hidden shrink-0 items-center gap-2 sm:flex">
            <Button
              ...
              disabled={isPending}
            >
              {t("inbox.actions.approve", { defaultValue: "Approve" })}
            </Button>
            <Button
              variant="destructive"
              size="sm"
              ...
              disabled={isPending}
            >
              {t("inbox.actions.reject", { defaultValue: "Reject" })}
            </Button>
          </div>
        ) : null}
      </div>
...
export function Inbox() {
  ...
  const creatorOptions = useMemo<CreatorOption[]>(() => {
    ...
    if (currentUserId) {
      options.set(`user:${currentUserId}`, {
        ...
        label: currentUserId === "local-board" ? t("inbox.creator.board", { defaultValue: "Board" }) : t("inbox.creator.me", { defaultValue: "Me" }),
        kind: "user",
  ...
  const archiveIssueMutation = useMutation({
    ...
    onError: (err, id, context) => {
      setActionError(err instanceof Error ? err.message : t("inbox.errors.archiveFailed", { defaultValue: "Failed to archive task" }));
    ...
    <div className="space-y-6">
      <div className="space-y-2">
        ...
        <div className="relative sm:hidden">
          ...
          <Input
            type="search"
            placeholder={t("inbox.search.placeholder", { defaultValue: "Search inbox…" })}
            value={searchQuery}
          ...
          {tab === "blocked" ? (
            <>
              ...
              <IssueColumnPicker
                ...
                title={t("inbox.columns.pickerTitle", { defaultValue: "Choose which inbox columns stay visible" })}
                iconOnly
              />
              <Popover>
                <PopoverTrigger asChild>
            ...
            <>
                ...
                <PopoverContent align="end" className="w-40 p-2">
                  <div className="space-y-0.5">
                    {([
                      ...
                      ["assignee", t("inbox.groupBy.assignee", { defaultValue: "Responsible" })],
              ...
              {canMarkAllRead && (
                <>
                  ...
                  >
                    {markAllReadMutation.isPending ? t("inbox.actions.marking", { defaultValue: "Marking…" }) : t("inbox.actions.markAllRead", { defaultValue: "Mark all as read" })}
                  </Button>
                      ...
                      <DialogHeader>
                        ...
                        <DialogDescription>
                          {unreadIssueIds.length === 1
                            ? t("inbox.markAllRead.descriptionOne", { defaultValue: "This will mark {{count}} unread item as read.", count: unreadIssueIds.length })
                            : t("inbox.markAllRead.descriptionOther", { defaultValue: "This will mark {{count}} unread items as read.", count: unreadIssueIds.length })}
                        </DialogDescription>
                      </DialogHeader>
                      <DialogFooter>
                        <Button variant="outline" onClick={() => setShowMarkAllReadConfirm(false)}>
                          {t("inbox.actions.cancel", { defaultValue: "Cancel" })}
                        </Button>
                        <Button
                          onClick={() => {
                        ...
                        >
                          {t("inbox.actions.markAllRead", { defaultValue: "Mark all as read" })}
                        </Button>
                      </DialogFooter>
                    </DialogContent>
                  </Dialog>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            <SelectTrigger className="h-8 w-(--sz-170px) text-xs">
              <SelectValue placeholder={t("inbox.category.placeholder", { defaultValue: "Category" })} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="everything">{t("inbox.category.everything", { defaultValue: "All categories" })}</SelectItem>
              <SelectItem value="issues_i_touched">{t("inbox.category.myRecentTasks", { defaultValue: "My recent tasks" })}</SelectItem>
              <SelectItem value="join_requests">{t("inbox.category.joinRequests", { defaultValue: "Join requests" })}</SelectItem>
              <SelectItem value="approvals">{t("inbox.category.approvals", { defaultValue: "Approvals" })}</SelectItem>
              <SelectItem value="failed_runs">{t("inbox.category.failedRuns", { defaultValue: "Failed runs" })}</SelectItem>
              <SelectItem value="alerts">{t("inbox.category.alerts", { defaultValue: "Alerts" })}</SelectItem>
            </SelectContent>
          </Select>
      ...
      {tab !== "blocked" && allLoaded && visibleSections.length === 0 && (
        <EmptyState
          ...
          message={
            searchQuery.trim()
              ? t("inbox.empty.noSearchMatch", { defaultValue: "No inbox items match your search." })
              : tab === "mine"
              ? t("inbox.empty.inboxZero", { defaultValue: "Inbox zero." })
              : tab === "unread"
              ? t("inbox.empty.noNew", { defaultValue: "No new inbox items." })
              : tab === "recent"
                ? t("inbox.empty.noRecent", { defaultValue: "No recent inbox items." })
                : t("inbox.empty.noFilterMatch", { defaultValue: "No inbox items match these filters." })
          }
        />
      )}
                    ...
                    <IssueRow
                      ...
                      titleSuffix={hasChildren && !isExpanded && depth === 0 ? (
                        <span className="ml-1.5 text-xs text-muted-foreground">
                          {childCount === 1
                            ? t("inbox.subtasks.countOne", { defaultValue: "({{count}} sub-task)", count: childCount })
                            : t("inbox.subtasks.countOther", { defaultValue: "({{count}} sub-tasks)", count: childCount })}
                        </span>
                      ) : undefined}
                        ...
                        <IssueGroupHeader
                          ...
                          trailing={canCreateIssueInGroup ? (
                            <Button
                              ...
                              title={t("inbox.actions.newTaskInGroup", { defaultValue: "New task in {{group}}", group: group.label })}
                              aria-label={t("inbox.actions.newTaskInGroup", { defaultValue: "New task in {{group}}", group: group.label })}
                              onClick={(event) => {
                                event.stopPropagation();
                      ...
                      elements.push(
                        <div key={`today-divider-${group.key}-${index}`} className="my-2 flex items-center gap-3 px-4">
                          ...
                          <span className="shrink-0 text-(length:--text-micro) font-medium uppercase tracking-wider text-zinc-500">
                            {t("inbox.divider.earlier", { defaultValue: "Earlier" })}
                          </span>
                        </div>,
                      );
                    }
                  ...
                  >
                    ...
                    <span className="text-sm">
                      ...
                      {dashboard!.agents.error === 1
                        ? t("inbox.alerts.agentErrorsOne", { defaultValue: "agent has errors" })
                        : t("inbox.alerts.agentErrorsOther", { defaultValue: "agents have errors" })}
                    </span>
                  </Link>
                  <button
                    type="button"
                ...
                <div className="group/alert relative flex items-center gap-3 px-4 py-3 transition-colors hover:bg-accent/50">
                  ...
                  >
                    ...
                    <span className="text-sm">
                      ...
                      {t("inbox.alerts.budgetUtilization", { defaultValue: "utilization this month" })}
                    </span>
                  </Link>
                  <button
                    type="button"
