...
export function InstanceAccess() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("instanceAccess.breadcrumbs.settings", { defaultValue: "Settings" }), href: "/company/settings" },
      {
        label: t("instanceAccess.breadcrumbs.instanceSettings", { defaultValue: "Instance settings" }),
        href: "/company/settings/instance/general",
      },
      { label: t("instanceAccess.breadcrumbs.access", { defaultValue: "Access" }) },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  const updateCompanyAccessMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      pushToast({ title: t("instanceAccess.toasts.companyAccessUpdated", { defaultValue: "Company access updated" }), tone: "success" });
    },
  });
  ...
  const setAdminMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      pushToast({ title: t("instanceAccess.toasts.instanceRoleUpdated", { defaultValue: "Instance role updated" }), tone: "success" });
    },
  });
  ...
  if (usersQuery.error) {
    const message =
      usersQuery.error instanceof ApiError && usersQuery.error.status === 403
        ? t("instanceAccess.errors.forbidden", { defaultValue: "Instance admin access is required to manage users." })
        : usersQuery.error instanceof Error
          ? usersQuery.error.message
          : t("instanceAccess.errors.loadFailed", { defaultValue: "Failed to load users." });
      ...
      <div className="grid gap-6 lg:grid-cols-(--gtc-34)">
        <Card className="block space-y-4 p-4">
          <label className="block space-y-2 text-sm">
            <span className="font-medium">{t("instanceAccess.search.label", { defaultValue: "Search users" })}</span>
            <input
              className="w-full rounded-md border border-border bg-background px-3 py-2"
              value={search}
              onChange={(event) => setSearch(event.target.value)}
              placeholder={t("instanceAccess.search.placeholder", { defaultValue: "Search by name or email" })}
            />
          </label>
            ...
            {(usersQuery.data ?? []).map((user) => (
              ...
              >
                ...
                <div className="mt-2 text-xs text-muted-foreground">
                  {t("instanceAccess.userCard.activeMemberships", {
        ...
        <Card className="block space-y-4 p-5">
          {!selectedUserId ? (
            <div className="text-sm text-muted-foreground">{t("instanceAccess.empty.noUser", { defaultValue: "Select a user to inspect instance access." })}</div>
            ...
            <div className="text-sm text-destructive">
              {userAccessQuery.error instanceof Error
                ...
                : t("instanceAccess.errors.loadUserAccessFailed", { defaultValue: "Failed to load user access." })}
            </div>
          ) : (
            <>
              <div className="flex flex-wrap items-start justify-between gap-4">
                ...
                >
                  {selectedUser?.isInstanceAdmin
                    ? t("instanceAccess.actions.removeInstanceAdmin", { defaultValue: "Remove instance admin" })
                    : t("instanceAccess.actions.promoteInstanceAdmin", { defaultValue: "Promote to instance admin" })}
                </Button>
              </div>

              <div className="space-y-3">
                <div>
                  <h2 className="text-sm font-semibold">{t("instanceAccess.companyAccess.title", { defaultValue: "Company access" })}</h2>
                  <p className="text-sm text-muted-foreground">
                    {t("instanceAccess.companyAccess.description", { defaultValue: "Toggle company membership for this user. New access defaults to an active operator membership." })}
                  </p>
                </div>
                ...
                <div className="flex justify-end">
                  ...
                  >
                    {updateCompanyAccessMutation.isPending
                      ? t("instanceAccess.actions.saving", { defaultValue: "Saving…" })
                      : t("instanceAccess.actions.saveCompanyAccess", { defaultValue: "Save company access" })}
                  </Button>
                </div>
              </div>

              <div className="space-y-2">
                <h2 className="text-sm font-semibold">{t("instanceAccess.memberships.title", { defaultValue: "Current memberships" })}</h2>
                <div className="space-y-2">
                    ...
                    >
                      <div>
                        ...
                        <div className="text-muted-foreground">
                          {membership.membershipRole || t("instanceAccess.memberships.roleUnset", { defaultValue: "unset" })} • {membership.status}
                        </div>
                      </div>
