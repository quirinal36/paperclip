...
function formatMs(value: number | null) {
  if (value === null || !Number.isFinite(value)) return t("issueChatLongThreadPerf.metrics.pending", { defaultValue: "pending" });
    ...
    <div data-testid="issue-chat-long-thread-perf" className="space-y-5">
      <div className="flex flex-col gap-3 border-b border-border pb-5 lg:flex-row lg:items-end lg:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">
            {t("issueChatLongThreadPerf.description", {
              defaultValue:
        ...
        <div className="grid min-w-(--sz-280px) grid-cols-2 gap-2">
          <MetricTile label={t("issueChatLongThreadPerf.metrics.fixtureRows", { defaultValue: "Fixture rows" })} value={String(rowTarget)} testId="perf-fixture-row-target" />
          <MetricTile label={t("issueChatLongThreadPerf.metrics.markdownRows", { defaultValue: "Markdown rows" })} value={String(LONG_THREAD_MARKDOWN_COMMENT_COUNT)} testId="perf-fixture-markdown-rows" />
        </div>
      </div>
      ...
      <div className="grid gap-4 xl:grid-cols-(--gtc-40)">
        <main className="min-w-0 space-y-4">
          <Card className="border-border/70">
            <CardHeader className="pb-2">
              <CardTitle className="text-base">{t("issueChatLongThreadPerf.sections.issueDocuments", { defaultValue: "Issue documents" })}</CardTitle>
            </CardHeader>
          ...
          <Card className="border-border/70">
            <CardHeader className="pb-2">
              <CardTitle className="text-base">{t("issueChatLongThreadPerf.sections.subIssues", { defaultValue: "Sub-issues" })}</CardTitle>
            </CardHeader>
        ...
        <aside className="space-y-4 xl:sticky xl:top-4 xl:self-start">
          <Card className="border-border/70">
            <CardHeader className="pb-2">
              <CardTitle className="text-base">{t("issueChatLongThreadPerf.sections.baselineMetrics", { defaultValue: "Baseline metrics" })}</CardTitle>
            </CardHeader>
            <CardContent className="grid gap-2">
              <MetricTile label={t("issueChatLongThreadPerf.metrics.profilerCommits", { defaultValue: "Profiler commits" })} value={String(metrics.commitCount)} testId="perf-commit-count" />
              <MetricTile label={t("issueChatLongThreadPerf.metrics.mountDuration", { defaultValue: "Mount duration" })} value={formatMs(metrics.mountActualDuration)} testId="perf-mount-duration" />
              <MetricTile label={t("issueChatLongThreadPerf.metrics.latestDuration", { defaultValue: "Latest duration" })} value={formatMs(metrics.latestActualDuration)} testId="perf-latest-duration" />
              <MetricTile label={t("issueChatLongThreadPerf.metrics.maxDuration", { defaultValue: "Max duration" })} value={formatMs(metrics.maxActualDuration)} testId="perf-max-duration" />
              <MetricTile label={t("issueChatLongThreadPerf.metrics.totalDuration", { defaultValue: "Total duration" })} value={formatMs(metrics.totalActualDuration)} testId="perf-total-duration" />
            </CardContent>
          </Card>

          <Card className="border-border/70">
            <CardHeader className="pb-2">
              <CardTitle className="text-base">{t("issueChatLongThreadPerf.sections.fixtureShape", { defaultValue: "Fixture shape" })}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-2">
