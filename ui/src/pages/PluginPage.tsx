...
export function PluginPage() {
  ...
  useEffect(() => {
    ...
    setBreadcrumbs([
      { label: t("pluginPage.breadcrumb.plugins", { defaultValue: "Plugins" }), href: "/company/settings/instance/plugins" },
    ...
    if (duplicateMatches.length > 1) {
      return (
        <div className="rounded-md border border-destructive/30 bg-destructive/5 px-3 py-2 text-sm text-destructive">
          {t("pluginPage.routeConflict.before", { defaultValue: "Multiple plugins declare the route " })}<code>{pluginRoutePath}</code>{t("pluginPage.routeConflict.after", { defaultValue: ". Use the plugin-id route until the conflict is resolved." })}
        </div>
      );
    }
  }
        ...
        <div className="flex items-center gap-2">
          <Button variant="ghost" size="sm" asChild>
            <Link to={companyPrefix ? `/${companyPrefix}/dashboard` : "/dashboard"}>
              ...
              {t("pluginPage.actions.back", { defaultValue: "Back" })}
            </Link>
          </Button>
        </div>
      )}
