...
export function ProfileSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("profileSettings.breadcrumbs.settings", { defaultValue: "Settings" }), href: "/company/settings" },
      { label: t("profileSettings.breadcrumbs.instanceSettings", { defaultValue: "Instance settings" }), href: "/company/settings/instance/general" },
      { label: t("profileSettings.breadcrumbs.profile", { defaultValue: "Profile" }) },
    ]);
  }, [setBreadcrumbs]);
  ...
  const updateMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("profileSettings.errors.updateFailed", { defaultValue: "Failed to update profile." }));
    },
  });
  ...
  const uploadAvatarMutation = useMutation({
    mutationFn: async (file: File) => {
      if (!selectedCompanyId) {
        throw new Error(t("profileSettings.errors.selectCompanyBeforeUpload", { defaultValue: "Select a company before uploading a profile avatar." }));
      }
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("profileSettings.errors.uploadAvatarFailed", { defaultValue: "Failed to upload avatar." }));
    },
  });
  ...
  const removeAvatarMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("profileSettings.errors.removeAvatarFailed", { defaultValue: "Failed to remove avatar." }));
    },
  });
  ...
  if (sessionQuery.error || !sessionQuery.data) {
    return (
      <div className="text-sm text-destructive">
        {sessionQuery.error instanceof Error ? sessionQuery.error.message : t("profileSettings.errors.loadFailed", { defaultValue: "Failed to load profile." })}
      </div>
    );
  }
  ...
  const uploadHint = selectedCompany
    ? t("profileSettings.upload.hintStored", { defaultValue: "Stored in Paperclip file storage for {{company}}.", company: selectedCompany.name })
    : t("profileSettings.upload.hintSelectCompany", { defaultValue: "Select a company to upload an avatar into Paperclip storage." });
    ...
    <div className="max-w-4xl space-y-6">
      <div className="space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          {t("profileSettings.description", { defaultValue: "Control how your account appears in the sidebar and other board surfaces." })}
        </p>
      </div>
      ...
      <section className="space-y-8">
            ...
            <div className="flex flex-wrap items-end gap-5 rounded-(--rad-24) border border-border/70 bg-background/92 p-5 shadow-(--shadow-extract-18) backdrop-blur-sm">
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  >
                    {uploadAvatarMutation.isPending ? <LoaderCircle className="size-4 animate-spin" /> : <Camera className="size-4" />}
                    {currentImage ? t("profileSettings.actions.changePhoto", { defaultValue: "Change photo" }) : t("profileSettings.actions.uploadPhoto", { defaultValue: "Upload photo" })}
                  </Button>
                  {currentImage ? (
                    <Button
                      type="button"
                    ...
                    >
                      ...
                      {t("profileSettings.actions.remove", { defaultValue: "Remove" })}
                    </Button>
                  ) : null}
                </div>
              </div>
              ...
              <div className="min-w-0 flex-1 space-y-2 pb-1">
                <div>
                  ...
                  <p className="truncate text-sm text-muted-foreground">{sessionQuery.data.user.email ?? t("profileSettings.fields.noEmail", { defaultValue: "No email" })}</p>
                </div>
                <p className="max-w-2xl text-sm leading-6 text-muted-foreground">
                  {t("profileSettings.avatar.clickHint", { defaultValue: "Click the avatar to upload a new image. " })}{uploadHint}
                </p>
              </div>
            </div>
          </div>
        ...
        >
          <div className="space-y-2">
            <Label htmlFor="profile-name">{t("profileSettings.fields.displayName", { defaultValue: "Display name" })}</Label>
            <Input
              id="profile-name"
              value={name}
              onChange={(event) => setName(event.target.value)}
              maxLength={120}
              placeholder={t("profileSettings.fields.namePlaceholder", { defaultValue: "Board" })}
            />
            <p className="text-xs text-muted-foreground">
              {t("profileSettings.fields.nameHint", { defaultValue: "Shown in the sidebar account footer and comment author surfaces." })}
            </p>
          </div>

          <div className="space-y-2">
            <Label htmlFor="profile-email">{t("profileSettings.fields.email", { defaultValue: "Email" })}</Label>
            <Input
              id="profile-email"
            ...
            />
            <p className="text-xs text-muted-foreground">
              {t("profileSettings.fields.emailHint", { defaultValue: "Email is managed by your auth session and is read-only here." })}
            </p>
          </div>
          ...
          <div className="md:col-span-2 flex justify-end">
            <Button type="submit" disabled={isSavingProfile || !name.trim()}>
              ...
              {updateMutation.isPending ? t("profileSettings.actions.saving", { defaultValue: "Saving..." }) : t("profileSettings.actions.saveProfile", { defaultValue: "Save profile" })}
            </Button>
          </div>
        </form>
