    ...
    >
      <PopoverTrigger asChild>
        <button
          ...
          aria-label={t("projectDetail.picker.changeIconColor", { defaultValue: "Change project icon and color" })}
        >
      ...
      <PopoverContent className="w-72 p-3" align="start">
        {/* Icon search + grid */}
        <p className="text-xs font-medium text-muted-foreground mb-2">{t("projectDetail.picker.iconLabel", { defaultValue: "Icon" })}</p>
        <Input
          placeholder={t("projectDetail.picker.searchIconsPlaceholder", { defaultValue: "Search icons..." })}
          value={search}
        ...
        <div className="grid grid-cols-7 gap-1 max-h-40 overflow-y-auto">
          ...
          {filteredIcons.length === 0 && (
            <p className="col-span-7 text-xs text-muted-foreground text-center py-2">{t("projectDetail.picker.noIconsMatch", { defaultValue: "No icons match" })}</p>
          )}
        </div>

        {/* Color swatches */}
        <div className="mt-3 border-t border-border pt-3">
          <p className="text-xs font-medium text-muted-foreground mb-2">{t("projectDetail.picker.colorLabel", { defaultValue: "Color" })}</p>
          <div className="grid grid-cols-5 gap-1.5">
            ...
            <button
              ...
              aria-label={t("projectDetail.picker.resetToNeutral", { defaultValue: "Reset to neutral gray" })}
              title={t("projectDetail.picker.neutralDefault", { defaultValue: "Neutral (default)" })}
            >
            ...
            {PROJECT_COLORS.map((swatch) => (
              <button
                ...
                aria-label={t("projectDetail.picker.selectColor", { defaultValue: "Select color {{swatch}}", swatch })}
              />
            ))}
          </div>
        </div>
...
export function ProjectDetail() {
  ...
  const archiveProject = useMutation({
    ...
    onSuccess: (updatedProject, archived) => {
      invalidateProject();
      const name = updatedProject?.name ?? project?.name ?? t("projectDetail.fallback.projectName", { defaultValue: "Project" });
      if (archived) {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: project?.name ?? routeProjectRef ?? t("projectDetail.fallback.projectName", { defaultValue: "Project" }) },
    ]);
  ...
  const projectBudgetSummary = useMemo(() => {
    ...
    return {
      ...
      scopeName: project?.name ?? t("projectDetail.fallback.projectName", { defaultValue: "Project" }),
      metric: "billed_cents",
      windowKind: "lifetime",
      amount: 0,
      observedAmount: 0,
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="min-w-0 space-y-2">
          ...
          {project.pauseReason === "budget" ? (
            <div className="inline-flex items-center gap-2 rounded-full border border-red-500/30 bg-red-500/10 px-3 py-1 text-(length:--text-micro) font-medium uppercase tracking-(--tracking-caps) text-red-800 dark:text-red-200">
              ...
              {t("projectDetail.badge.pausedByBudget", { defaultValue: "Paused by budget hard stop" })}
            </div>
          ) : null}
          {project.managedByPlugin ? (
            <div className="inline-flex items-center gap-2 rounded-full border border-border bg-muted px-3 py-1 text-(length:--text-micro) font-medium text-muted-foreground">
              ...
              {t("projectDetail.badge.managedBy", { defaultValue: "Managed by {{name}}", name: project.managedByPlugin.pluginDisplayName })}
            </div>
          ) : null}
        </div>
      ...
      <SummarySlotCard
        ...
        scopeKind="project"
        scopeId={project.id}
        title={t("projectDetail.summary.title", { defaultValue: "Project summary" })}
        description={t("projectDetail.summary.description", { defaultValue: "Summarizer keeps the latest project status, next step, and operator-needed items here." })}
      />
      ...
      <Tabs value={activeTab ?? "list"} onValueChange={(value) => handleTabChange(value as ProjectTab)}>
        <PageTabBar
          items={[
            { value: "list", label: t("projectDetail.tabs.tasks", { defaultValue: "Tasks" }) },
