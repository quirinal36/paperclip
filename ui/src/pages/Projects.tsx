...
export function Projects() {
  ...
  const activeSortOption = PROJECT_SORT_OPTIONS.find((option) => option.field === sortField);
  const sortLabel = t(`projects.sort.options.${activeSortOption?.field ?? "name"}`, {
    defaultValue: activeSortOption?.label ?? "Name",
  });
  ...
  if (!selectedCompanyId) {
    return (
      <EmptyState
        icon={Hexagon}
        message={t("projects.empty.noCompany", { defaultValue: "Select a company to view projects." })}
      />
    );
  }
    ...
    <div className="space-y-4">
        ...
        <Popover>
          <PopoverTrigger asChild>
            <Button
              ...
              title={t("projects.sort.tooltip", { defaultValue: "Sort" })}
            >
              <ArrowUpDown className="h-3.5 w-3.5 sm:h-3 sm:w-3 sm:mr-1" />
              <span>{t("projects.sort.buttonLabel", { defaultValue: "Sort: {{label}}", label: sortLabel })}</span>
            </Button>
          </PopoverTrigger>
          <PopoverContent align="start" className="w-44 p-0">
            <div className="p-2 space-y-0.5">
              {PROJECT_SORT_OPTIONS.map((option) => (
                ...
                >
                  <span>{t(`projects.sort.options.${option.field}`, { defaultValue: option.label })}</span>
                  {sortField === option.field ? (
      ...
      {projects.length > 0 && (
        <div className="space-y-6">
          {([
            ["mine", t("projects.sections.mine", { defaultValue: "My Projects" }), groupedProjects.mine],
            ["other", t("projects.sections.other", { defaultValue: "Other Projects" }), groupedProjects.other],
          ] as const).map(([sectionId, label, sectionProjects]) => {
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-muted-foreground">
                    {sectionProjects.length === 1
                      ? t("projects.projectCount.singular", { defaultValue: "{{value}} project", value: sectionProjects.length })
                      : t("projects.projectCount.plural", { defaultValue: "{{value}} projects", value: sectionProjects.length })}
                  </span>
                </div>
                            ...
                            <span
                              ...
                              title={
                                (project.taskCount ?? 0) === 1
                                  ? t("projects.taskCount.singular", { defaultValue: "{{value}} task", value: formatNumber(project.taskCount ?? 0) })
                                  : t("projects.taskCount.plural", { defaultValue: "{{value}} tasks", value: formatNumber(project.taskCount ?? 0) })
                              }
                            >
                              {(project.taskCount ?? 0) === 1
                                ? t("projects.taskCount.singular", { defaultValue: "{{value}} task", value: formatNumber(project.taskCount ?? 0) })
