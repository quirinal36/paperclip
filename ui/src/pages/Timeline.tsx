...
function rangeError(range: DateRangeState): string | null {
  if (!range.fromDate || !range.toDate) return t("timeline.errors.chooseDates", { defaultValue: "Choose a start and end date." });
  if (!rangeWindow(range)) return t("timeline.errors.startBeforeEnd", { defaultValue: "Start date must be before end date." });
  return null;
}
...
function TimelineSummaryStats({
  ...
  const stats: { label: string; value: string; icon: LucideIcon }[] = [
    { label: t("timeline.stats.runs", { defaultValue: "Runs" }), value: formatInteger(summary.runs), icon: GanttChartSquare },
    { label: t("timeline.stats.agents", { defaultValue: "Agents" }), value: formatInteger(summary.agents), icon: Bot },
    { label: t("timeline.stats.runTime", { defaultValue: "Run time" }), value: formatDuration(0, summary.activeMs), icon: Clock3 },
    {
      ...
      value: summary.totalTokens > 0 ? formatCompactInteger(summary.totalTokens) : t("timeline.stats.notTracked", { defaultValue: "Not tracked" }),
      icon: Coins,
    },
  ];
...
export function Timeline() {
  ...
  if (!selectedCompanyId) {
    return (
      <>
        ...
        <EmptyState icon={GanttChartSquare} message={t("timeline.emptyStates.selectCompany", { defaultValue: "Select a company to view its work timeline." })} />
      </>
    );
  }
  ...
  const header = (
    <div className="flex items-center gap-2">
      ...
      <h1 className="text-3xl font-semibold tracking-tight">{t("timeline.title", { defaultValue: "Work Timeline" })}</h1>
    </div>
  );
  ...
  const rangeControls = (
    <label className="flex min-w-0 flex-wrap items-center gap-2 text-xs text-muted-foreground">
      {t("timeline.range.label", { defaultValue: "Range" })}
      <Segmented
        value={rangePreset}
        onChange={(preset) => {
        ...
        options={[
          ...
          { value: "7d", label: t("timeline.range.last7Days", { defaultValue: "7 days" }) },
          { value: "30d", label: t("timeline.range.last30Days", { defaultValue: "30 days" }) },
        ]}
      />
      <Input
        type="date"
        ...
        aria-label={t("timeline.range.startDateLabel", { defaultValue: "Timeline start date" })}
      />
      <span>{t("timeline.range.to", { defaultValue: "to" })}</span>
      <Input
        type="date"
        ...
        className="h-8 w-(--sz-150px) text-xs"
        aria-label={t("timeline.range.endDateLabel", { defaultValue: "Timeline end date" })}
      />
    </label>
  );
  ...
  const toolbar = (
    <div className="flex flex-wrap items-start gap-3">
      ...
      <div className="ml-auto flex items-center gap-1 pt-3" aria-label={t("timeline.zoom.controlsLabel", { defaultValue: "Timeline zoom controls" })}>
        <Button
          type="button"
          variant="outline"
          size="icon-xs"
          onClick={() => adjustZoom(0.8)}
          aria-label={t("timeline.zoom.zoomOut", { defaultValue: "Zoom out" })}
          title={t("timeline.zoom.zoomOut", { defaultValue: "Zoom out" })}
        >
        ...
        <Button
          ...
          aria-label={t("timeline.zoom.zoomIn", { defaultValue: "Zoom in" })}
          title={t("timeline.zoom.zoomIn", { defaultValue: "Zoom in" })}
        >
        ...
        <Button
          ...
          size="icon-xs"
          onClick={resetZoom}
          aria-label={t("timeline.zoom.resetZoom", { defaultValue: "Reset zoom" })}
          title={t("timeline.zoom.resetZoom", { defaultValue: "Reset zoom" })}
        >
  ...
  return (
    <div className="space-y-6">
      ...
      {error && (
        <EmptyState
          icon={GanttChartSquare}
          message={t("timeline.errors.loadFailed", { defaultValue: "Couldn't load the timeline. The aggregation endpoint may be unavailable." })}
        />
      )}
      ...
      {data && !isLoading && !dateRangeError && (
        data.spans.length === 0 ? (
          <div className="space-y-3">
            <EmptyState icon={GanttChartSquare} message={t("timeline.emptyStates.noActivity", { defaultValue: "No activity in this window." })} />
            <div className="flex flex-wrap items-center justify-end gap-3">
              {rangeControls}
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              <p className="text-xs text-muted-foreground">
                {data.spans.length === 1
                  ? t("timeline.footer.runCountOne", { defaultValue: "{{count}} run", count: data.spans.length })
                  : t("timeline.footer.runCountOther", { defaultValue: "{{count}} runs", count: data.spans.length })}
                {" "}·{" "}
                {new Date(data.window.from).toLocaleString()}{" "}
                {t("timeline.footer.rangeTo", { defaultValue: "to" })}{" "}
                {new Date(data.window.to).toLocaleString()}
                {data.window.capped ? ` · ${t("timeline.footer.windowCapped", { defaultValue: "window capped" })}` : ""}
              </p>
              {rangeControls}
            </div>
          </div>
