...
export function AgentSkillsTab({ agent, companyId }: { agent: Agent; companyId?: string }) {
  ...
  const materializationNote = t("agentSkillsTab.materializationNote", {
    defaultValue:
  ...
  const applicationLabel = useMemo(() => {
    switch (skillSnapshot?.mode) {
      case "persistent":
        return t("agentSkillsTab.applicationLabel.persistent", { defaultValue: "Kept in workspace" });
      case "ephemeral":
        return t("agentSkillsTab.applicationLabel.ephemeral", { defaultValue: "Applied on next run" });
      case "unsupported":
        return t("agentSkillsTab.applicationLabel.unsupported", { defaultValue: "Tracked only" });
      default:
        return null;
    }
    ...
    <div className="max-w-4xl space-y-4">
      ...
      <div className="flex flex-col gap-3">
        <div className="flex flex-wrap items-center gap-2">
          <span className="text-sm font-medium text-foreground">
            {t("agentSkillsTab.header.enabledCount", {
          ...
          <div className="ml-auto flex w-full flex-col items-stretch gap-2 sm:w-auto sm:flex-row sm:items-center">
            <div className="relative w-full sm:w-auto">
              ...
              <Input
                ...
                placeholder={t("agentSkillsTab.search.placeholder", { defaultValue: "Search skills" })}
                className="h-8 w-full pl-8 sm:w-56"
                aria-label={t("agentSkillsTab.search.ariaLabel", { defaultValue: "Search skills" })}
              />
            </div>
            <Button asChild variant="outline" size="sm" className="shrink-0">
              <Link to="/skills" className="no-underline">
                <Store className="h-3.5 w-3.5" />
                {t("agentSkillsTab.actions.browseStore", { defaultValue: "Browse skills store" })}
              </Link>
            </Button>
          </div>
        </div>
        ...
        {syncSkills.isError ? (
          <p className="text-xs text-destructive">
            {syncSkills.error instanceof Error
              ...
              : t("agentSkillsTab.errors.updateFailed", { defaultValue: "Failed to update skills" })}
          </p>
        ) : null}
      </div>
          ...
          {staleDesiredKeys.map((key) => (
            ...
            >
              <span className="min-w-0 truncate">
                ...
                {t("agentSkillsTab.stale.missingSuffix", {
              ...
              >
                <X className="h-3 w-3" />
                {t("agentSkillsTab.actions.remove", { defaultValue: "Remove" })}
              </button>
            </div>
          ))}
        </div>
      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          <SkillSection
            title={t("agentSkillsTab.sections.enabled", { defaultValue: "Enabled on this agent" })}
          ...
          >
            {filteredEnabled.length > 0 ? (
              ...
              <SectionEmpty>
                {search
                  ? t("agentSkillsTab.empty.enabledNoMatch", {
                      defaultValue: "No enabled skills match your search.",
                    })
                  : t("agentSkillsTab.empty.enabledNone", {
          ...
          <SkillSection
            title={t("agentSkillsTab.sections.available", { defaultValue: "Available from the library" })}
              ...
              <div className="overflow-hidden rounded-lg border border-border">
                <CollapsibleTrigger className="flex w-full items-center gap-2 bg-muted/50 px-3 py-2 text-left outline-none focus-visible:ring-2 focus-visible:ring-ring">
                  ...
                  <span className="text-xs font-medium text-muted-foreground">
                    {t("agentSkillsTab.sections.detected", { defaultValue: "Detected on adapter (read-only)" })}
                  </span>
                ...
                <CollapsibleContent>
                  {filteredDetected.length > 0 ? (
                    ...
                    <SectionEmpty>
                      {t("agentSkillsTab.empty.detectedNoMatch", {
          ...
          <div className="text-xs text-muted-foreground">
            {t("agentSkillsTab.adapterFooter", {
  ...
  if (pending) {
    return (
      <span className="inline-flex items-center gap-1.5 text-xs text-muted-foreground">
        ...
        {t("agentSkillsTab.saveStatus.saving", { defaultValue: "Saving…" })}
      </span>
    );
  }
  if (error) {
    return (
      <span className="inline-flex items-center gap-1.5 text-xs text-destructive">
        ...
        {t("agentSkillsTab.saveStatus.error", { defaultValue: "Couldn’t save" })}
      </span>
    );
  }
  if (unsaved) {
    return (
      <span className="inline-flex items-center gap-1.5 text-xs text-muted-foreground">
        <Loader2 className="h-3.5 w-3.5 animate-spin" />
        {t("agentSkillsTab.saveStatus.savingSoon", { defaultValue: "Saving soon…" })}
      </span>
    );
  }
  return (
    <span className="inline-flex items-center gap-1.5 text-xs text-(--status-task-done)">
      ...
      {t("agentSkillsTab.saveStatus.saved", { defaultValue: "Saved" })}
    </span>
  );
}
    ...
    <div className="flex flex-col items-center gap-3 rounded-lg border border-dashed border-border px-6 py-10 text-center">
      ...
      <div className="space-y-1">
        <p className="text-sm font-medium text-foreground">
          {t("agentSkillsTab.emptyLibrary.title", { defaultValue: "No skills in the company library" })}
        </p>
        <p className="text-xs text-muted-foreground">
          {t("agentSkillsTab.emptyLibrary.description", {
