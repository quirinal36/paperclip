...
export function AppNotConnected() {
  ...
  const appName = application?.name ?? t("appNotConnected.fallback.appName", { defaultValue: "App" });
  useEffect(() => {
    if (!activeTab) return;
    setBreadcrumbs([
      { label: selectedCompany?.name ?? t("appNotConnected.breadcrumbs.company", { defaultValue: "Company" }), href: "/dashboard" },
      { label: t("appNotConnected.breadcrumbs.apps", { defaultValue: "Apps" }), href: "/apps" },
  ...
  const remove = useMutation({
    ...
    onSuccess: () => {
      ...
      pushToast({
        title: t("appNotConnected.toast.removed.title", { defaultValue: "App removed" }),
        body: t("appNotConnected.toast.removed.body", {
    ...
    onError: (error) => {
      pushToast({
        title: t("appNotConnected.toast.removeFailed.title", { defaultValue: "Couldn’t remove the app" }),
        body: error instanceof Error ? error.message : t("appNotConnected.toast.removeFailed.retry", { defaultValue: "Please try again." }),
        tone: "error",
      });
    },
  });
  ...
  if (!selectedCompanyId) {
    return <div className="p-6 text-sm text-muted-foreground">{t("appNotConnected.selectCompany", { defaultValue: "Select a company to manage apps." })}</div>;
  }
  ...
  if (!application) {
    return (
      <div className="max-w-3xl space-y-3 p-6 text-sm text-muted-foreground">
        <p>{t("appNotConnected.notFound.message", { defaultValue: "This app doesn’t exist anymore." })}</p>
        <Button variant="outline" size="sm" onClick={() => navigate("/apps")}>{t("appNotConnected.notFound.back", { defaultValue: "Back to apps" })}</Button>
      </div>
    );
  }
  if (activeConnection) {
    ...
    <div className="max-w-3xl space-y-6 pb-12">
      ...
      {activeTab === "review" && (
        previousConnection ? (
          ...
          <EmptyTab
            title={t("appNotConnected.review.emptyTitle", { defaultValue: "Nothing is waiting for your OK right now." })}
            body={t("appNotConnected.review.emptyBody", { defaultValue: "Review requests will appear here after this app is connected." })}
          />
        )
      )}
      ...
      {activeTab === "test" && (
        <EmptyTab
          title={t("appNotConnected.test.emptyTitle", { defaultValue: "Reconnect to test this app." })}
          body={t("appNotConnected.test.emptyBody", { defaultValue: "Testing becomes available after this app is connected again." })}
        />
      )}
      {activeTab === "activity" && (
        previousConnection ? (
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          ...
          <span className="inline-flex items-center rounded-full border border-border bg-background px-2 py-0.5 text-xs font-medium text-muted-foreground">
            {t("appNotConnected.header.notConnected", { defaultValue: "Not connected" })}
          </span>
        </div>
        {description && (
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h2 className="text-sm font-bold text-foreground">
              {previousConnection
                ? t("appNotConnected.setup.reconnectTitle", { defaultValue: "Reconnect this app" })
                : t("appNotConnected.setup.connectTitle", { defaultValue: "Connect this app" })}
            </h2>
            <p className="mt-0.5 text-sm text-muted-foreground">
              {previousConnection
                ? t("appNotConnected.setup.reconnectDescription", { defaultValue: "We kept the previous setup. Add a working key to bring it back online." })
                : t("appNotConnected.setup.connectDescription", { defaultValue: "Agents can't use it until it's connected." })}
            </p>
          </div>
          <Button onClick={onConnect}>
            {previousConnection
              ? t("appNotConnected.setup.reconnectAction", { defaultValue: "Reconnect" })
              : t("appNotConnected.setup.connectAction", { defaultValue: "Connect" })}
          </Button>
        </div>
      </section>
...
function PreviousSetup({
  ...
  return (
    <section className="rounded-xl border border-border bg-card px-5 py-4">
      <h2 className="text-sm font-bold text-foreground">{t("appNotConnected.previousSetup.title", { defaultValue: "Previous setup" })}</h2>
      {connection.healthMessage && (
        <p className="mt-2 rounded-lg border border-amber-500/40 bg-amber-500/10 px-3 py-2 text-xs text-amber-700 dark:text-amber-300">
          {t("appNotConnected.previousSetup.lastError", { defaultValue: "Last error: {{message}}", message: connection.healthMessage })}
        </p>
      )}
      <dl className="mt-3 grid gap-2 text-xs sm:grid-cols-(--gtc-59)">
        <dt className="text-muted-foreground">{t("appNotConnected.previousSetup.address", { defaultValue: "Address" })}</dt>
        <dd className="break-all font-mono text-foreground">{previousAddress}</dd>
        <dt className="text-muted-foreground">{t("appNotConnected.previousSetup.connectionType", { defaultValue: "Connection type" })}</dt>
        <dd className="text-foreground">{connectionTransportLabel(connection.transport)}</dd>
        <dt className="text-muted-foreground">{t("appNotConnected.previousSetup.lastUsed", { defaultValue: "Last used" })}</dt>
        <dd className="text-foreground">
          {connection.lastUsedAt ? timeAgo(connection.lastUsedAt) : t("appNotConnected.previousSetup.never", { defaultValue: "Never" })}
        </dd>
      </dl>
    </section>
  );
...
function PermissionsTab({ previousConnection }: { previousConnection: ToolConnection | null }) {
  ...
  return (
    <section className="rounded-xl border border-border bg-card px-5 py-4">
      <h2 className="text-sm font-bold text-foreground">{t("appNotConnected.permissions.title", { defaultValue: "Permissions paused" })}</h2>
      <p className="mt-1 text-sm text-muted-foreground">
        {t("appNotConnected.permissions.description", { defaultValue: "Reconnect this app to edit who can use it and which actions need a human first." })}
      </p>
      {previousConnection && (
        <p className="mt-3 text-xs text-muted-foreground">
          {t("appNotConnected.permissions.retained", { defaultValue: "Previous setup is retained for reconnect, but access controls stay read-only until the app is online." })}
        </p>
      )}
    </section>
  );
  ...
  return (
    <div className="space-y-6">
      {previousConnection ? (
        ...
        <EmptyTab
          title={t("appNotConnected.advanced.emptyTitle", { defaultValue: "No previous connection details" })}
          body={t("appNotConnected.advanced.emptyBody", { defaultValue: "Technical details will appear here after this app is connected." })}
        />
      )}
