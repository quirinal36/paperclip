...
function statusFor(application: ToolApplication, connections: ToolConnection[]): AppStatus {
  if (connections.length === 0) {
    return { label: t("connections.status.notConnected", { defaultValue: "Not connected" }), tone: "not_connected" };
  }
  if (
    ...
    return { label: t("connections.status.paused", { defaultValue: "Paused" }), tone: "paused" };
  }
  if (connections.some((connection) => isAttentionHealthStatus(connection.healthStatus))) {
    return { label: t("connections.status.needsAttention", { defaultValue: "Needs attention" }), tone: "attention" };
  }
  return { label: t("connections.status.healthy", { defaultValue: "Healthy" }), tone: "connected" };
}
...
export function Connections() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      {
        label: selectedCompany?.name ?? t("connections.breadcrumbs.company", { defaultValue: "Company" }),
        href: "/dashboard",
      },
      { label: t("connections.breadcrumbs.apps", { defaultValue: "Apps" }), href: "/apps" },
      { label: t("connections.breadcrumbs.connections", { defaultValue: "Connections" }) },
    ]);
  ...
  if (!selectedCompanyId) {
    return (
      <div className="p-6 text-sm text-muted-foreground">
        {t("connections.selectCompany", { defaultValue: "Select a company to manage apps." })}
      </div>
    );
  }
        ...
        <div className="space-y-5">
          <header className="flex flex-wrap items-end justify-between gap-3">
            ...
            <Button onClick={() => navigate(BROWSE_HREF)}>
              {t("connections.actions.connectApp", { defaultValue: "Connect an app" })}
            </Button>
          </header>

          <div className="flex flex-wrap items-center gap-2">
            <FilterChip active={filter === "all"} onClick={() => setFilter("all")}>
              {t("connections.filters.all", { defaultValue: "All ({{count}})", count: rows.length })}
            </FilterChip>
            <FilterChip
            ...
            >
              {t("connections.filters.needsAttention", {
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <div className="text-sm font-semibold text-amber-900 dark:text-amber-100">
                  {reviewCount === 1
                    ? t("connections.review.waitingOne", {
                        defaultValue: "{{count}} action is waiting for your OK",
                        count: reviewCount,
                      })
                    : t("connections.review.waitingMany", {
                        defaultValue: "{{count}} actions are waiting for your OK",
                        count: reviewCount,
                      })}
                </div>
                <div className="truncate text-xs text-amber-700 dark:text-amber-300">
                  {t("connections.review.description", {
              ...
              <span className="shrink-0 text-xs font-semibold text-amber-800 dark:text-amber-200">
                {t("connections.review.cta", { defaultValue: "Review →" })}
              </span>
            </button>
          )}
          ...
          {rowsNeedingAttention.length > 0 && (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <div className="text-sm font-semibold text-red-900 dark:text-red-100">
                  {rowsNeedingAttention.length === 1
                    ? t("connections.attentionBanner.titleOne", {
                        defaultValue: "{{count}} app needs attention",
                        count: rowsNeedingAttention.length,
                      })
                    : t("connections.attentionBanner.titleMany", {
              ...
              <span className="shrink-0 text-xs font-semibold text-red-800 dark:text-red-200">
                {t("connections.attentionBanner.cta", { defaultValue: "Fix →" })}
              </span>
            </button>
          )}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-muted/40 text-left text-(length:--text-micro) font-semibold uppercase tracking-wide text-muted-foreground">
                  <th className="px-4 py-2.5">{t("connections.table.app", { defaultValue: "App" })}</th>
                  <th className="px-4 py-2.5">{t("connections.table.status", { defaultValue: "Status" })}</th>
                  <th className="px-4 py-2.5">{t("connections.table.actions", { defaultValue: "Actions" })}</th>
                  <th className="px-4 py-2.5">{t("connections.table.lastUsed", { defaultValue: "Last used" })}</th>
              ...
              <tbody>
                {visibleRows.map((row) => {
                  ...
                  const actionLabel = !primaryConnection
                    ? t("connections.actions.connect", { defaultValue: "Connect" })
                    : status.tone === "attention"
                      ? t("connections.actions.reconnect", { defaultValue: "Reconnect" })
                      : t("connections.actions.open", { defaultValue: "Open" });
                  return (
                    <tr
                      key={application.id}
...
function floatSummary(rows: AppRow[]): string {
  ...
  if (names.length <= 2) {
    return names.length === 2
      ? t("connections.attentionBanner.twoApps", {
  ...
  return t("connections.attentionBanner.moreApps", {
    defaultValue: "{{first}} and {{count}} more",
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-dashed border-border bg-card p-10 text-center">
        ...
        <p className="mt-4 text-sm font-medium text-foreground">
          {t("connections.empty.title", { defaultValue: "No connections yet." })}
        </p>
        <p className="mt-1 text-sm text-muted-foreground">
          {t("connections.empty.bodyBefore", { defaultValue: "Add one from " })}
          <span className="font-medium text-foreground">
            {t("connections.empty.appsLink", { defaultValue: "Apps" })}
          </span>
          {t("connections.empty.bodyAfter", {
            defaultValue: " to give your agents the tools they need.",
          })}
        </p>
        <Button className="mt-6" onClick={onBrowse}>
          {t("connections.actions.browseApps", { defaultValue: "Browse apps" })}
        </Button>
      </div>
    </div>
  );
