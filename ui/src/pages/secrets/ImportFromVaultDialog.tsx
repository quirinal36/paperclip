...
function statusBadgeLabel(status: RemoteSecretImportCandidate["status"]) {
  switch (status) {
    case "duplicate":
      return t("importFromVaultDialog.statusBadge.imported", { defaultValue: "Imported" });
    case "conflict":
      return t("importFromVaultDialog.statusBadge.conflict", { defaultValue: "Conflict" });
    case "ready":
    default:
      return t("importFromVaultDialog.statusBadge.ready", { defaultValue: "Ready" });
  }
}
  ...
  switch (status) {
    case "imported":
      return (
        ...
        >
          <CheckCircle2 className="h-3 w-3" /> {t("importFromVaultDialog.rowResult.created", { defaultValue: "Created" })}
        </Badge>
      );
    case "skipped":
      return (
        ...
        >
          <Link2 className="h-3 w-3" /> {t("importFromVaultDialog.rowResult.skipped", { defaultValue: "Skipped" })}
        </Badge>
      );
    case "error":
    default:
      return (
        ...
        >
          <XCircle className="h-3 w-3" /> {t("importFromVaultDialog.rowResult.failed", { defaultValue: "Failed" })}
        </Badge>
      );
  }
}
...
function formatRelativeShort(value: string | null | undefined): string {
  ...
  if (seconds < 60)
    return t("importFromVaultDialog.time.secondsAgo", { defaultValue: "{{seconds}}s ago", seconds });
  const minutes = Math.floor(seconds / 60);
  if (minutes < 60)
    return t("importFromVaultDialog.time.minutesAgo", { defaultValue: "{{minutes}}m ago", minutes });
  const hours = Math.floor(minutes / 60);
  if (hours < 48)
    return t("importFromVaultDialog.time.hoursAgo", { defaultValue: "{{hours}}h ago", hours });
  const days = Math.floor(hours / 24);
  if (days < 30)
    return t("importFromVaultDialog.time.daysAgo", { defaultValue: "{{days}}d ago", days });
...
function readableErrorMessage(error: unknown): string {
  if (error instanceof ApiError) {
    return (
      error.message ||
      t("importFromVaultDialog.errors.requestFailed", {
  ...
  return t("importFromVaultDialog.errors.unexpected", { defaultValue: "Unexpected error" });
}
...
function validateDraftRow(
  ...
  if (!draft.name.trim())
    return t("importFromVaultDialog.errors.nameRequired", { defaultValue: "Name is required." });
  if (draft.name.length > 160)
    return t("importFromVaultDialog.errors.nameTooLong", {
      defaultValue: "Name must be 160 characters or fewer.",
    });
  if (!draft.key.trim())
    return t("importFromVaultDialog.errors.keyRequired", { defaultValue: "Key is required." });
  if (!KEY_PATTERN.test(draft.key)) {
    return t("importFromVaultDialog.errors.keyPattern", {
      defaultValue: "Key may only contain lowercase letters, numbers, dot, underscore, or hyphen.",
    });
  }
  if (draft.key.length > 120)
    return t("importFromVaultDialog.errors.keyTooLong", {
      defaultValue: "Key must be 120 characters or fewer.",
    });
  if (draft.description.length > 500)
    return t("importFromVaultDialog.errors.descriptionTooLong", {
  ...
  for (const existingSecret of existing) {
    if (existingSecret.name.trim().toLowerCase() === lowerName) {
      return t("importFromVaultDialog.errors.duplicateName", {
        defaultValue: "A Paperclip secret already uses this name.",
      });
    }
    if (existingSecret.key.trim().toLowerCase() === lowerKey) {
      return t("importFromVaultDialog.errors.duplicateKey", {
  ...
  for (const other of otherDrafts) {
    ...
    if (other.name.trim().toLowerCase() === lowerName) {
      return t("importFromVaultDialog.errors.batchDuplicateName", {
        defaultValue: "Another row in this batch already uses this name.",
      });
    }
    if (other.key.trim().toLowerCase() === lowerKey) {
      return t("importFromVaultDialog.errors.batchDuplicateKey", {
...
export function ImportFromVaultDialog({
  ...
  const importMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      if (result.errorCount === draftList.length && result.errorCount > 0) {
        toast.pushToast({
          title: t("importFromVaultDialog.toast.importFailed", { defaultValue: "Import failed" }),
          body: t("importFromVaultDialog.toast.noneImported", {
        ...
        toast.pushToast({
          title:
            result.errorCount > 0
              ? t("importFromVaultDialog.toast.completedWithErrors", {
                  defaultValue: "Import completed with errors",
                })
              : t("importFromVaultDialog.toast.complete", { defaultValue: "Import complete" }),
          body: t("importFromVaultDialog.toast.summary", {
  ...
  function handleLoadMore() {
    ...
    secretsApi
      ...
      .catch((error) => {
        toast.pushToast({
          title: t("importFromVaultDialog.toast.loadMoreFailed", {
  ...
  function handleClose(force = false) {
    ...
    if (!force && step !== "result" && selection.size > 0 && !importResult) {
      const pending = selection.size;
      const ok = window.confirm(
        pending === 1
          ? t("importFromVaultDialog.confirm.discardOne", {
              defaultValue: "Discard {{pending}} pending import?",
              pending,
            })
          : t("importFromVaultDialog.confirm.discardMany", {
              defaultValue: "Discard {{pending}} pending imports?",
              pending,
            }),
      );
      if (!ok) return;
        ...
        <header className="flex items-start justify-between gap-3 border-b border-border/60 px-5 py-4">
          <div className="flex flex-col gap-1">
            ...
            <DialogDescription className="text-xs text-muted-foreground">
              {t("importFromVaultDialog.description", {
                defaultValue: "Bring AWS-managed secrets into Paperclip as external references.",
              })}
            </DialogDescription>
            <Stepper step={step} />
          </div>
          <button
            ...
            onClick={() => handleClose()}
            aria-label={t("importFromVaultDialog.aria.closeDialog", { defaultValue: "Close import dialog" })}
          >
            <X className="h-4 w-4" />
          </button>
        </header>
          ...
          <div className="flex items-center gap-2">
            {step !== "result" && (
              <Button variant="ghost" size="sm" onClick={() => handleClose()}>
                {t("importFromVaultDialog.actions.cancel", { defaultValue: "Cancel" })}
              </Button>
            )}
            {step === "review" && (
              <Button
              ...
              >
                {t("importFromVaultDialog.actions.back", { defaultValue: "Back" })}
              </Button>
            )}
            {step === "select" && (
              <Button
              ...
              >
                {t("importFromVaultDialog.actions.continueToReview", { defaultValue: "Continue → Review" })}
              </Button>
            )}
            {step === "review" && (
              <Button
              ...
              >
                {importMutation.isPending ? (
                  <>
                    ...
                    {t("importFromVaultDialog.actions.importing", { defaultValue: "Importing…" })}
                  </>
                ) : (
                  t("importFromVaultDialog.actions.import", {
            ...
            {step === "result" && (
              <Button size="sm" onClick={() => handleClose(true)}>
                {t("importFromVaultDialog.actions.done", { defaultValue: "Done" })}
              </Button>
            )}
          </div>
        </footer>
...
function Stepper({ step }: { step: Step }) {
  ...
  const steps: { id: Step; label: string }[] = [
    { id: "select", label: t("importFromVaultDialog.stepper.select", { defaultValue: "Select" }) },
    { id: "review", label: t("importFromVaultDialog.stepper.review", { defaultValue: "Review" }) },
    { id: "result", label: t("importFromVaultDialog.stepper.result", { defaultValue: "Result" }) },
  ];
  ...
  if (noEligibleVaults) {
    return (
      <div className="flex min-h-0 flex-1 items-center justify-center p-6" data-testid="select-empty-vaults">
        <EmptyState
          icon={Cloud}
          message={t("importFromVaultDialog.selectStep.noVaultConfigured", {
            defaultValue: "No AWS provider vault configured. Add one to import secrets.",
          })}
          action={
            onManageVaults
              ? t("importFromVaultDialog.selectStep.manageVaults", { defaultValue: "Manage vaults" })
              : undefined
          }
    ...
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="flex flex-wrap items-center gap-2 border-b border-border/60 px-5 py-3">
        <label className="text-xs uppercase tracking-wide text-muted-foreground">
          {t("importFromVaultDialog.selectStep.vaultLabel", { defaultValue: "Vault" })}
        </label>
        {awsVaults.length === 1 && eligible.length === 1 ? (
          ...
          >
            <SelectTrigger
              size="sm"
              className="text-xs"
              aria-label={t("importFromVaultDialog.selectStep.selectVaultAria", { defaultValue: "Select AWS vault" })}
            >
              <SelectValue
                placeholder={t("importFromVaultDialog.selectStep.selectVaultPlaceholder", {
        ...
        <div className="relative ml-auto w-64">
          ...
          <Input
            ...
            placeholder={t("importFromVaultDialog.selectStep.searchPlaceholder", {
              defaultValue: "Search by name, ARN, tag",
            })}
            className="pl-7 pr-7 text-xs"
            aria-label={t("importFromVaultDialog.selectStep.searchAria", { defaultValue: "Search remote secrets" })}
        ...
        <Button
          ...
          aria-label={t("importFromVaultDialog.selectStep.refreshAria", { defaultValue: "Refresh remote secrets" })}
        >
      ...
      {selectedNotVisible > 0 && (
        <div className="flex items-center justify-between border-b border-border/60 bg-muted/20 px-5 py-1.5 text-xs text-muted-foreground">
          <span>
            {t("importFromVaultDialog.selectStep.selectedNotVisible", {
              defaultValue: "{{selected}} selected · {{notVisible}} not visible with current search",
              selected: selection.size,
              notVisible: selectedNotVisible,
            })}
          </span>
          <Button
            variant="ghost"
          ...
          >
            {showOnlySelected
              ? t("importFromVaultDialog.selectStep.showAll", { defaultValue: "Show all" })
              : t("importFromVaultDialog.selectStep.showSelected", { defaultValue: "Show selected" })}
          </Button>
        </div>
      )}
      ...
      <div className="min-h-0 flex-1 overflow-y-auto" data-testid="vault-table-scroll">
              ...
              <tr>
                <th className="px-3 py-2 text-left">
                  <Checkbox
                    ...
                    aria-label={t("importFromVaultDialog.table.selectAllAria", {
                      defaultValue: "Select all loaded ({{total}})",
                      total: selectableInLoaded.length,
                    })}
                    disabled={selectableInLoaded.length === 0}
                  />
                </th>
                <th className="px-2 py-2 text-left font-medium">
                  {t("importFromVaultDialog.table.remoteName", { defaultValue: "Remote name" })}
                </th>
                <th className="px-2 py-2 text-left font-medium">
                  {t("importFromVaultDialog.table.reference", { defaultValue: "Reference" })}
                </th>
                <th className="px-2 py-2 text-left font-medium">
                  {t("importFromVaultDialog.table.lastChanged", { defaultValue: "Last changed" })}
                </th>
                <th className="px-2 py-2 text-left font-medium">
                  {t("importFromVaultDialog.table.suggestedName", { defaultValue: "Suggested name" })}
                </th>
                <th className="px-2 py-2 text-left font-medium">
                  {t("importFromVaultDialog.table.state", { defaultValue: "State" })}
                </th>
              </tr>
            </thead>
                  ...
                  >
                    <td className="px-3 py-2.5" onClick={(e) => e.stopPropagation()}>
                      <Checkbox
                        ...
                        aria-label={t("importFromVaultDialog.table.selectRowAria", {
                        ...
                        {candidate.status === "duplicate" &&
                          candidate.conflicts.find((c) => c.type === "exact_reference")?.existingSecretId && (
                            <span className="text-(length:--text-micro) text-muted-foreground">
                              {t("importFromVaultDialog.selectStep.alreadyImported", {
        ...
        {hasNextPage && !previewError && (
          <div className="flex items-center justify-between border-t border-border/60 px-5 py-2 text-xs text-muted-foreground">
            <span>
              {t("importFromVaultDialog.loadMore.loaded", {
              ...
              {selectableInLoaded.length > 0 && (
                <span>
                  {" "}
                  ·{" "}
                  {t("importFromVaultDialog.loadMore.selectable", {
                    defaultValue: "{{selectable}} selectable",
                    selectable: selectableInLoaded.length,
                  })}
                </span>
              )}
            </span>
            ...
            >
              {pageLoading ? (
                <>
                  <Loader2 className="mr-1.5 h-3.5 w-3.5 animate-spin" />{" "}
                  {t("importFromVaultDialog.loadMore.loading", { defaultValue: "Loading…" })}
                </>
              ) : (
                t("importFromVaultDialog.loadMore.loadMore", {
                  defaultValue: "Load {{amount}} more",
                  amount: PAGE_SIZE,
                })
              )}
            </Button>
    ...
    >
      ...
      <div className="flex-1">
        <div className="font-medium">
          {isPermission
            ? t("importFromVaultDialog.error.permissionTitle", { defaultValue: "AWS denied list access" })
            : isThrottling
              ? t("importFromVaultDialog.error.throttleTitle", {
                  defaultValue: "AWS throttled the listing request",
                })
              : t("importFromVaultDialog.error.genericTitle", {
                  defaultValue: "Could not load remote secrets",
                })}
        </div>
        <div className="mt-1 text-xs leading-relaxed text-destructive/80">
          {isPermission
            ? t("importFromVaultDialog.error.permissionBody", {
                defaultValue:
        ...
        <div className="mt-2 flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={onRetry}>
            ...
            {t("importFromVaultDialog.actions.retry", { defaultValue: "Retry" })}
          </Button>
          {isPermission && (
            <a
            ...
            >
              {t("importFromVaultDialog.error.iamReference", { defaultValue: "IAM reference" })}{" "}
...
function EmptyCandidates({ query }: { query: string }) {
  ...
  if (query) {
    return (
      <EmptyState
        icon={Search}
        message={t("importFromVaultDialog.emptyCandidates.noMatch", {
  ...
  return (
    <EmptyState
      icon={Database}
      message={t("importFromVaultDialog.emptyCandidates.noSecrets", {
  ...
  if (drafts.length === 0) {
    return (
      <div className="flex min-h-0 flex-1 items-center justify-center p-6">
        <EmptyState
          icon={Info}
          message={t("importFromVaultDialog.reviewStep.emptyMessage", {
    ...
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="flex flex-wrap items-center gap-3 border-b border-border/60 bg-muted/20 px-5 py-3 text-xs">
        <span className="font-medium">
          {t("importFromVaultDialog.reviewStep.readyCount", {
            defaultValue: "{{ready}} secrets ready to import",
            ready,
          })}
        </span>
        {blocked > 0 && (
          <span className="text-amber-600 dark:text-amber-400">
            {t("importFromVaultDialog.reviewStep.blockedCount", {
              defaultValue: "{{blocked}} need attention before import",
              blocked,
            })}
          </span>
        )}
            ...
            >
              <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
                    <label className="flex flex-col gap-1 text-xs">
                      <span className="text-muted-foreground">
                        {t("importFromVaultDialog.reviewStep.nameLabel", { defaultValue: "Paperclip name" })}
                      </span>
                      <Input
                        value={draft.name}
                        onChange={(e) =>
                    ...
                    <label className="flex flex-col gap-1 text-xs">
                      <span className="text-muted-foreground">
                        {t("importFromVaultDialog.reviewStep.keyLabel", { defaultValue: "Key" })}
                      </span>
                      <Input
                        value={draft.key}
                        onChange={(e) =>
                    ...
                    <label className="flex flex-col gap-1 text-xs">
                      <span className="text-muted-foreground">
                        {t("importFromVaultDialog.reviewStep.descriptionLabel", {
                ...
                <Button
                  ...
                  aria-label={t("importFromVaultDialog.reviewStep.removeAria", {
...
function ResultStep({ result, draftList }: ResultStepProps) {
  ...
  const heading =
    result.errorCount === result.results.length && result.errorCount > 0
      ? t("importFromVaultDialog.resultStep.headingFailed", { defaultValue: "Import failed" })
      : result.errorCount === 0 && result.skippedCount === 0
        ? t("importFromVaultDialog.resultStep.headingAllImported", {
            defaultValue: "All {{imported}} secrets imported",
            imported: result.importedCount,
          })
        : t("importFromVaultDialog.resultStep.headingComplete", { defaultValue: "Import complete" });
    ...
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="border-b border-border/60 px-5 py-3" data-testid="result-summary">
        ...
        <div className="mt-1 flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
          <span className="text-emerald-600 dark:text-emerald-400">
            ✓{" "}
            {t("importFromVaultDialog.resultStep.created", {
          ...
          <span>
            ⊘{" "}
            {t("importFromVaultDialog.resultStep.skipped", {
          ...
          <span className="text-destructive">
            ⨯{" "}
            {t("importFromVaultDialog.resultStep.failed", {
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        {grouped.created.length > 0 && (
          <ResultGroup
            label={t("importFromVaultDialog.resultStep.groupCreated", { defaultValue: "Created" })}
        ...
        {grouped.skipped.length > 0 && (
          <ResultGroup
            label={t("importFromVaultDialog.resultStep.groupSkipped", { defaultValue: "Skipped" })}
        ...
        {grouped.failed.length > 0 && (
          <ResultGroup
            label={t("importFromVaultDialog.resultStep.groupFailed", { defaultValue: "Failed" })}
            rows={grouped.failed}
            draftLookup={draftLookup}
          />
        )}
...
function FooterStatus({
  ...
  if (step === "select") {
    return (
      <div className="text-xs text-muted-foreground">
        {totalSelected === 0
          ? t("importFromVaultDialog.footerStatus.selectPrompt", {
              defaultValue: "Select remote secrets to import",
            })
          : t("importFromVaultDialog.footerStatus.selectedCount", {
  ...
  if (step === "review") {
    return (
      <div className="text-xs text-muted-foreground">
        {t("importFromVaultDialog.footerStatus.ready", {
          defaultValue: "{{ready}} ready",
          ready: readyReviewCount,
        })}
        {blockedReviewCount > 0 && (
          <span className="ml-2 text-amber-600 dark:text-amber-400">
            ·{" "}
            {t("importFromVaultDialog.footerStatus.blocked", {
              defaultValue: "{{blocked}} blocked",
              blocked: blockedReviewCount,
            })}
          </span>
        )}
      </div>
  ...
  if (result) {
    return (
      <div className="flex items-center gap-3 text-xs text-muted-foreground">
        <span>
          {t("importFromVaultDialog.footerStatus.created", {
            defaultValue: "{{created}} created",
            created: result.importedCount,
          })}
        </span>
        <span>
          {t("importFromVaultDialog.footerStatus.skipped", {
            defaultValue: "{{skipped}} skipped",
            skipped: result.skippedCount,
          })}
        </span>
        <span>
          {t("importFromVaultDialog.footerStatus.failed", {
            defaultValue: "{{failed}} failed",
            failed: result.errorCount,
          })}
        </span>
