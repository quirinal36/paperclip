...
export function MyUserSecretsTab({ companyId }: { companyId: string }) {
  ...
  const clear = useMutation({
    ...
    onSuccess: () => {
      ...
      pushToast({ title: t("myUserSecretsTab.toast.valueCleared", { defaultValue: "Value cleared" }), tone: "info" });
    },
    onError: (err) =>
      pushToast({
        title: t("myUserSecretsTab.toast.clearFailed", { defaultValue: "Could not clear value" }),
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-3 overflow-hidden">
      <div className="flex items-start gap-2 rounded-md border border-violet-500/30 bg-violet-500/5 px-4 py-3 text-xs text-violet-800 dark:text-violet-200">
        ...
        <p>
          {t("myUserSecretsTab.notice.body", {
            defaultValue:
          ...
          {missingCount > 0 ? (
            <span className="font-medium">
              {" "}
              {missingCount === 1
                ? t("myUserSecretsTab.notice.missingOne", {
                    count: missingCount,
                    defaultValue: "{{count}} required secret still needs your value.",
                  })
                : t("myUserSecretsTab.notice.missingOther", {
                    count: missingCount,
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        {mySecretsQuery.isError ? (
          <div className="flex items-center gap-2 py-4 text-sm text-destructive">
            ...
            {t("myUserSecretsTab.error.loadFailed", { defaultValue: "Failed to load your secrets:" })}{" "}
            {(mySecretsQuery.error as Error).message}
            <Button variant="ghost" size="sm" onClick={() => mySecretsQuery.refetch()}>
              {t("myUserSecretsTab.actions.retry", { defaultValue: "Retry" })}
            </Button>
          </div>
        ) : entries.length === 0 && !mySecretsQuery.isPending ? (
          <EmptyState
            icon={KeyRound}
            message={t("myUserSecretsTab.empty.message", {
              defaultValue:
      ...
      <div className="flex shrink-0 items-center gap-2">
        ...
        {!disabledDefinition ? (
          <Button size="sm" variant={secret ? "outline" : "default"} onClick={onSet}>
            {secret
              ? t("myUserSecretsTab.actions.update", { defaultValue: "Update" })
              : t("myUserSecretsTab.actions.setValue", { defaultValue: "Set value" })}
          </Button>
        ) : null}
        {secret ? (
          <Button
            ...
            onClick={onClear}
            disabled={clearing}
            title={t("myUserSecretsTab.row.clearTitle", { defaultValue: "Clear my value" })}
          >
