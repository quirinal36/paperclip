...
export function RunYourOwnTab({ companyId }: { companyId: string }) {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      pushToast({
        title: t("runYourOwnTab.toast.added.title", { defaultValue: "Tool added" }),
        body: t("runYourOwnTab.toast.added.body", {
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-5 rounded-lg border border-border bg-card p-5">
        <div className="space-y-1.5">
          <Label htmlFor="ryo-name">{t("runYourOwnTab.fields.name.label", { defaultValue: "Name" })}</Label>
          <Input
            id="ryo-name"
            value={name}
            onChange={(event) => setName(event.target.value)}
            placeholder={t("runYourOwnTab.fields.name.placeholder", { defaultValue: "Acme tools" })}
            maxLength={160}
          />
          <p className="text-xs text-muted-foreground">
            {t("runYourOwnTab.fields.name.help", {
        ...
        <div className="space-y-1.5">
          <Label htmlFor="ryo-command">{t("runYourOwnTab.fields.command.label", { defaultValue: "Command" })}</Label>
          <Input
            id="ryo-command"
            value={command}
          ...
          />
          <p className="text-xs text-muted-foreground">
            {t("runYourOwnTab.fields.command.help", {
        ...
        <div className="space-y-2">
          <div className="flex items-baseline gap-2">
            <Label>{t("runYourOwnTab.fields.keys.label", { defaultValue: "Keys it needs" })}</Label>
            <span className="text-xs text-muted-foreground">
              · {t("runYourOwnTab.fields.keys.optionalHint", { defaultValue: "optional, depends on the tool" })}
            </span>
          </div>
          {keyRows.length > 0 ? (
            <div className="space-y-2">
                  ...
                  <div key={row.id} className="space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      <Button
                        ...
                        variant="ghost"
                        size="icon"
                        aria-label={t("runYourOwnTab.fields.keys.removeAria", { defaultValue: "Remove key" })}
                    ...
                    {invalid ? (
                      <p className="text-xs text-destructive">
                        {t("runYourOwnTab.fields.keys.invalid", {
                          defaultValue:
          ...
          <Button type="button" variant="outline" size="sm" onClick={addKeyRow} className="gap-1.5">
            ...
            {t("runYourOwnTab.fields.keys.add", { defaultValue: "Add a key" })}
          </Button>
        </div>
        ...
        <div className="flex items-start gap-2.5 rounded-md bg-muted/50 px-3 py-2.5">
          ...
          <div className="text-xs">
            <p className="font-medium text-foreground">
              {t("runYourOwnTab.isolation.title", {
                defaultValue: "This runs in your company's own workspace, isolated from everything else.",
              })}
            </p>
            <p className="mt-0.5 flex items-center gap-1 text-muted-foreground">
              <Lock className="h-3 w-3" />
              {t("runYourOwnTab.isolation.adminsOnly", { defaultValue: "Only administrators see this option." })}
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <Button onClick={() => createMutation.mutate()} disabled={!canSubmit || createMutation.isPending}>
            {createMutation.isPending
              ? t("runYourOwnTab.actions.adding", { defaultValue: "Adding…" })
              : t("runYourOwnTab.actions.checkAndContinue", { defaultValue: "Check & continue" })}
          </Button>
          <span className="text-xs text-muted-foreground">
            {t("runYourOwnTab.actions.registerHint", {
      ...
      <div className="space-y-3">
        <h3 className="text-sm font-semibold text-foreground">
          {t("runYourOwnTab.list.heading", { defaultValue: "Your own tools" })}
        </h3>
        {templates.isLoading ? (
          <LoadingState />
        ) : templates.isError ? (
          ...
          <p className="text-sm text-muted-foreground">
            {t("runYourOwnTab.list.empty", { defaultValue: "You haven't added any of your own tools yet." })}
          </p>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-muted/40 text-left text-(length:--text-micro) font-semibold uppercase tracking-wide text-muted-foreground">
                  <th className="px-4 py-2.5">{t("runYourOwnTab.list.columns.name", { defaultValue: "Name" })}</th>
                  <th className="px-4 py-2.5">{t("runYourOwnTab.list.columns.command", { defaultValue: "Command" })}</th>
                  <th className="px-4 py-2.5">{t("runYourOwnTab.list.columns.keys", { defaultValue: "Keys" })}</th>
                  <th className="px-4 py-2.5">{t("runYourOwnTab.list.columns.added", { defaultValue: "Added" })}</th>
...
function RunYourOwnRow({
  ...
  const disableMutation = useMutation({
    ...
    onSuccess: () => {
      pushToast({ title: t("runYourOwnTab.toast.turnedOff.title", { defaultValue: "Tool turned off" }), tone: "success" });
      qc.invalidateQueries({ queryKey: queryKeys.tools.stdioTemplates(companyId) });
    },
    onError: (error) => {
      pushToast({
        title: t("runYourOwnTab.toast.turnOffError.title", { defaultValue: "Couldn't turn it off" }),
  ...
  return (
    <tr className="border-b border-border last:border-0">
      <td className="px-4 py-3">
        ...
        {disabled ? (
          <Badge variant="outline">{t("runYourOwnTab.row.off", { defaultValue: "off" })}</Badge>
        ) : null}
      </td>
      <td className="px-4 py-3">
      ...
      <td className="px-4 py-3 text-muted-foreground">
        {template.envKeys.length > 0
          ...
          : t("runYourOwnTab.row.noKeys", { defaultValue: "none" })}
      </td>
      <td className="px-4 py-3">
      ...
      <td className="px-4 py-3 text-right">
        {disabled ? null : (
          ...
          >
            {t("runYourOwnTab.row.turnOff", { defaultValue: "Turn off" })}
          </Button>
        )}
      </td>
    </tr>
