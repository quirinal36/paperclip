  ...
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>
            {t("connectionDialogs.catalog.title", {
              defaultValue: "Tool catalog — {{name}}",
        ...
        {catalog.isLoading ? (
          ...
          <p className="py-6 text-sm text-muted-foreground">
            {t("connectionDialogs.catalog.empty", {
              defaultValue:
...
export function AddConnectionDialog({
  ...
  const create = useMutation({
    ...
    onError: (err) =>
      pushToast({
        title: t("connectionDialogs.toasts.createError", { defaultValue: "Could not create connection" }),
  ...
  const probe = useMutation({
    ...
    onError: (err) =>
      pushToast({
        title: t("connectionDialogs.toasts.probeError", { defaultValue: "Probe failed" }),
        body: err instanceof ApiError ? err.message : String(err),
        tone: "error",
      }),
  });
  ...
  const activate = useMutation({
    ...
    onSuccess: () => {
      ...
      pushToast({
        title: t("connectionDialogs.toasts.activated", { defaultValue: "Connection activated" }),
        tone: "success",
      });
      onClose();
    },
    onError: (err) =>
      pushToast({
        title: t("connectionDialogs.toasts.activateError", { defaultValue: "Activation failed" }),
        body: err instanceof ApiError ? err.message : String(err),
        tone: "error",
      }),
  });
  ...
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-xl">
        <DialogHeader>
          <DialogTitle>{t("connectionDialogs.dialog.title", { defaultValue: "Add application" })}</DialogTitle>
          <DialogDescription>
            {t("connectionDialogs.dialog.description", {
              defaultValue:
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            <span className={step === 1 ? "font-medium text-foreground" : ""}>
              {t("connectionDialogs.steps.application", { defaultValue: "1 Application" })}
            </span>
            <span>/</span>
            <span className={step === 2 ? "font-medium text-foreground" : ""}>
              {t("connectionDialogs.steps.connection", { defaultValue: "2 Connection" })}
            </span>
          </div>
          ...
          {step === 1 && !locked ? (
            <>
              <div className="space-y-1.5">
                <Label>{t("connectionDialogs.fields.application", { defaultValue: "Application" })}</Label>
                <Select value={applicationMode} onValueChange={(v) => setApplicationMode(v as "existing" | "new")}>
                  ...
                  <SelectContent>
                    <SelectItem value="existing">
                      {t("connectionDialogs.applicationMode.useExisting", {
                        defaultValue: "Use existing application",
                      })}
                    </SelectItem>
                    <SelectItem value="new">
                      {t("connectionDialogs.applicationMode.createNew", {
                        defaultValue: "Create new application",
                      })}
                    </SelectItem>
                  </SelectContent>
                </Select>
              ...
              {applicationMode === "existing" ? (
                <div className="space-y-1.5">
                  <Label>
                    {t("connectionDialogs.fields.existingApplication", { defaultValue: "Existing application" })}
                  </Label>
                  <Select value={applicationId} onValueChange={setApplicationId}>
                    <SelectTrigger>
                      <SelectValue
                        placeholder={t("connectionDialogs.placeholders.selectApplication", {
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="app-name">
                    {t("connectionDialogs.fields.newApplicationName", { defaultValue: "New application name" })}
                  </Label>
                  <Input
                    id="app-name"
                    value={applicationName}
                    onChange={(e) => setApplicationName(e.target.value)}
                    placeholder={t("connectionDialogs.placeholders.newApplicationExample", {
                      defaultValue: "e.g. GitHub Triage",
                    })}
                  />
                  <p className="text-xs text-muted-foreground">
                    {t("connectionDialogs.hints.applicationTypeInferred", {
            ...
            <>
              {applicationMode === "new" ? (
                <div className="rounded-md border border-border bg-muted/30 px-3 py-2 text-xs text-muted-foreground">
                  ...
                  {t("connectionDialogs.newApp.willBeCreatedAs", {
                    defaultValue: "will be created as {{type}}.",
                    type: inferredType,
                  })}
                </div>
              ) : null}
              ...
              <div className="space-y-1.5">
                <Label htmlFor="conn-name">
                  {t("connectionDialogs.fields.connectionName", { defaultValue: "Connection name" })}
                </Label>
                <Input
                  id="conn-name"
                  value={name}
                  onChange={(e) => setName(e.target.value)}
                  placeholder={t("connectionDialogs.placeholders.connectionNameExample", {
              ...
              <div className="space-y-1.5">
                <Label>{t("connectionDialogs.fields.transport", { defaultValue: "Transport" })}</Label>
                <Select
                  value={transport}
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="mcp_remote">
                      {t("connectionDialogs.transport.remoteHttp", {
                        defaultValue: "Remote HTTP (no local process)",
                      })}
                    </SelectItem>
                    <SelectItem value="local_stdio">
                      {t("connectionDialogs.transport.localStdio", {
              ...
              {transport === "mcp_remote" ? (
                <div className="space-y-1.5">
                  <Label htmlFor="conn-url">
                    {t("connectionDialogs.fields.endpointUrl", { defaultValue: "Endpoint URL" })}
                  </Label>
                  <Input
                    id="conn-url"
                    value={endpointUrl}
                ...
                <div className="space-y-1.5">
                  <Label>{t("connectionDialogs.fields.commandTemplate", { defaultValue: "Command template" })}</Label>
                  <Select value={templateId} onValueChange={setTemplateId} disabled={locked}>
                    <SelectTrigger>
                      <SelectValue
                        placeholder={t("connectionDialogs.placeholders.selectTemplate", {
                  ...
                  <p className="text-xs text-muted-foreground">
                    {t("connectionDialogs.hints.templatesApprovedOnly", {
                      defaultValue:
              ...
              <div className="space-y-1.5">
                <Label>
                  {t("connectionDialogs.fields.credentialReferences", { defaultValue: "Credential references" })}
                </Label>
                {creds.length > 0 ? (
                      ...
                      >
                        ...
                        {!locked ? (
                          <button
                            ...
                            aria-label={t("connectionDialogs.credentials.removeRef", {
                ...
                {!locked ? (
                  <>
                    <div className="flex items-end gap-2">
                      <div className="flex-1 space-y-1">
                        <Select value={pendingSecretId} onValueChange={setPendingSecretId}>
                          <SelectTrigger>
                            <SelectValue
                              placeholder={t("connectionDialogs.placeholders.selectSecret", {
                      ...
                      <Input
                        ...
                        placeholder={t("connectionDialogs.placeholders.header", { defaultValue: "Header" })}
                        className="w-32"
                        aria-label={t("connectionDialogs.credentials.headerName", { defaultValue: "Header name" })}
                      />
                      <Button type="button" size="sm" variant="outline" onClick={addCred} disabled={!pendingSecretId}>
                        {t("connectionDialogs.actions.add", { defaultValue: "Add" })}
                      </Button>
                    </div>
                    {pendingSecretId ? (
                    ...
                    <p className="text-xs text-muted-foreground">
                      {t("connectionDialogs.credentials.vaultOnlyPrefix", {
                        defaultValue:
                          "Free-text secrets are not accepted — pick a vault entry; Paperclip stores only the",
                      })}
                      <span className="font-mono"> vault://</span>{" "}
                      {t("connectionDialogs.credentials.vaultOnlySuffix", {
                        defaultValue: "reference and resolves it at gateway use time.",
                      })}
                    </p>
                  </>
                ) : null}
          ...
          {locked ? (
            probe.isPending ? (
              <div className="rounded-md border border-border bg-muted/40 p-3">
                <LoadingState
                  label={t("connectionDialogs.probe.loading", { defaultValue: "Probing connection…" })}
                />
              </div>
            ) : probe.isError ? (
              ...
              <div className="rounded-md border border-border bg-muted/40 p-3">
                <div className="flex items-center gap-2 text-sm">
                  <span className="font-medium text-foreground">
                    {t("connectionDialogs.probe.result", { defaultValue: "Probe result" })}
                  </span>
                ...
                <div className="mt-2 grid grid-cols-3 gap-2">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {t("connectionDialogs.probe.toolsDiscovered", { defaultValue: "tools discovered" })}
                    </p>
                  </div>
                  <div>
                    <p className="text-lg font-semibold tabular-nums text-foreground">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {t("connectionDialogs.probe.probeLatency", { defaultValue: "probe latency" })}
                    </p>
                  </div>
                  <div>
                    <p className="text-lg font-semibold tabular-nums text-foreground">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {t("connectionDialogs.probe.quarantined", { defaultValue: "quarantined" })}
                    </p>
                  </div>
                </div>
                ...
                <p className="mt-2 text-(length:--text-micro) text-muted-foreground">
                  {t("connectionDialogs.probe.latencyNote", {
                    defaultValue:
        ...
        <DialogFooter>
          <Button variant="outline" onClick={onClose}>
            {t("connectionDialogs.actions.cancel", { defaultValue: "Cancel" })}
          </Button>
          {step === 1 && !locked ? (
            <Button disabled={!appChoiceValid} onClick={() => setStep(2)}>
              {t("connectionDialogs.actions.continue", { defaultValue: "Continue" })}
            </Button>
          ) : !locked ? (
            <>
              <Button variant="outline" onClick={() => setStep(1)}>
                {t("connectionDialogs.actions.back", { defaultValue: "Back" })}
              </Button>
              <Button disabled={!canCreate} onClick={() => create.mutate()}>
                {create.isPending
                  ? t("connectionDialogs.actions.creatingDraft", { defaultValue: "Creating draft…" })
                  : t("connectionDialogs.actions.createAndProbe", { defaultValue: "Create & probe" })}
              </Button>
            </>
          ) : (
            <>
              <Button variant="outline" disabled={probe.isPending} onClick={() => draft && probe.mutate(draft.id)}>
                ...
                {probe.isPending
                  ? t("connectionDialogs.actions.probing", { defaultValue: "Probing…" })
                  : t("connectionDialogs.actions.reProbe", { defaultValue: "Re-probe" })}
              </Button>
              <Button disabled={activate.isPending || probe.isPending} onClick={() => draft && activate.mutate(draft.id)}>
                {activate.isPending
                  ? t("connectionDialogs.actions.activating", { defaultValue: "Activating…" })
                  : t("connectionDialogs.actions.activate", { defaultValue: "Activate" })}
              </Button>
            </>
          )}
        </DialogFooter>
