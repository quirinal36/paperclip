...
export function ProfilesIndex({
  ...
  const duplicate = useMutation({
    ...
    onSuccess: () => {
      pushToast({
        title: t("profilesIndex.toast.duplicated.title", { defaultValue: "Profile duplicated" }),
        body: t("profilesIndex.toast.duplicated.body", { defaultValue: "The copy is not assigned to anyone yet." }),
        tone: "success",
      });
      invalidate();
    },
    onError: (error: unknown) =>
      pushToast({
        title: t("profilesIndex.toast.duplicateError", { defaultValue: "Could not duplicate" }),
  ...
  const archive = useMutation({
    ...
    onSuccess: () => {
      pushToast({ title: t("profilesIndex.toast.archived", { defaultValue: "Profile archived" }), tone: "success" });
      invalidate();
    },
    onError: (error: unknown) =>
      pushToast({
        title: t("profilesIndex.toast.archiveError", { defaultValue: "Could not archive" }),
        body: errorBody(error),
        tone: "error",
      }),
  });
  ...
  const restore = useMutation({
    ...
    onSuccess: () => {
      pushToast({ title: t("profilesIndex.toast.restored", { defaultValue: "Profile restored" }), tone: "success" });
      invalidate();
    },
    onError: (error: unknown) =>
      pushToast({
        title: t("profilesIndex.toast.restoreError", { defaultValue: "Could not restore" }),
        body: errorBody(error),
        tone: "error",
      }),
  });
  ...
  const remove = useMutation({
    ...
    onSuccess: () => {
      pushToast({ title: t("profilesIndex.toast.deleted", { defaultValue: "Profile deleted" }), tone: "success" });
      invalidate();
    },
    onError: (error: unknown) =>
      pushToast({
        title: t("profilesIndex.toast.deleteError", { defaultValue: "Could not delete" }),
        body: errorBody(error),
        tone: "error",
      }),
  });
  ...
  const header = (
    <ToolsPageHeader
      title={t("profilesIndex.header.title", { defaultValue: "Access profiles" })}
      description={t("profilesIndex.header.description", {
        defaultValue:
          "Decide which tools your agents can use. Build a profile once, then assign it to the agents that need it.",
      })}
      actions={
        <>
          <Button variant="outline" onClick={() => setResolverOpen(true)}>
            <ShieldCheck className="mr-1.5 h-4 w-4" />
            {t("profilesIndex.header.checkAccess", { defaultValue: "Check an agent's access" })}
          </Button>
          <Button onClick={() => navigate(newProfileHref())}>
            <Plus className="mr-1.5 h-4 w-4" />
            {t("profilesIndex.header.newProfile", { defaultValue: "New profile" })}
          </Button>
        </>
      }
    />
    ...
    <Sheet open={resolverOpen} onOpenChange={setResolverOpen}>
      <SheetContent className="w-full gap-0 p-0 sm:max-w-xl">
        <SheetHeader className="border-b border-border">
          <SheetTitle>{t("profilesIndex.resolver.title", { defaultValue: "Check an agent's access" })}</SheetTitle>
          <SheetDescription>
            {t("profilesIndex.resolver.description", {
              defaultValue:
  ...
  if (profiles.isLoading) {
    return (
      <div className="space-y-5">
        {header}
        <LoadingState label={t("profilesIndex.states.loading", { defaultValue: "Loading profiles…" })} />
      </div>
    );
  }
  if (profiles.isError) {
    ...
    <div className="space-y-5">
      ...
      <div className="inline-flex rounded-md border border-border p-0.5">
        {(["active", "archived"] as const).map((key) => (
          ...
          >
            {key === "active"
              ? t("profilesIndex.filter.active", { defaultValue: "Active" })
              : t("profilesIndex.filter.archived", { defaultValue: "Archived" })}
          </button>
        ))}
      </div>
      ...
      {rows.length === 0 ? (
        statusFilter === "archived" ? (
          <div className="rounded-lg border border-dashed border-border px-4 py-5 text-sm text-muted-foreground">
            {t("profilesIndex.empty.noArchived", { defaultValue: "No archived profiles." })}
          </div>
        ) : (
          ...
          <table className="w-full table-fixed text-sm">
            ...
            <thead>
              <tr className="border-b border-border bg-muted/40 text-left text-xs font-medium text-muted-foreground">
                <th className="px-3 py-2 font-medium">{t("profilesIndex.table.profile", { defaultValue: "Profile" })}</th>
                <th className="px-3 py-2 font-medium">{t("profilesIndex.table.allows", { defaultValue: "Allows" })}</th>
                <th className="px-3 py-2 font-medium">{t("profilesIndex.table.assignedTo", { defaultValue: "Assigned to" })}</th>
                <th className="px-3 py-2 font-medium">{t("profilesIndex.table.status", { defaultValue: "Status" })}</th>
                <th className="px-3 py-2 font-medium">{t("profilesIndex.table.updated", { defaultValue: "Updated" })}</th>
                      ...
                      <span className="inline-flex items-center gap-2">
                        ...
                        {(profile.newToolsPendingCount ?? 0) > 0 ? (
                          <Badge variant="outline" className="border-amber-500/50 bg-amber-500/10 text-amber-800 dark:text-amber-200">
                            {t("profilesIndex.table.newToolsCount", {
                              defaultValue: "{{newCount}} new",
                              newCount: profile.newToolsPendingCount,
                            })}
                          </Badge>
                        ) : null}
                      </span>
                      ...
                      {assigned.unassigned ? (
                        <span className="text-muted-foreground">
                          ...
                          <span className="ml-1 text-xs text-muted-foreground/70">
                            {t("profilesIndex.table.doesNotChangeAccess", { defaultValue: "— does not change access" })}
                          </span>
                        </span>
                      ) : (
                    ...
                    <td className="px-3 py-1.5">
                      <span className="inline-flex items-center gap-2">
                        ...
                        {draft ? (
                          ...
                          >
                            {t("profilesIndex.actions.resume", { defaultValue: "Resume" })}
                          </button>
                        ) : null}
                      </span>
                    </td>
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          type="button"
          aria-label={t("profilesIndex.menu.actionsLabel", { defaultValue: "Profile actions" })}
      ...
      <DropdownMenuContent align="end">
        <DropdownMenuItem onSelect={onEdit}>{t("profilesIndex.menu.edit", { defaultValue: "Edit" })}</DropdownMenuItem>
        <DropdownMenuItem onSelect={onDuplicate}>{t("profilesIndex.menu.duplicate", { defaultValue: "Duplicate" })}</DropdownMenuItem>
        {onRestore ? (
          <DropdownMenuItem onSelect={onRestore}>
            <ArchiveRestore className="mr-1.5 h-4 w-4" />
            {t("profilesIndex.menu.restore", { defaultValue: "Restore" })}
          </DropdownMenuItem>
        ) : (
          <DropdownMenuItem onSelect={onArchive}>{t("profilesIndex.menu.archive", { defaultValue: "Archive" })}</DropdownMenuItem>
        )}
        <DropdownMenuSeparator />
        <DropdownMenuItem onSelect={onDelete} className="text-destructive focus:text-destructive">
          {t("profilesIndex.menu.delete", { defaultValue: "Delete" })}
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
  );
    ...
    <div className="rounded-lg border border-dashed border-border p-6">
      <div className="mb-4 max-w-2xl">
        <h3 className="text-base font-semibold text-foreground">
          {t("profilesIndex.emptyPicker.title", { defaultValue: "Create your first access profile" })}
        </h3>
        <p className="text-sm text-muted-foreground">
          {t("profilesIndex.emptyPicker.description", {
            defaultValue: "Pick a starting point. You can fine-tune exactly which tools it allows in the next step.",
          })}
        </p>
      </div>
