  ...
  if (boardAccess.isLoading) {
    return (
      <div className="mx-auto max-w-xl py-10 text-sm text-muted-foreground">
        {t("toolsAdminGate.loading", { defaultValue: "Loading…" })}
      </div>
    );
  }
        ...
        <div className="flex flex-col gap-3 rounded-lg border border-border bg-card p-6">
          <div className="flex items-center gap-2 text-foreground">
            ...
            <h1 className="text-lg font-semibold">
              {t("toolsAdminGate.notAdmin.title", {
                defaultValue: "Access profiles are for administrators",
              })}
            </h1>
          </div>
          <p className="text-sm text-muted-foreground">
            {t("toolsAdminGate.notAdmin.description", {
              defaultValue:
                "Access profiles decide which tools your agents can use. Ask an administrator to set these up, or head back to",
            })}{" "}
            <Link to="/apps" className="font-medium text-primary hover:underline">
              {t("toolsAdminGate.notAdmin.appsLink", { defaultValue: "your apps" })}
            </Link>
            .
          </p>
        </div>
