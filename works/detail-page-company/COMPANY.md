...
- 이슈에 첨부된 상품 이미지만 제품 정체성을 보존하기 위한 입력으로 허용한다.
- 판매 페이지·브랜드 사이트·경쟁사 페이지에서 캡처하거나 다운로드한 이미지는 분석 전용이며 최종 소재로 재사용하지 않는다.
- 최종 `image_assets`는 전부 Higgsfield MCP의 생성 또는 처리 결과여야 한다. 첨부 원본을 직접 `<img>`로 렌더하지 않는다.
- 상품 이미지는 Higgsfield로 업스케일·누끼·여백 확장·배경 합성 등을 거쳐 고급화한다.
- 사람 모델, 얼굴, 손, 포즈, 배경, 소품은 참고 이미지에서 가져오지 않는다. 모델과 얼굴은 Higgsfield가 새로 생성한다.
