...
| 대상 | 방법 |
|---|---|
| 일반 웹페이지·브랜드 사이트 | `WebFetch` |
| **네이버 스마트스토어·쿠팡·올리브영·네이버 블로그** 등 차단되는 국내 커머스 | `WebFetch`가 402/403으로 막힌다. `insane-search` 스킬로 우회한다 |
| YouTube 등 영상 | `insane-search` 스킬(yt-dlp)로 **자막·설명·제목**을 뽑는다 |
