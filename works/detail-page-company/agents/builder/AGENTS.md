...
제품 우선 컷은 다음 기준으로 조판한다.

- hero_packshot과 concept_scene은 제품 foreground가 캔버스 폭의 55~75%, 섹션 높이의 55~80%를 차지하도록 배치한다.
- 제품 주인공 섹션은 최소 1,200px 높이로 잡는다. 제품을 카드·썸네일·작은 인셋으로 줄여 텍스트 옆에 붙이지 않는다.
- 제품은 contain 방식으로 전체 형태와 라벨을 보존한다. 용기·뚜껑·라벨을 자르거나 카피에 가리지 않게 안전 여백을 둔다.
- model_in_use는 사용 맥락을 위한 예외로 제품이 작을 수 있다. 단, page 안에 hero_packshot 또는 concept_scene 제품 주인공 컷이 반드시 있어야 한다.
- page_spec에 subject_coverage가 없더라도 위 기본값을 적용하며, 카피를 줄여 제품 크기를 우선한다.
