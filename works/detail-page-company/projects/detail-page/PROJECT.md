...
- 이미지 없이 자사 판매 페이지만 들어오면 analyst는 참고 자료 분석까지만 진행하고
  `attached_product_image_required`를 남긴다. designer는 사용자가 상품 이미지를 첨부할 때까지 진행하지 않는다.

### 이미지 출처 정책

- 참고 페이지에서 캡처·다운로드한 이미지는 **분석 전용**이다. 최종 상세페이지에 재사용하지 않는다.
- 최종 이미지의 제품 픽셀은 이슈 첨부 상품 이미지만 입력으로 삼고, 원본 URL을 직접 렌더하지 않는다.
- 제품 이미지 향상, 누끼, 배경, 컨셉 장면, 사람 모델과 얼굴은 모두 Higgsfield MCP로 생성·처리한다.
- 참고 페이지나 첨부 이미지 속 사람의 얼굴·신체·포즈·배경·소품을 재사용하지 않는다.
